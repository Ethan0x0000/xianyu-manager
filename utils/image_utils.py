...
class ImageManager:
    ...
    def __init__(self, upload_dir: str = "data/uploads/images"):
        """初始化图片管理器
    ...
    def save_image(
        self, image_data: bytes, original_filename: str = None
    ) -> Optional[str]:
        """保存图片文件
    ...
    def _validate_image_data(self, image_data: bytes) -> bool:
        ...
        try:
            ...
            if len(image_data) > self.max_size:
                logger.warning(
                    f"图片文件过大: {len(image_data)} bytes > {self.max_size} bytes"
                )
                return False
            ...
            with Image.open(BytesIO(image_data)) as img:
                ...
                if total_pixels > max_pixels:
                    logger.warning(
                        f"图片像素总数过大: {total_pixels}，最大允许: {max_pixels}"
                    )
                    return False
        ...
        try:
            ...
            with Image.open(BytesIO(image_data)) as img:
                ...
                if width > max_output_dimension or height > max_output_dimension:
                    # 计算缩放比例，保持宽高比
                    ratio = min(
                        max_output_dimension / width, max_output_dimension / height
                    )
                    new_width = int(width * ratio)
                    new_height = int(height * ratio)
                    img = img.resize((new_width, new_height), Image.Resampling.LANCZOS)
                    logger.info(
                        f"图片已调整尺寸: {width}x{height} -> {new_width}x{new_height}"
                    )
                else:
