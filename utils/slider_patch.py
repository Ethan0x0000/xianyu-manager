...
def _handle_slider_verification(page, user_id: str, max_attempts: int = 5) -> bool:
    ...
    try:
        ...
        # 如果检测到滑块，尝试处理（有限次数重试）
        logger.warning(
            f"【{user_id}】检测到滑块验证，开始处理（最多尝试{max_attempts}次）..."
        )
        ...
        while attempt < max_attempts:
            ...
            try:
                    ...
                    try:
                        ...
                        if track_box and slider_box:
                            ...
                            if track_width and slider_width:
                                ...
                                logger.info(
                                    f"【{user_id}】计算滑动距离: {distance}px (轨道宽度: {track_width}px, 滑块宽度: {slider_width}px)"
                                )
                            elif track_width:
                                # 如果只有轨道宽度，使用轨道宽度的80%作为滑动距离
                                distance = int(track_width * 0.8)
                                logger.info(
                                    f"【{user_id}】使用轨道宽度的80%作为滑动距离: {distance}px"
                                )
                ...
                try:
                    ...
                    try:
                        ...
                        try:
                            ...
                            if not is_visible:
                                ...
                                logger.debug(
                                    f"【{user_id}】page 类型: {type(page).__name__}, real_page 类型: {type(real_page).__name__}"
                                )

                                try:
                                    logger.info(
                                        f"【{user_id}】刷新浏览器页面（重新加载）..."
                                    )
                                    logger.info(
                                        f"【{user_id}】当前URL: {real_page.url}"
                                    )
                                    ...
                                    logger.info(
                                        f"【{user_id}】刷新后URL: {real_page.url}"
                                    )
                                    logger.info(f"【{user_id}】重新开始验证...")

                                    # 递归调用自身重新验证（减少尝试次数避免无限循环）
                                    return _handle_slider_verification(
                                        real_page,
                                        user_id,
                                        max_attempts=max(5, max_attempts - 2),
                                    )

                                except Exception as refresh_e:
                                    logger.error(
                                        f"【{user_id}】刷新页面失败: {refresh_e}"
                                    )
                                    return False
                            else:
                    ...
                    if slider_visible:
                        try:
                            ...
                            logger.debug(
                                f"【{user_id}】滚动到滑块失败: {scroll_e}，跳过此步骤"
                            )
                ...
                try:
                    # 调用复用的滑块拖动函数
                    drag_success = _execute_slider_drag(
                        page, slider_element, distance, user_id
                    )
                    ...
                    try:
                        ...
                        for selector in failure_selectors:
                            try:
                                ...
                                if selector.startswith("text:"):
                                    ...
                                    failure_msg = page.locator(
                                        f"text={text_content}"
                                    ).first
                                    if failure_msg.count() > 0:
                                        logger.warning(
                                            f"【{user_id}】⚠️ 检测到验证失败提示: {selector}"
                                        )
                        ...
                        if verification_failed:
                            ...
                            for retry_selector in retry_selectors:
                                try:
                                    if retry_selector.startswith("text:"):
                                        text_content = retry_selector.replace(
                                            "text:", ""
                                        )
                                        retry_element = page.locator(
                                            f"text={text_content}"
                                        ).first
                                        if retry_element.count() > 0:
                                            logger.info(
                                                f"【{user_id}】找到重试元素: {retry_selector}，点击重试..."
                                            )
                                            retry_element.click()
                                            time.sleep(1.5)
                                            retry_clicked = True
                                        ...
                                        retry_element = page.query_selector(
                                            retry_selector
                                        )
                                        if retry_element:
                                            logger.info(
                            ...
                            if not retry_clicked:
                                ...
                                try:
                                    # 尝试点击包含错误消息的元素
                                    error_elem1 = page.locator(
                                        'xpath=//*[contains(text(), "验证失败")]'
                                    ).first
                        ...
                        try:
                            ...
                            if not slider_check:
                                ...
                                try:
                                    ...
                                    if (
                                        "验证失败" in page_html_check
                                        or "点击框体重试" in page_html_check
                                    ):
                                ...
                                if not has_failure_text:
                                    ...
                                    if all_missing:
                                        logger.info(
                                            f"【{user_id}】✅ 所有滑块元素都已消失且无失败消息，验证成功！"
                                        )
                                        verification_success = True
                                    else:
                                        logger.debug(
                                            f"【{user_id}】滑块元素消失但仍有其他滑块元素存在"
                                        )
                                else:
                                    logger.warning(
                                        f"【{user_id}】⚠️ 滑块元素消失但检测到失败消息，判定为失败"
                                    )
                                    verification_failed = True
                            else:
                            ...
                            logger.debug(
                                f"【{user_id}】检查验证成功时出错: {success_check_e}"
                            )
                    ...
                    if verification_success:
                        ...
                        logger.warning(
                            f"【{user_id}】⚠️ 滑块验证失败，已点击重试，准备重新滑动..."
                        )
                        # 不返回，继续循环重试
                    else:
        ...
        # 达到最大尝试次数，刷新页面重试
        logger.error(
            f"【{user_id}】❌ 滑块验证失败：已达到最大尝试次数({max_attempts})，准备刷新页面重试..."
        )
        ...
        try:
            ...
            if slider_appeared:
                ...
                logger.warning(
                    f"【{user_id}】⚠️ 刷新后未检测到滑块，检查是否已登录成功..."
                )
        ...
        def new_check_date_validity(self) -> bool:
            ...
            try:
                ...
                history_file = os.path.join(
                    trajectory_history_dir, f"{self.user_id}_history.json"
                )
                ...
                if days_diff > max_days:
                    logger.warning(
                        f"【{self.user_id}】日期验证失败，已超过最大使用天数 {max_days} 天（实际: {days_diff} 天）"
                    )
                    return False
                ...
                try:
                    ...
                    if history:
                        ...
                        if latest_timestamp > 0:
                            ...
                            if days_diff > max_days:
                                logger.warning(
                                    f"【{self.user_id}】日期验证失败，学习数据记录已超过最大使用天数 {max_days} 天（实际: {days_diff} 天）"
                                )
                                return False

                            logger.info(
                                f"【{self.user_id}】日期验证通过，剩余可用天数: {remaining_days} 天"
                            )
                            return True
                except (json.JSONDecodeError, ValueError, KeyError) as e:
                    logger.warning(
                        f"【{self.user_id}】读取历史记录文件失败: {e}，使用文件修改时间判断"
                    )
...
def _execute_slider_drag(page, slider_element, distance, user_id="unknown"):
    ...
    try:
        ...
        logger.debug(
            f"【{user_id}】page 类型: {type(page).__name__}, mouse_page 类型: {type(mouse_page).__name__}"
        )
        ...
        try:
            ...
            mouse_page.mouse.move(
                slider_x + offset_x, slider_y + offset_y, steps=random.randint(5, 10)
            )
            time.sleep(random.uniform(0.15, 0.3))

            # 再精确移动到滑块中心
            mouse_page.mouse.move(slider_x, slider_y, steps=random.randint(3, 6))
        ...
        try:
            ...
            logger.info(
                f"【{user_id}】生成优化轨迹: 距离={distance:.1f}px, 点数={len(optimized_trajectory)}"
            )
            ...
            for i, point in enumerate(optimized_trajectory):
                ...
                # 移动鼠标
                mouse_page.mouse.move(current_x, current_y, steps=random.randint(1, 3))
            ...
            logger.info(
                f"【{user_id}】滑动完成: 耗时={elapsed_time:.2f}秒, 最终位置=({current_x:.1f}, {current_y:.1f})"
            )
...
def patch_simulate_slide():
    ...
    try:
        ...
        def patched_simulate_slide(
            self, slider_button: ElementHandle, trajectory: Any
        ) -> Any:
            """
            优化的滑动模拟方法
            实现更人性化的滑动轨迹，提高成功率
            ...
            try:
                ...
                try:
                    ...
                    # 再精确移动到滑块中心
                    self.page.mouse.move(slider_x, slider_y, steps=random.randint(3, 6))
                    time.sleep(random.uniform(0.1, 0.25))
                except Exception as e:
                    logger.warning(f"【{user_id}】移动到滑块失败: {e}，继续尝试")
                ...
                try:
                    ...
                    if isinstance(trajectory, list) and len(trajectory) > 0:
                        ...
                        if isinstance(first_item, dict):
                            ...
                            try:
                                ...
                                logger.warning(
                                    f"【{user_id}】解析 tuple/list 格式轨迹失败: {e}"
                                )
                                total_distance = 0
                            ...
                            try:
                                ...
                                if isinstance(last_value, (int, float)):
                                    ...
                                    isinstance(last_value, (tuple, list))
                                    and len(last_value) > 0
                                ):
                                ...
                                logger.warning(
                                    f"【{user_id}】无法解析轨迹格式: {type(first_item)}, 错误: {e}"
                                )
                                total_distance = 0
                    else:
                        total_distance = 0
                    ...
                    if total_distance <= 0:
                        try:
                            ...
                            if hasattr(self, "calculate_slide_distance"):
                                ...
                                if slider_track:
                                    total_distance = self.calculate_slide_distance(
                            ...
                            if total_distance <= 0:
                                ...
                                if slider_track:
                                    ...
                                    if track_box and button_box:
                                        total_distance = (
                                            track_box["width"] - button_box["width"]
                                        )
                                        logger.debug(
                                            f"【{user_id}】从轨道计算距离: {total_distance}px"
                                        )
                        except Exception as e:
                    ...
                    if total_distance <= 0:
                        ...
                        logger.warning(
                            f"【{user_id}】使用默认滑动距离: {total_distance}px"
                        )
                    ...
                    logger.info(
                        f"【{user_id}】生成优化轨迹: 距离={total_distance:.1f}px, 点数={len(optimized_trajectory)}"
                    )
                    ...
                    for i, point in enumerate(optimized_trajectory):
                        ...
                        # 移动鼠标
                        self.page.mouse.move(
                            current_x, current_y, steps=random.randint(1, 3)
                        )
            ...
            try:
                ...
                for selector in selectors:
                    try:
                        ...
                        if element:
                            logger.info(
                                f"【{user_id}】✓ 在Frame {idx} 找到登录元素: {selector}"
                            )
                ...
                try:
                    for selector in slider_selectors:
                        try:
                            ...
                            if element and element.is_visible():
                                logger.info(
                                    f"【{user_id}】✅ 在Frame {idx} 检测到滑块验证: {selector}"
                                )
                                return True, frame
                        except:
                            continue
        ...
        try:
            ...
            for selector in qr_selectors:
                try:
                    ...
                    if element and element.is_visible():
                        logger.info(
                            f"【{user_id}】✅ 在Frame {idx} 检测到二维码验证: {selector}"
                        )
            ...
            try:
                ...
                for keyword in face_keywords:
                    if keyword in frame_content:
                        logger.info(
                            f"【{user_id}】✅ 在Frame {idx} 检测到人脸验证: {keyword}"
                        )
...
def _check_login_success_by_element(page_or_context, user_id: str) -> bool:
    ...
    try:
        ...
        if element:
            ...
            logger.info(
                f"【{user_id}】  - innerHTML内容 (前200字符): {inner_html[:200]}"
            )

            # 判断是否有数据：子元素数量大于0
            if child_count > 0:
                logger.success(
                    f"【{user_id}】✅ 登录成功！检测到列表有 {child_count} 个子元素"
                )
                logger.info(
                    f"【{user_id}】================================================"
                )
...
def _send_qr_verification_notification(cookie_id: str, qr_url: str):
    ...
    try:
        ...
        # 使用公共通知方法发送
        send_notification(
            cookie_id, notification_title, notification_message, "warning"
        )
...
def patch_login_with_password_headful():
    ...
    try:
        ...
        original_method = getattr(
            XianyuSliderStealth, "login_with_password_headful", None
        )

        def patched_login_with_password_headful(
            self, account: str, password: str, show_browser: bool
        ) -> dict:
            """
            重写的密码登录方法
            ...
            try:
                ...
                try:
                    ...
                    user_data_dir = os.path.join(
                        os.getcwd(), "browser_data", f"user_{user_id}"
                    )
                    ...
                    logger.info(
                        f"【{user_id}】浏览器已成功启动（{'有头' if show_browser else '无头'}模式）"
                    )
                    ...
                    try:
                        ...
                        login_frame = _find_frame_with_login(
                            page, login_selectors, user_id
                        )
                        if not login_frame:
                        ...
                        password_input = login_frame.query_selector(
                            "#fm-login-password"
                        )
                        if password_input:
                        ...
                        try:
                            ...
                            if agreement_checkbox:
                                is_checked = agreement_checkbox.evaluate(
                                    "el => el.checked"
                                )
                                if not is_checked:
                        ...
                        login_button = login_frame.query_selector(
                        ...
                        while monitor_elapsed < max_monitor_time and not login_verified:
                            ...
                            logger.debug(
                                f"【{user_id}】监控中... 已用时: {monitor_elapsed}秒"
                            )

                            # 0. 优先检测滑块验证（最重要，响应最快）
                            if not slider_handled:
                                has_slider, slider_frame = (
                                    _detect_slider_verification_in_page(page, user_id)
                                )

                                if has_slider and slider_frame:
                                    logger.warning(
                                        f"【{user_id}】🔍 检测到滑块验证！立即处理... (页面刷新次数: {page_refresh_count}/{max_page_refreshes})"
                                    )
                                    ...
                                    slider_success = _handle_slider_verification(
                                        slider_frame, user_id, max_attempts=5
                                    )
                                    ...
                                    if slider_success:
                                        ...
                                        if page_refresh_count < max_page_refreshes:
                                            ...
                                            logger.warning(
                                                f"【{user_id}】⚠️ 滑块验证失败，刷新页面重试 (第{page_refresh_count}/{max_page_refreshes}次刷新)"
                                            )
                                            ...
                                            try:
                                                ...
                                                logger.error(
                                                    f"【{user_id}】刷新页面失败: {refresh_err}"
                                                )
                                                break
                                        else:
                                            logger.error(
                                                f"【{user_id}】❌ 已达到最大刷新次数({max_page_refreshes})，停止尝试"
                                            )
                                            break

                            # 1. 检查账密错误
                            try:
                                has_error, error_message = _check_login_error(
                                    page, user_id
                                )
                                if has_error:
                                    logger.error(
                                        f"【{user_id}】❌❌❌ 登录失败：{error_message} ❌❌❌"
                                    )
                                    logger.error(
                                        f"【{user_id}】请检查账号和密码是否正确！"
                                    )
                                    ...
                                    try:
                                        ...
                                        logger.warning(
                                            f"【{user_id}】发送账密错误通知失败: {notify_err}"
                                        )
                        ...
                        if login_verified:
                            logger.success(
                                f"【{user_id}】✅ 登录流程完成，跳过额外等待"
                            )
                        else:
                            logger.warning(
                                f"【{user_id}】⚠️ 监控超时，检查是否需要额外验证"
                            )
                        ...
                        if not login_verified:
                            has_qr, qr_frame = _detect_qr_code_verification(
                                page, user_id
                            )
                        ...
                        if has_qr and not login_verified:
                            ...
                            while True:
                                ...
                                try:
                                    if _check_login_success_by_element(page, user_id):
                                        logger.success(
                                            f"【{user_id}】✅ 验证成功，登录状态已确认！"
                                        )
                                        break
                                    else:
                                        logger.info(
                                            f"【{user_id}】等待验证中... (每{check_interval}秒检查一次)"
                                        )
                                except Exception as e:
                                    logger.debug(
                                        f"【{user_id}】检查登录状态时出错: {e}"
                                    )
                        ...
                        try:
                            ...
                            logger.info(
                                f"【{user_id}】成功获取Cookie，包含 {len(cookies_dict)} 个字段"
                            )
                            ...
                            for key in important_keys:
                                if key in cookies_dict:
                                    ...
                                    logger.info(
                                        f"【{user_id}】  ✅ {key}: {'存在' if val else '为空'} (长度: {len(str(val)) if val else 0})"
                                    )
                                else:
                            ...
                            if cookies_dict:
                                ...
                                if _check_login_success_by_element(page, user_id):
                                    ...
                                    logger.info(
                                        f"获取到 {len(cookies_dict)} 个Cookie字段"
                                    )
                                    ...
                                    logger.warning(
                                        "可能原因：1) 登录未完成 2) 需要额外验证 3) 页面未加载完成"
                                    )
                                    return None
                            else:
                    ...
                    try:
                        if show_browser:
                            logger.info(
                                f"【{user_id}】有头模式：保持浏览器打开，等待手动关闭..."
                            )
                            logger.info(
                                f"【{user_id}】关闭浏览器后，缓存将自动保存到: {user_data_dir}"
                            )
                            # 不关闭浏览器，让用户手动关闭
                        else:
                            context.close()
        ...
        # 替换方法
        XianyuSliderStealth.login_with_password_headful = (
            patched_login_with_password_headful
        )

        logger.info(
            "✓ login_with_password_headful 方法已通过猴子补丁替换（使用Playwright）"
        )
        return True
