...
SLIDER_VERIFICATION = config.get(
    "SLIDER_VERIFICATION", {"max_concurrent": 3, "wait_timeout": 60}
)
API_ENDPOINTS = config.get("API_ENDPOINTS", {})
DEFAULT_HEADERS = config.get("DEFAULT_HEADERS", {})
WEBSOCKET_HEADERS = config.get("WEBSOCKET_HEADERS", {})
APP_CONFIG = config.get("APP_CONFIG", {})
AUTO_REPLY = config.get(
    "AUTO_REPLY",
    {
        "enabled": True,
...
YIFAN_API = config.get("YIFAN_API", {"callback_url": "", "query_url": ""})
_cookies_raw = config.get("COOKIES", [])
if isinstance(_cookies_raw, list):
    _cookies_list = _cookies_raw
else:
    # 兼容旧格式，仅有 value 字段
    val = _cookies_raw.get("value") if isinstance(_cookies_raw, dict) else None
    _cookies_list = [{"id": "default", "value": val}] if val else []

COOKIES_LIST = _cookies_list
