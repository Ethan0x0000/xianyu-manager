...
class AutoReplyPauseManager:
    ...
    def pause_chat(self, chat_id: str, cookie_id: str):
        ...
        if pause_minutes == 0:
            logger.info(
                f"【{cookie_id}】检测到手动发出消息，但暂停时间设置为0，不暂停自动回复"
            )
            return
        ...
        logger.info(
            f"【{cookie_id}】检测到手动发出消息，chat_id {chat_id} 自动回复暂停{pause_minutes}分钟，恢复时间: {end_time}"
        )
    ...
    def cleanup_expired_pauses(self):
        ...
        expired_chats = [
            chat_id
            for chat_id, pause_until in self.paused_chats.items()
            if current_time >= pause_until
        ]
...
def log_captcha_event(
    cookie_id: str, event_type: str, success: bool = None, details: str = ""
):
    """
    简单记录滑块验证事件到txt文件
...
class XianyuLive:
    ...
    def cache_auth_prewarmed_token(
        cls, cookie_id: str, token: str, source: str = "generic_auth"
    ):
    ...
    def cache_qr_prewarmed_token(cls, cookie_id: str, token: str):
        ...
        cls._qr_prewarmed_tokens[cookie_id] = {"token": token, "timestamp": time.time()}
    ...
    def mark_manual_refresh_handoff(
        cls, cookie_id: str, source: str = "manual_refresh_handoff", ttl: int = None
    ) -> Dict[str, Any]:
            ...
            state.update(
                {
                    ...
                    "slider_failed_bypass_used": state.get(
                        "slider_failed_bypass_used", False
                    ),
                    "previous_cookie_refresh_enabled": state.get(
                        "previous_cookie_refresh_enabled",
                        previous_cookie_refresh_enabled,
                    ),
                }
            )
    ...
    def acquire_auth_recovery_lock(
        cls, cookie_id: str, owner: str, ttl: int = None
        ...
        with cls._auth_recovery_lock:
            ...
            if (
                ...
                and now <= existing.get("expires_at", 0)
            ):
    ...
    def get_init_auth_failure_state(cls, cookie_id: str) -> Optional[Dict[str, Any]]:
        ...
        with cls._init_auth_failure_lock:
            ...
            if not state:
                return None
            if state.get("circuit_until") and time.time() > state.get(
                "circuit_until", 0
            ):
                state = {
                    "count": 0,
    ...
    def record_init_auth_failure(cls, cookie_id: str, reason: str) -> Dict[str, Any]:
        ...
        with cls._init_auth_failure_lock:
            ...
            if (
                ...
                or (now - window_started_at) > cls._init_auth_failure_window
            ):
                state["count"] = 0
    ...
    def set_password_login_failure_backoff(
        cls, cookie_id: str, reason: str, seconds: int
    ):
        """设置密码登录失败后的退避时间"""
    ...
    def classify_password_login_failure(error_message: str) -> Tuple[str, int]:
        ...
        if any(
            keyword in message
            for keyword in ["账号密码错误", "账密错误", "用户名或密码错误", "密码错误"]
        ):
            return "credentials", 1800
        if any(
            keyword in message
            for keyword in ["前置滑块", "风控", "拦截", "框体错误", "点击框体重试"]
        ):
        ...
        if any(
            keyword in message
            for keyword in ["网络", "timeout", "cannot connect", "连接", "dns", "ssl"]
        ):
            return "network", 180
        return "unknown", 300
    ...
    def _summarize_cookie_string(self, cookie_string: str) -> str:
        ...
        for part in cookie_string.split(";"):
            ...
            if "=" in part:
                ...
                segments.append(
                    f"{key.strip()}={self._mask_secret_value(value.strip(), head=4, tail=2)}"
                )
            else:
    ...
    def _normalize_risk_trigger_scene(
        self, trigger_reason: str = None, default: str = "unknown"
    ) -> str:
    ...
    def _sanitize_verification_meta(
        ...
        try:
            ...
            if x5secdata:
                meta["verification_token_hash"] = hashlib.sha256(
                    x5secdata.encode("utf-8")
                ).hexdigest()[:16]
    ...
    def _load_proxy_config(self) -> dict:
        ...
        try:
            ...
            logger.warning(
                f"【{self.cookie_id}】加载代理配置失败: {e}，使用默认配置（无代理）"
            )
            return {
                "proxy_type": "none",
                "proxy_host": "",
    ...
    def _get_proxy_url(self) -> str:
        ...
        if (
            ...
            or self.proxy_config.get("proxy_type", "none") == "none"
        ):
            return None
        ...
        if proxy_user and proxy_pass:
            # 带认证的代理
            proxy_url = (
                f"{proxy_type}://{proxy_user}:{proxy_pass}@{proxy_host}:{proxy_port}"
            )
        else:
            # 无认证的代理
    ...
    def _set_connection_state(self, new_state: ConnectionState, reason: str = ""):
        ...
        if self.connection_state != new_state:
            ...
            # 记录状态转换
            state_msg = (
                f"【{self.cookie_id}】连接状态: {old_state.value} → {new_state.value}"
            )
            if reason:
                state_msg += f" ({reason})"
    ...
    def _invalidate_heartbeat_generation(self, reason: str = "") -> int:
        """切换心跳世代，使旧心跳任务立即失效。"""
        self._heartbeat_generation = getattr(self, "_heartbeat_generation", 0) + 1
        if reason:
            logger.info(
                f"【{self.cookie_id}】心跳世代切换为 {self._heartbeat_generation} ({reason})"
            )
        return self._heartbeat_generation

    async def _start_heartbeat_task(self, ws, reason: str = ""):
        """安全启动新的心跳任务，并尽量等待旧任务退出。"""
        generation = self._invalidate_heartbeat_generation(reason or "启动心跳任务")

        old_task = self.heartbeat_task
        if old_task and not old_task.done():
            ...
            try:
                ...
                logger.warning(
                    f"【{self.cookie_id}】等待旧心跳任务退出时出错: {self._safe_str(e)}"
                )

        self.heartbeat_task = asyncio.create_task(
            self.heartbeat_loop(ws, heartbeat_generation=generation)
        )
    ...
    def _reset_background_tasks(self):
        ...
        """
        logger.info(
            f"【{self.cookie_id}】准备重置后台任务引用（仅重置依赖WebSocket的任务）..."
        )

        # 只处理心跳任务（依赖WebSocket，需要重启）
        if self.heartbeat_task:
            self._invalidate_heartbeat_generation("重置后台任务")
            status = "已完成" if self.heartbeat_task.done() else "运行中"
            logger.info(
                f"【{self.cookie_id}】发现心跳任务（状态: {status}），需要重置（因为依赖WebSocket连接）"
            )
            # 尝试取消心跳任务（但不等待）
            if not self.heartbeat_task.done():
                try:
                    ...
                    logger.debug(
                        f"【{self.cookie_id}】已发送取消信号给心跳任务（不等待响应）"
                    )
                except Exception as e:
        ...
        if other_tasks_status:
            logger.info(
                f"【{self.cookie_id}】其他任务继续运行（不依赖WebSocket）: {', '.join(other_tasks_status)}"
            )
        else:
    ...
    async def _cancel_background_tasks(self):
        ...
        try:
            ...
            if not tasks_to_cancel:
                logger.info(
                    f"【{self.cookie_id}】没有后台任务需要取消（所有任务已完成或不存在）"
                )
                # 立即重置任务引用
                self.heartbeat_task = None
            ...
            logger.info(
                f"【{self.cookie_id}】开始取消 {len(tasks_to_cancel)} 个未完成的后台任务..."
            )
            ...
            for task_name, task in tasks_to_cancel:
                try:
                    if task.done():
                        logger.info(
                            f"【{self.cookie_id}】任务已完成，跳过取消: {task_name}"
                        )
                    else:
                        task.cancel()
            ...
            try:
                ...
                if not pending_tasks_list:
                    ...
                    logger.info(
                        f"【{self.cookie_id}】等待 {len(pending_tasks_list)} 个未完成任务响应（超时时间: {wait_timeout}秒）..."
                    )
                    try:
                        # 使用 wait 等待任务完成，设置超时
                        ...
                        logger.info(
                            f"【{self.cookie_id}】asyncio.wait() 返回，耗时 {elapsed:.3f}秒，已完成: {len(done)}，未完成: {len(pending)}"
                        )
                        ...
                        for task_name, task in tasks_to_cancel:
                            if task in done:
                                try:
                                    task.result()
                                    logger.warning(
                                        f"【{self.cookie_id}】⚠️ 任务正常完成（非取消）: {task_name}"
                                    )
                                except asyncio.CancelledError:
                                    logger.info(
                                        f"【{self.cookie_id}】✅ 任务已成功取消: {task_name}"
                                    )
                                except Exception as e:
                                    logger.warning(
                                        f"【{self.cookie_id}】⚠️ 任务取消时出现异常 {task_name}: {e}"
                                    )
                                ...
                                if task in pending:
                                    ...
                                    if task.done():
                                        try:
                                            task.result()
                                            logger.warning(
                                                f"【{self.cookie_id}】任务在等待期间完成: {task_name}"
                                            )
                                        except asyncio.CancelledError:
                                            logger.info(
                                                f"【{self.cookie_id}】任务在等待期间被取消: {task_name}"
                                            )
                                        except Exception as e:
                                            logger.warning(
                                                f"【{self.cookie_id}】任务在等待期间异常 {task_name}: {e}"
                                            )
                                    else:
                                        logger.warning(
                                            f"【{self.cookie_id}】任务仍未完成: {task_name} (done={task.done()})"
                                        )

                            logger.warning(
                                f"【{self.cookie_id}】等待超时 ({elapsed:.3f}秒)，以下任务可能仍在运行: {', '.join(pending_names)}"
                            )
                            ...
                            for task_name, task in tasks_to_cancel:
                                if task in pending and not task.done():
                                    try:
                                        task.cancel()
                                        logger.warning(
                                            f"【{self.cookie_id}】强制取消任务: {task_name}"
                                        )
                                    except Exception as e:
                                        logger.warning(
                                            f"【{self.cookie_id}】强制取消任务失败 {task_name}: {e}"
                                        )
                                    ...
                                    for task_name, task in tasks_to_cancel:
                                        if task in done2:
                                            try:
                                                ...
                                                logger.info(
                                                    f"【{self.cookie_id}】任务在二次等待期间被取消: {task_name}"
                                                )
                                            except Exception as e:
                                                logger.warning(
                                                    f"【{self.cookie_id}】任务在二次等待期间异常 {task_name}: {e}"
                                                )
                                except Exception as e:
                                    logger.warning(
                                        f"【{self.cookie_id}】二次等待任务时出错: {e}"
                                    )

                            logger.warning(
                                f"【{self.cookie_id}】强制继续重连流程，未完成的任务将在后台继续运行（但已标记为取消）"
                            )
                        else:
                            logger.info(
                                f"【{self.cookie_id}】所有后台任务已取消 (耗时 {elapsed:.3f}秒)"
                            )

                    except Exception as e:
                        elapsed = time.time() - start_time
                        logger.warning(
                            f"【{self.cookie_id}】等待任务时出错 (耗时 {elapsed:.3f}秒): {e}"
                        )
                        import traceback

                        logger.warning(
                            f"【{self.cookie_id}】等待任务异常堆栈:\n{traceback.format_exc()}"
                        )

            except Exception as e:
                elapsed = time.time() - start_time
                logger.error(
                    f"【{self.cookie_id}】等待任务取消时出错 (耗时 {elapsed:.3f}秒): {e}"
                )
                import traceback

                logger.error(
                    f"【{self.cookie_id}】等待任务取消异常堆栈:\n{traceback.format_exc()}"
                )
            ...
            for task_name, task in tasks_to_cancel:
                if task and not task.done():
                    logger.warning(
                        f"【{self.cookie_id}】⚠️ 任务取消流程完成后，任务仍未完成: {task_name} (done={task.done()})"
                    )
    ...
    def _cleanup_instance_caches(self):
        ...
        try:
            ...
            expired_notifications = [
                key
                for key, last_time in self.last_notification_time.items()
            ...
            if expired_notifications:
                ...
                logger.warning(
                    f"【{self.cookie_id}】清理了 {len(expired_notifications)} 个过期通知记录"
                )
            ...
            expired_deliveries = [
                order_id
                for order_id, last_time in self.last_delivery_time.items()
            ...
            if expired_deliveries:
                ...
                logger.warning(
                    f"【{self.cookie_id}】清理了 {len(expired_deliveries)} 个过期发货记录"
                )
            ...
            expired_confirms = [
                order_id
                for order_id, last_time in self.confirmed_orders.items()
            ...
            if expired_confirms:
                ...
                logger.warning(
                    f"【{self.cookie_id}】清理了 {len(expired_confirms)} 个过期订单确认记录"
                )

            # 只有实际清理了内容才记录总数日志
            if cleaned_total > 0:
                logger.info(
                    f"【{self.cookie_id}】实例缓存清理完成，共清理 {cleaned_total} 条记录"
                )
                logger.warning(
                    f"【{self.cookie_id}】当前缓存数量 - 通知: {len(self.last_notification_time)}, 发货: {len(self.last_delivery_time)}, 确认: {len(self.confirmed_orders)}"
                )
    ...
    async def _cleanup_playwright_cache(self):
        ...
        try:
            ...
            if total_cleaned > 0:
                logger.info(
                    f"【{self.cookie_id}】Playwright缓存清理完成: 删除了 {total_cleaned} 个文件/目录，释放 {total_size_mb:.2f} MB"
                )
            else:
                logger.warning(
                    f"【{self.cookie_id}】Playwright缓存清理: 没有需要清理的临时文件"
                )

        except Exception as e:
            logger.warning(
                f"【{self.cookie_id}】清理Playwright缓存时出错: {self._safe_str(e)}"
            )
                ...
                for log_file in log_files:
                    try:
                        ...
                        if file_mtime < cutoff_time:
                            ...
                            total_cleaned += 1
                            logger.debug(
                                f"【{self.cookie_id}】删除过期日志文件: {log_file} (修改时间: {file_mtime})"
                            )
                    except Exception as e:
                        logger.warning(
                            f"【{self.cookie_id}】删除日志文件失败 {log_file}: {self._safe_str(e)}"
                        )

            if total_cleaned > 0:
                logger.info(
                    f"【{self.cookie_id}】日志清理完成: 删除了 {total_cleaned} 个日志文件，释放 {total_size_mb:.2f} MB (保留 {retention_days} 天内的日志)"
                )
            else:
                logger.debug(
                    f"【{self.cookie_id}】日志清理: 没有需要清理的过期日志文件 (保留 {retention_days} 天)"
                )
    ...
    def __init__(
        ...
        if not cookies_str:
            raise ValueError(
                "未提供cookies，请在global_config.yml中配置COOKIES_STR或通过参数传入"
            )

        logger.info(f"【{cookie_id}】解析cookies...")
        self.cookies = trans_cookies(cookies_str)
        logger.info(
            f"【{cookie_id}】cookies解析完成，包含字段: {list(self.cookies.keys())}"
        )
        ...
        if "unb" not in self.cookies:
            raise ValueError(
                f"【{cookie_id}】Cookie中缺少必需的'unb'字段，当前字段: {list(self.cookies.keys())}"
            )
        ...
        if prewarmed_token_info:
            ...
            self.last_token_refresh_time = prewarmed_token_info.get(
                "timestamp", time.time()
            )
            logger.info(
        ...
        if self.proxy_config.get("proxy_type", "none") != "none":
            logger.info(
                f"【{cookie_id}】已加载代理配置: {self.proxy_config['proxy_type']}://{self.proxy_config['proxy_host']}:{self.proxy_config['proxy_port']}"
            )
        ...
        # 浏览器Cookie刷新成功标志
        self.browser_cookie_refreshed = (
            False  # 标记_refresh_cookies_via_browser是否成功更新过数据库
        )
        self.restarted_in_browser_refresh = (
            False  # 刷新流程内部是否已触发重启（用于去重）
        )
        ...
        self._message_debounce_delay = (
            3  # 防抖延迟默认值（秒），实际值通过property从数据库动态读取
        )
        ...
        self.processed_message_ids_max_size = (
            10000  # 最大保存10000个消息ID，防止内存泄漏
        )
    ...
    def _init_order_status_handler(self):
        ...
        try:
            ...
            logger.error(
                f"【{self.cookie_id}】初始化订单状态处理器失败: {self._safe_str(e)}"
            )
    ...
    def is_manual_refresh_active(
        cls, cookie_id: str, allow_handoff_recovery: bool = False
    ) -> bool:
        """检查指定账号是否处于手动刷新保护期。"""
        if not cookie_id:
            return False
    ...
    def begin_manual_refresh(
        cls, cookie_id: str, source: str = "manual_refresh"
    ) -> Dict[str, Any]:
        ...
        with cls._manual_refresh_lock:
            ...
            if existing:
                ...
                return {
                    "started": False,
                    "already_active": True,
                    "previous_cookie_refresh_enabled": existing.get(
                        "previous_cookie_refresh_enabled"
                    ),
                }
        ...
        if live_instance is not None and previous_cookie_refresh_enabled is not None:
            ...
            logger.warning(
                f"【{cookie_id}】已进入手动刷新保护期，当前无运行中的账号实例"
            )
    ...
    def end_manual_refresh(cls, cookie_id: str, source: str = "manual_refresh") -> bool:
        ...
        if live_instance is not None and previous_cookie_refresh_enabled is not None:
            ...
            logger.warning(
                f"【{cookie_id}】手动刷新保护期已结束，当前无运行中的账号实例可恢复"
            )
        ...
        *,
        ...
        if source != "senderNick":
            ...
            if any(keyword in text for keyword in invalid_keywords):
                logger.info(
                    f"{log_prefix} 👤 忽略系统关键词型买家昵称({source}): {text}"
                )
                return None
    ...
    def _resolve_delivery_log_buyer_nick(
        self,
        buyer_nick: Any = None,
        *,
        order_id: str = None,
        ...
        try:
            if normalized_order_id:
                ...
                if order_info:
                    ...
                    if (
                        not order_cookie_id
                        or order_cookie_id == str(self.cookie_id).strip()
                    ):
                    ...
                    if not normalized_buyer_id:
                        normalized_buyer_id = (
                            str(order_info.get("buyer_id") or "").strip() or None
                        )
    ...
    def _lookup_delivery_order_by_sid(
        self, sid: str, *, minutes: int = 10, log_prefix: str = ""
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(
                f"{log_prefix} sid兜底查单异常: {self._safe_str(lookup_error)}"
            )
    ...
    async def _refresh_sid_lookup_if_needed(
        self,
        sid: str,
        sid_lookup: Dict[str, Any],
        *,
        item_id: str = None,
        buyer_id: str = None,
        minutes: int = 10,
        allow_bargain_ready: bool = False,
        log_prefix: str = "",
    ) -> Dict[str, Any]:
        ...
        if not recent_order or match_type not in {
            "not_ready",
            "other_status",
            "suspicious_shipped",
        }:
        ...
        try:
            await self.fetch_order_detail_info(
                order_id, refresh_item_id, refresh_buyer_id, sid=sid, force_refresh=True
            )
        except Exception as refresh_error:
            logger.warning(
                f"{log_prefix} sid未就绪订单强刷失败: {self._safe_str(refresh_error)}"
            )
            return sid_lookup

        refreshed_lookup = self._lookup_delivery_order_by_sid(
            sid, minutes=minutes, log_prefix=log_prefix
        )
        refreshed_order = refreshed_lookup.get("order") or {}

        if (
            allow_bargain_ready
            and refreshed_lookup.get("match_type") == "not_ready"
            and refreshed_order
            and str(refreshed_order.get("order_status") or "").strip()
            in {"processing", "pending_payment"}
            and self._has_bargain_success_evidence(refreshed_order)
        ):
    ...
    async def _ensure_item_owned_by_current_account(
        self,
        item_id: str,
        *,
        log_prefix: str = "",
        ...
        logger.info(
            f"{log_prefix} 商品 {item_id} 未命中本地缓存，刷新在售商品列表后重试归属校验"
        )
        try:
            for page_number in range(1, max_pages + 1):
                ...
                if not result.get("success"):
                    logger.warning(
                        f"{log_prefix} 刷新在售商品列表失败，停止归属校验回退: page={page_number}, result={result}"
                    )
                    break

                current_items = result.get("items", [])
                if any(
                    str(item.get("id", "")).strip() == str(item_id).strip()
                    for item in current_items
                ):
                    logger.info(
                        f"{log_prefix} 商品 {item_id} 在第 {page_number} 页在售商品列表中命中，归属校验通过"
                    )
                    return True
            ...
            logger.error(
                f"{log_prefix} 刷新在售商品列表进行归属校验失败: {self._safe_str(e)}"
            )
    ...
    def _extract_query_value_from_url(self, url_text: Any, key: str) -> Optional[str]:
        ...
        try:
            ...
            logger.debug(
                f"【{self.cookie_id}】解析链接参数失败: key={key}, error={self._safe_str(e)}"
            )
            return None

    def _extract_buyer_id_from_message_meta(
        self, message_meta: dict, *, meta_label: str, log_prefix: str = ""
        ...
        candidates = [
            (
                "reminderUrl.peerUserId",
                self._extract_query_value_from_url(
                    message_meta.get("reminderUrl"), "peerUserId"
                ),
            ),
            (
                "bizTag.senderId",
                self._normalize_buyer_id_value(
                    biz_tag_dict.get("senderId") or biz_tag_dict.get("sender_id")
                ),
            ),
            (
                f"{meta_label}.senderUserId",
                self._normalize_buyer_id_value(message_meta.get("senderUserId")),
            ),
        ]
    ...
    def _select_buyer_identity_for_order_write(
        self,
        order_id: str,
        *,
        incoming_buyer_id: Any = None,
        incoming_buyer_nick: Any = None,
        existing_order: Dict[str, Any] = None,
        buyer_id_source: str = None,
        buyer_nick_source: str = "unknown",
        log_prefix: str = "",
        ...
        existing_buyer_id = self._normalize_buyer_id_value(
            (existing_order or {}).get("buyer_id")
        )
        ...
        if existing_buyer_is_trustworthy:
            if not incoming_buyer_id:
                return (
                    existing_buyer_id,
                    incoming_buyer_nick or existing_buyer_nick,
                    False,
                )
    ...
    def _extract_order_message_context(
        ...
        item_id = None
        log_prefix = (
            f"【{self.cookie_id}】[{msg_id}]" if msg_id else f"【{self.cookie_id}】"
        )
        ...
        try:
            ...
            if isinstance(message_1, str):
                ...
                if isinstance(message_4, dict):
                    buyer_id, buyer_id_source = (
                        self._extract_buyer_id_from_message_meta(
                            message_4,
                    ...
                    if not buyer_nick:
                        ...
                        if buyer_nick:
                            logger.info(
                                f"{log_prefix} 👤 从message[4].reminderTitle提取到买家昵称: {buyer_nick}"
                            )
                    if buyer_nick:
                        logger.info(
                            f"{log_prefix} 👤 从message[4]提取到买家昵称: {buyer_nick}"
                        )
                logger.info(
                ...
                if "10" in message_1 and isinstance(message_1["10"], dict):
                    ...
                    if not buyer_nick:
                        ...
                        if buyer_nick:
                            logger.info(
                                f"{log_prefix} 👤 从reminderTitle提取到买家昵称: {buyer_nick}"
                            )
                    if buyer_nick:
            ...
            logger.warning(
                f"{log_prefix} 提取订单上下文失败: {self._safe_str(context_e)}"
            )
    ...
    def _preload_basic_order_info(
        ...
        try:
            ...
            buyer_id_to_save, buyer_nick_to_save, should_skip_write = (
                self._select_buyer_identity_for_order_write(
                    order_id,
    ...
    async def _retry_order_detail_after_delay(
        self,
        order_id: str,
        item_id: str = None,
        buyer_id: str = None,
        ...
        try:
            ...
            logger.info(
                f"【{self.cookie_id}】开始延迟补抓订单详情: order_id={order_id}, delay={delay_seconds}s"
            )
            ...
            if result:
                ...
                logger.warning(
                    f"【{self.cookie_id}】订单详情延迟补抓仍失败，保留基础订单: {order_id}"
                )
        except asyncio.CancelledError:
            ...
            logger.error(
                f"【{self.cookie_id}】订单详情延迟补抓异常: {order_id} - {self._safe_str(e)}"
            )
        finally:
    ...
    def _schedule_order_detail_retry(
        self,
        order_id: str,
        item_id: str = None,
        buyer_id: str = None,
        ...
        if existing_task and not existing_task.done():
            logger.info(
                f"【{self.cookie_id}】订单详情补抓任务已存在，跳过重复调度: {order_id}"
            )
            return
        ...
        logger.info(
            f"【{self.cookie_id}】已调度订单详情补抓任务: order_id={order_id}, delay={delay_seconds}s"
        )
    ...
    async def _enqueue_message(
        self, message_data: dict, websocket, msg_id: str = "unknown"
    ) -> bool:
        """
        将消息放入优先级队列
        ...
        try:
            ...
            try:
                ...
                if priority <= 1:
                    logger.info(
                        f"【{self.cookie_id}】📥 高优先级消息入队 [P{priority}][ID:{msg_id}] 队列大小: {self.message_queue.qsize()}"
                    )
                else:
                    logger.debug(
                        f"【{self.cookie_id}】📥 消息入队 [P{priority}][ID:{msg_id}] 队列大小: {self.message_queue.qsize()}"
                    )
                ...
                logger.warning(
                    f"【{self.cookie_id}】⚠️ 消息队列已满({self.message_queue_max_size})，消息[ID:{msg_id}]被丢弃"
                )
                return False
        ...
        while self.message_queue_running:
            try:
                ...
                if age > self.message_expire_seconds:
                    ...
                    logger.warning(
                        f"【{self.cookie_id}】⏰ 工作协程#{worker_id} 丢弃过期消息 [ID:{message_item['msg_id']}] 已等待{age:.1f}秒"
                    )
                ...
                try:
                    logger.debug(
                        f"【{self.cookie_id}】🔄 工作协程#{worker_id} 开始处理消息 [P{priority}][ID:{msg_id}] 等待{age:.2f}秒"
                    )
                    ...
                    async with self.message_semaphore:
                        ...
                        try:
                            await self.handle_message(
                                message_item["data"], message_item["websocket"], msg_id
                            )
                    ...
                    logger.debug(
                        f"【{self.cookie_id}】✅ 工作协程#{worker_id} 完成消息处理 [ID:{msg_id}]"
                    )

                except Exception as e:
                    self.queue_stats["errors"] += 1
                    logger.error(
                        f"【{self.cookie_id}】❌ 工作协程#{worker_id} 处理消息失败 [ID:{msg_id}]: {self._safe_str(e)}"
                    )
                finally:
                    self.message_queue.task_done()
                ...
                logger.info(
                    f"【{self.cookie_id}】🛑 消息处理工作协程 #{worker_id} 被取消"
                )
                break
            except Exception as e:
                logger.error(
                    f"【{self.cookie_id}】工作协程#{worker_id} 异常: {self._safe_str(e)}"
                )
    ...
    async def _start_message_queue_workers(self):
        ...
        logger.info(
            f"【{self.cookie_id}】🚀 消息队列系统已启动，{self.message_queue_workers}个工作协程"
        )
        ...
        while self.message_queue_running:
            try:
                ...
                if stats["received"] > 0:
                    ...
                    drop_rate = (
                        (stats["dropped_full"] + stats["dropped_expired"])
                        / stats["received"]
                        * 100
                    )
                    ...
                    if drop_rate > 10:
                        logger.warning(
                            f"【{self.cookie_id}】⚠️ 消息丢弃率过高({drop_rate:.1f}%)，建议增加工作协程数量或检查消息处理效率"
                        )
    ...
    def is_auto_confirm_enabled(self) -> bool:
        ...
        try:
            ...
            logger.error(
                f"【{self.cookie_id}】获取自动确认发货设置失败: {self._safe_str(e)}"
            )
            return True  # 出错时默认启用
    ...
    def is_auto_comment_enabled(self) -> bool:
        ...
        try:
            ...
            logger.error(
                f"【{self.cookie_id}】获取自动好评设置失败: {self._safe_str(e)}"
            )
    ...
    async def handle_auto_comment(self, message: dict, msg_time: str, msg_id: str = ""):
        ...
        try:
            ...
            if not self.is_auto_comment_enabled():
                logger.info(
                    f"[{msg_time}] 【{self.cookie_id}】[{msg_id}] 未启用自动好评，跳过"
                )
                return False
            ...
            if not order_id:
                logger.warning(
                    f"[{msg_time}] 【{self.cookie_id}】[{msg_id}] 无法从评价消息中提取订单ID，跳过自动好评"
                )
                return False

            logger.info(
                f"[{msg_time}] 【{self.cookie_id}】[{msg_id}] 检测到评价提醒，订单ID: {order_id}"
            )
            ...
            if not template:
                logger.warning(
                    f"[{msg_time}] 【{self.cookie_id}】[{msg_id}] 未设置激活的好评模板，跳过自动好评"
                )
                return False
            ...
            if not comment_content:
                logger.warning(
                    f"[{msg_time}] 【{self.cookie_id}】[{msg_id}] 好评模板内容为空，跳过自动好评"
                )
                return False
            ...
            if result.get("success"):
                logger.info(
                    f"[{msg_time}] 【{self.cookie_id}】[{msg_id}] ✅ 订单 {order_id} 自动好评成功"
                )
                return True
            else:
                logger.warning(
                    f"[{msg_time}] 【{self.cookie_id}】[{msg_id}] ❌ 订单 {order_id} 自动好评失败: {result.get('message', '未知错误')}"
                )
                return False

        except Exception as e:
            logger.error(
                f"[{msg_time}] 【{self.cookie_id}】[{msg_id}] 自动好评异常: {self._safe_str(e)}"
            )
            return False
    ...
    def mark_delivery_sent(self, order_id: str, context: str = "自动发货完成"):
        ...
        # 更新订单状态为已发货
        logger.info(
            f"【{self.cookie_id}】检查自动发货订单状态处理器: handler_exists={self.order_status_handler is not None}"
        )
        if self.order_status_handler:
            logger.info(
                f"【{self.cookie_id}】准备调用订单状态处理器.handle_auto_delivery_order_status: {order_id}"
            )
            try:
                success = self.order_status_handler.handle_auto_delivery_order_status(
                    order_id=order_id, cookie_id=self.cookie_id, context=context
                )
                logger.info(
                    f"【{self.cookie_id}】订单状态处理器.handle_auto_delivery_order_status返回结果: {success}"
                )
                if success:
                    logger.info(
                        f"【{self.cookie_id}】订单 {order_id} 状态已更新为已发货"
                    )
                else:
                    logger.warning(
                        f"【{self.cookie_id}】订单 {order_id} 状态更新为已发货失败"
                    )
            except Exception as e:
                logger.error(
                    f"【{self.cookie_id}】订单状态更新失败: {self._safe_str(e)}"
                )
                import traceback

                logger.error(
                    f"【{self.cookie_id}】详细错误信息: {traceback.format_exc()}"
                )
        else:
            logger.warning(
                f"【{self.cookie_id}】订单状态处理器为None，跳过自动发货状态更新: {order_id}"
            )
    ...
    def _record_delivery_log(
        ...
        try:
            ...
            logger.error(
                f"【{self.cookie_id}】记录发货日志失败: {self._safe_str(log_e)}"
            )

    def _format_delivery_log_reason(
        self, reason: str = None, rule_meta: dict = None
    ) -> str:
    ...
    async def _finalize_delivery_after_send(
        self,
        delivery_meta: dict = None,
        order_id: str = None,
        item_id: str = None,
        ...
        if not meta.get("success"):
            return {"success": False, "error": "发货元数据无效，无法提交副作用"}
        ...
        if consume_required:
            if reservation_id:
                finalize_state = db_manager.finalize_batch_data_reservation(
                    reservation_id
                )
                ...
                reservation_already_finalized = bool(
                    finalize_state.get("already_finalized")
                )
                ...
                consumed = db_manager.consume_specific_batch_data(
                    card_id, expected_line
                )
                if not consumed:
                    return {
                        "success": False,
            ...
            if not self.is_auto_confirm_enabled():
                ...
                if order_id in self.confirmed_orders:
                    ...
                    if current_time - last_confirm_time < self.order_confirm_cooldown:
                        logger.info(
                            f"订单 {order_id} 已在 {self.order_confirm_cooldown} 秒内确认过，跳过重复确认"
                        )
                        should_confirm = False

                if should_confirm:
                    logger.info(
                        f"开始自动确认发货: 订单ID={order_id}, 商品ID={item_id}"
                    )
    ...
    def _release_data_reservation_if_needed(
        self, delivery_meta: dict = None, error: str = None
    ) -> bool:
        meta = delivery_meta or {}
        reservation_id = meta.get("data_reservation_id")
        if not reservation_id:
    ...
    def _get_pending_delivery_finalization_meta(
        self, order_id: str, delivery_unit_index: int = 1
    ):
        if not order_id:
            return None
    ...
    def _persist_delivery_finalization_state(
        self,
        order_id: str,
        item_id: str,
        buyer_id: str,
    ...
    def _summarize_delivery_progress(self, order_id: str, expected_quantity: int = 1):
        if not order_id:
            return {
                ...
                "remaining_unit_indexes": list(
                    range(1, max(1, int(expected_quantity or 1)) + 1)
                ),
                "states": [],
            }
        ...
        return db_manager.get_delivery_progress_summary(
            order_id, expected_quantity=expected_quantity
        )

    def _resolve_external_order_status(
        self, current_status: str, incoming_status: str, source: str
    ):
        from db_manager import db_manager

        merged_status = db_manager.resolve_external_order_status(
            current_status, incoming_status, source=source
        )
    ...
    def _mark_order_bargain_flow(
        ...
        sid: str = None,
        *,
        apply_configured_price: bool = False,
        success_detected=...,
        context: str = "",
    ) -> bool:
        if not order_id:
        ...
        if apply_configured_price and effective_item_id:
            ...
            configured_amount = self._normalize_order_amount_text(
                item_config.get("item_price") if item_config else None
            )
            configured_amount_value = self._parse_order_amount_float(configured_amount)
            existing_amount_value = self._parse_order_amount_float(
                existing_order.get("amount")
            )
            if configured_amount_value is not None and (
                existing_amount_value is None
                or configured_amount_value < existing_amount_value - 0.009
            ):
    ...
    def _apply_bargain_amount_override(
        self,
        order_id: str,
        item_id: str,
        amount: Any,
    ...
    def _resolve_delivery_progress_order_status(
        self, current_status: str, aggregate_status: str
    ):
        from db_manager import db_manager
        ...
        if normalized_current == "shipped" and normalized_aggregate in {
            "pending_ship",
            "partial_success",
            "partial_pending_finalize",
        }:
        ...
        if (
            normalized_current in {"partial_success", "partial_pending_finalize"}
            and normalized_aggregate == "pending_ship"
        ):
            logger.warning(
    ...
    def _sync_order_delivery_progress(
        self,
        order_id: str,
        cookie_id: str,
        ...
        summary = self._summarize_delivery_progress(
            order_id, expected_quantity=expected_quantity
        )
        ...
        try:
            ...
            previous_status = (
                db_manager._normalize_order_status(current_order.get("order_status"))
                if current_order
                else None
            )
        except Exception as e:
            logger.warning(
                f"【{self.cookie_id}】读取订单旧状态失败: {self._safe_str(e)}"
            )
        ...
        status_to_write = self._resolve_delivery_progress_order_status(
            previous_status, aggregate_status
        )
        ...
        if (
            ...
            and status_to_write == "shipped"
            and previous_status != "shipped"
        ):
            try:
                self.order_status_handler.handle_auto_delivery_order_status(
                    order_id=order_id, cookie_id=cookie_id, context=context
                )
            except Exception as e:
                logger.warning(
                    f"【{self.cookie_id}】通过状态处理器同步已发货状态失败: {self._safe_str(e)}"
                )
        ...
        try:
            ...
            if status_to_write and status_to_write != previous_status:
                success = db_manager.insert_or_update_order(
                    order_id=order_id, order_status=status_to_write, cookie_id=cookie_id
                )

            if (
                success
                and status_to_write in {"partial_success", "partial_pending_finalize"}
                and previous_status != status_to_write
            ):
                try:
                    from order_event_hub import publish_order_update_event

                    publish_order_update_event(
                        order_id, source="delivery_progress_sync"
                    )
            ...
            logger.warning(
                f"【{self.cookie_id}】写入订单聚合发货状态失败: {self._safe_str(e)}"
            )
    ...
    async def _delayed_lock_release(self, lock_key: str, delay_minutes: int = 10):
        ...
        try:
            ...
            logger.info(
                f"【{self.cookie_id}】订单锁 {lock_key} 将在 {delay_minutes} 分钟后释放"
            )
            ...
            logger.error(
                f"【{self.cookie_id}】订单锁 {lock_key} 延迟释放失败: {self._safe_str(e)}"
            )
    ...
    def cleanup_expired_locks(self, max_age_hours: int = 24):
        ...
        try:
            ...
            total_expired = (
                len(expired_delivery_locks)
                + len(expired_detail_locks)
                + len(expired_refresh_marks)
            )
            if total_expired > 0:
                logger.info(
                ...
                logger.warning(
                    f"【{self.cookie_id}】当前锁数量 - 发货锁: {len(self._order_locks)}, 详情锁: {len(self._order_detail_locks)}"
                )

        except Exception as e:
            logger.error(
                f"【{self.cookie_id}】清理过期锁时发生错误: {self._safe_str(e)}"
            )
    ...
    def _has_delivery_progress_evidence(self, order_id: str) -> bool:
        ...
        try:
            summary = (
                self._summarize_delivery_progress(
                    normalized_order_id, expected_quantity=1
                )
                or {}
            )
    ...
    def _reserve_order_detail_force_refresh(
        self,
        order_id: str,
        *,
        reason: str,
        ...
        cooldown = float(
            cooldown_seconds or self.order_detail_force_refresh_cooldown or 0
        )
        now = time.time()
    ...
    def _should_force_refresh_after_status_signal(
        self, status_signal: str, current_status: str, order_id: str = None
    ) -> bool:
        ...
        if normalized_signal == "pending_ship":
            if (
                normalized_current == "shipped"
                and not self._has_delivery_progress_evidence(order_id)
            ):
                logger.warning(
        ...
        if normalized_signal in {
            ...
            if not normalized_current or normalized_current == "unknown":
                return True
            return self._get_order_status_priority(
                normalized_signal
            ) > self._get_order_status_priority(normalized_current)

        return False

    def _should_accept_order_detail_status_correction(
        self,
        current_status: str,
        incoming_status: str,
        incoming_source: str,
    ...
    def _should_reject_order_detail_status_update(
        self,
        current_status: str,
        incoming_status: str,
        incoming_source: str,
        *,
        ...
        if force_refresh and normalized_current in {
            "shipped",
            "pending_ship",
            "partial_success",
            "partial_pending_finalize",
    ...
    async def _maybe_force_refresh_order_detail_for_signal(
        self,
        order_id: str,
        *,
        item_id: str = None,
        ...
        if not self._should_force_refresh_after_status_signal(
            status_signal, current_status, normalized_order_id
        ):
            logger.info(
        ...
        if not self._should_force_refresh_after_status_signal(
            status_signal, latest_status, normalized_order_id
        ):
            logger.info(
    ...
    def _classify_message_route(
        self, *, message: dict, message_1: dict, message_10: dict, send_message: str
    ) -> Dict[str, Any]:
        ...
        reminder_content = (
            self._extract_chat_message_text(message_10)
            or str(send_message or "").strip()
        )
        ...
        if isinstance(message, dict) and isinstance(message.get("3"), dict):
            red_reminder = str(
                message.get("3", {}).get("redReminder", "") or ""
            ).strip()
        ...
        is_system_biz = (
            bool(task_name) or "SECURITY" in biz_tag_raw or "taskId" in biz_tag_raw
        )
        ...
        if send_message in special_flow_messages or card_title in special_flow_titles:
            ...
            def _contains_any(markers) -> bool:
                return any(
                    marker and marker in text for text in texts for marker in markers
                )
            ...
            if is_system_message and order_status_signal:
                route = "order_status"
            elif _contains_any(system_notice_markers) and (
                is_system_message or message_direction != 2
            ):
                route = "system_notice"
        ...
        if not is_group_message:
            if route == "user_chat":
                should_notify = True
            elif route == "order_status" and order_status_signal in {
                "pending_ship",
                "refunding",
                "cancelled",
            }:
    ...
    def _extract_order_id(self, message: dict, raw_message_data: dict = None) -> str:
        ...
        try:
            ...
            for source, candidate_text in self._collect_order_id_candidate_texts(
                ...
                if order_id:
                    logger.info(
                        f"【{self.cookie_id}】🎯 最终提取到订单ID: {order_id} (source={source})"
                    )
                    return order_id
            ...
            if raw_message_data:
                ...
                for source, candidate_text in self._collect_order_id_candidate_texts(
                    raw_message_data, root="raw_message"
                ):
                    order_id = self._extract_order_id_from_candidate_text(
                        candidate_text, source=source
                    )
                    if order_id:
                        logger.info(
                            f"【{self.cookie_id}】🎯 从原始消息提取到订单ID: {order_id} (source={source})"
                        )
                        return order_id

                try:
                    sync_data_list = (
                        raw_message_data.get("body", {})
                        .get("syncPushPackage", {})
                        .get("data", [])
                    )
                    for idx, sync_data_item in enumerate(sync_data_list[:20]):
                        if (
                            not isinstance(sync_data_item, dict)
                            or "data" not in sync_data_item
                        ):
                            continue
                        ...
                        for (
                            source,
                            candidate_text,
                        ) in self._collect_order_id_candidate_texts(
                            decoded_data, root=f"raw_sync[{idx}]"
                        ):
                            order_id = self._extract_order_id_from_candidate_text(
                                candidate_text, source=source
                            )
                            if order_id:
                                logger.info(
                                    f"【{self.cookie_id}】🎯 从syncPushPackage.data提取到订单ID: {order_id} (source={source})"
                                )
                                return order_id
    ...
    async def _handle_simple_message_auto_delivery(
        self,
        websocket,
        order_id: str,
        item_id: str,
        ...
        try:
            logger.info(
                f"[{msg_time}] 【{self.cookie_id}】[{msg_id}] 🚀 开始处理简化消息自动发货: order_id={order_id}, item_id={item_id}"
            )
            ...
            if item_id and item_id != "未知商品":
                try:
                    if not await self._ensure_item_owned_by_current_account(
                        ...
                        logger.warning(
                            f"[{msg_time}] 【{self.cookie_id}】[{msg_id}] ❌ 商品 {item_id} 不属于当前账号，跳过自动发货"
                        )
                        ...
                        return
                    logger.info(
                        f"[{msg_time}] 【{self.cookie_id}】[{msg_id}] ✅ 商品 {item_id} 归属验证通过"
                    )
                except Exception as e:
                    logger.error(
                        f"[{msg_time}] 【{self.cookie_id}】[{msg_id}] 检查商品归属失败: {self._safe_str(e)}，跳过自动发货"
                    )
                    self._record_delivery_log(
                        order_id=order_id,
                        item_id=item_id,
            ...
            if not self.can_auto_delivery(order_id):
                logger.info(
                    f"[{msg_time}] 【{self.cookie_id}】[{msg_id}] 订单 {order_id} 在冷却期内，跳过发货"
                )
                self._record_delivery_log(
                    order_id=order_id,
                    item_id=item_id,
            ...
            if self.is_lock_held(lock_key):
                logger.info(
                    f"[{msg_time}] 【{self.cookie_id}】[{msg_id}] 🔒 订单 {lock_key} 延迟锁仍在持有状态，跳过发货"
                )
                self._record_delivery_log(
                    order_id=order_id,
                    item_id=item_id,
            ...
            async with order_lock:
                logger.info(
                    f"[{msg_time}] 【{self.cookie_id}】[{msg_id}] 获取订单锁成功: {lock_key}"
                )

                # 再次检查延迟锁和冷却状态
                if self.is_lock_held(lock_key) or not self.can_auto_delivery(order_id):
                    logger.info(
                        f"[{msg_time}] 【{self.cookie_id}】[{msg_id}] 获取锁后检查发现订单已处理，跳过发货"
                    )
                    self._record_delivery_log(
                        order_id=order_id,
                        item_id=item_id,
                ...
                logger.info(
                    f"[{msg_time}] 【{self.cookie_id}】[{msg_id}] 📤 开始执行自动发货内容发送（发送成功后再确认发货）"
                )

                # 获取商品标题
                item_title = "待获取商品信息"

                pending_finalize_meta = self._get_pending_delivery_finalization_meta(
                    order_id, 1
                )
                if pending_finalize_meta:
                    ...
                    if not finalize_result.get("success"):
                        self._persist_delivery_finalization_state(
                            ...
                            channel="auto",
                            status="sent",
                            last_error=finalize_result.get("error")
                            or "补完成 finalize 失败",
                        )
                        self._record_delivery_log(
                            order_id=order_id,
                            ...
                            buyer_id=user_id,
                            status="failed",
                            reason=finalize_result.get("error")
                            or "检测到已发送记录，但补完成发货收尾失败",
                            channel="auto",
                        ...
                        await self.send_delivery_failure_notification(
                            ...
                            item_id=item_id,
                            error_message=finalize_result.get("error")
                            or "检测到已发送记录，但补完成发货收尾失败",
                            chat_id=chat_id,
                        )
                        return
                    ...
                    logger.info(
                        f"[{msg_time}] 【{self.cookie_id}】[{msg_id}] ✅ 简化消息自动发货补完成收尾成功"
                    )
                    return
                ...
                if isinstance(delivery_result, dict):
                    ...
                    delivery_rule_meta = {
                        ...
                        "data_card_pending_consume": delivery_result.get(
                            "data_card_pending_consume"
                        ),
                        "data_line": delivery_result.get("data_line"),
                        "data_reservation_id": delivery_result.get(
                            "data_reservation_id"
                        ),
                        "data_reservation_status": delivery_result.get(
                            "data_reservation_status"
                        ),
                        "delivery_unit_index": delivery_result.get(
                            "delivery_unit_index"
                        ),
                    }
                else:
                ...
                if delivery_content:
                    ...
                    try:
                        ...
                        if not self._mark_data_reservation_sent_if_needed(
                            delivery_result
                            if isinstance(delivery_result, dict)
                            else delivery_rule_meta
                        ):
                    ...
                    logger.info(
                        f"[{msg_time}] 【{self.cookie_id}】[{msg_id}] ✅ 简化消息自动发货完成"
                    )
                else:
                    logger.warning(
                        f"[{msg_time}] 【{self.cookie_id}】[{msg_id}] ❌ 未找到匹配的发货规则或获取发货内容失败"
                    )
                    self._record_delivery_log(
                        order_id=order_id,
                        item_id=item_id,
            ...
            self._release_data_reservation_if_needed(
                delivery_result
                if "delivery_result" in locals() and isinstance(delivery_result, dict)
                else delivery_rule_meta
                if "delivery_rule_meta" in locals()
                else None,
            ...
            logger.error(
                f"[{msg_time}] 【{self.cookie_id}】[{msg_id}] 简化消息自动发货异常: {self._safe_str(e)}"
            )
            import traceback

            logger.error(
                f"[{msg_time}] 【{self.cookie_id}】[{msg_id}] 异常堆栈: {traceback.format_exc()}"
            )

    async def _handle_auto_delivery(
        self,
        websocket,
        message: dict,
        send_user_name: str,
        ...
        try:
            ...
            if item_id and item_id != "未知商品":
                try:
                    if not await self._ensure_item_owned_by_current_account(
                        item_id, log_prefix=f"[{msg_time}] 【{self.cookie_id}】"
                    ):
                        logger.warning(
                            f"[{msg_time}] 【{self.cookie_id}】❌ 商品 {item_id} 不属于当前账号，跳过自动发货"
                        )
                        self._record_delivery_log(
                            item_id=item_id,
                        ...
                        return
                    logger.warning(
                        f"[{msg_time}] 【{self.cookie_id}】✅ 商品 {item_id} 归属验证通过"
                    )
                except Exception as e:
                    logger.error(
                        f"[{msg_time}] 【{self.cookie_id}】检查商品归属失败: {self._safe_str(e)}，跳过自动发货"
                    )
                    self._record_delivery_log(
                        item_id=item_id,
                        buyer_id=send_user_id,
            ...
            if not order_id:
                ...
                try:
                    message_1 = (
                        message.get("1", {}) if isinstance(message, dict) else {}
                    )
                    ...
                    logger.warning(
                        f"[{msg_time}] 【{self.cookie_id}】解析sid失败: {self._safe_str(sid_e)}"
                    )
                ...
                if fallback_sid:
                    try:
                        ...
                        logger.error(
                            f"[{msg_time}] 【{self.cookie_id}】sid兜底查单异常: {self._safe_str(sid_query_e)}"
                        )
                    ...
                    if recent_order and sid_match_type in {
                        "pending_ship",
                        "bargain_ready",
                    }:
                        ...
                        if (
                            send_user_id
                            and fallback_buyer_id
                            and self._is_trustworthy_buyer_id(fallback_buyer_id)
                            and str(send_user_id) != str(fallback_buyer_id)
                        ):
                            logger.warning(
                        ...
                        if (
                            item_id
                            and item_id != "未知商品"
                            and fallback_item_id
                            and str(item_id) != str(fallback_item_id)
                        ):
                            logger.warning(
                    ...
                    logger.warning(
                        f"[{msg_time}] 【{self.cookie_id}】❌ 未能提取到订单ID且无可用sid，跳过自动发货"
                    )
                    self._record_delivery_log(
                        item_id=item_id,
                        buyer_id=send_user_id,
            ...
            try:
                ...
                logger.error(
                    f"[{msg_time}] 【{self.cookie_id}】查询订单一致性校验失败: {self._safe_str(order_check_e)}"
                )
                existing_order = None
            ...
            if existing_order:
                ...
                if (
                    send_user_id
                    and existing_buyer_id
                    and self._is_trustworthy_buyer_id(existing_buyer_id)
                    and str(send_user_id) != str(existing_buyer_id)
                ):
                    logger.warning(
                ...
                if (
                    ...
                    and str(item_id) != str(existing_item_id)
                ):
                    logger.warning(
                ...
                if (not item_id or item_id == "未知商品") and existing_item_id:
                    ...
                    logger.info(
                        f"[{msg_time}] 【{self.cookie_id}】订单一致性校验补全商品ID: {item_id}"
                    )

            logger.info(
                f"[{msg_time}] 【{self.cookie_id}】提取到订单ID: {order_id}，将在自动发货时处理确认发货"
            )
            ...
            if self.is_lock_held(lock_key):
                logger.info(
                    f"[{msg_time}] 【{self.cookie_id}】🔒【提前检查】订单 {lock_key} 延迟锁仍在持有状态，跳过发货"
                )
                self._record_delivery_log(
                    order_id=order_id,
                    item_id=item_id,
            ...
            if not self.can_auto_delivery(order_id):
                logger.info(
                    f"[{msg_time}] 【{self.cookie_id}】订单 {order_id} 在冷却期内，跳过发货"
                )
                self._record_delivery_log(
                    order_id=order_id,
                    item_id=item_id,
            ...
            async with order_lock:
                logger.info(
                    f"[{msg_time}] 【{self.cookie_id}】获取订单锁成功: {lock_key}，开始处理自动发货"
                )

                # 第三重检查：获取锁后再次检查延迟锁状态（双重检查，防止在等待锁期间状态发生变化）
                if self.is_lock_held(lock_key):
                    logger.info(
                        f"[{msg_time}] 【{self.cookie_id}】订单 {lock_key} 在获取锁后检查发现延迟锁仍持有，跳过发货"
                    )
                    self._record_delivery_log(
                        order_id=order_id,
                        item_id=item_id,
                ...
                if not self.can_auto_delivery(order_id):
                    logger.info(
                        f"[{msg_time}] 【{self.cookie_id}】订单 {order_id} 在获取锁后检查发现仍在冷却期，跳过发货"
                    )
                    self._record_delivery_log(
                        order_id=order_id,
                        item_id=item_id,
                ...
                try:
                    ...
                    logger.info(
                        f"【{self.cookie_id}】准备自动发货: item_id={item_id}, item_title={item_title}"
                    )
                            ...
                            if order_detail and order_detail.get("quantity"):
                                try:
                                    ...
                                    if order_quantity > 1:
                                        ...
                                        logger.info(
                                            f"订单数量为 {order_quantity}，发送单个卡券"
                                        )
                                except (ValueError, TypeError):
                                    logger.warning(
                                        f"订单数量格式无效: {order_detail.get('quantity')}，发送单个卡券"
                                    )
                            else:
                                logger.info(f"未获取到订单数量信息，发送单个卡券")
                        except Exception as e:
                            logger.error(
                                f"获取订单详情失败: {self._safe_str(e)}，发送单个卡券"
                            )
                    ...
                    for i in range(quantity_to_send):
                        ...
                        try:
                            pending_finalize_meta = (
                                self._get_pending_delivery_finalization_meta(
                                    order_id, unit_index
                                )
                            )
                            if pending_finalize_meta:
                                finalize_result = (
                                    await self._finalize_delivery_after_send(
                                        delivery_meta=pending_finalize_meta,
                                        order_id=order_id,
                                        item_id=item_id,
                                    )
                            ...
                            if isinstance(delivery_result, dict):
                                ...
                                delivery_error = delivery_result.get("error")
                                delivery_steps = (
                                    delivery_result.get("delivery_steps") or []
                                )
                                rule_meta = {
                                    "success": True,
                                    "rule_id": delivery_result.get("rule_id"),
                                    ...
                                    "card_type": delivery_result.get("card_type"),
                                    "match_mode": delivery_result.get("match_mode"),
                                    "order_spec_mode": delivery_result.get(
                                        "order_spec_mode"
                                    ),
                                    "rule_spec_mode": delivery_result.get(
                                        "rule_spec_mode"
                                    ),
                                    "item_config_mode": delivery_result.get(
                                        "item_config_mode"
                                    ),
                                    "card_id": delivery_result.get("card_id"),
                                    "card_description": delivery_result.get(
                                        "card_description"
                                    ),
                                    "data_card_pending_consume": delivery_result.get(
                                        "data_card_pending_consume"
                            ...
                            if not delivery_content:
                                failure_reason = (
                                    delivery_error
                                    or f"第 {unit_index}/{quantity_to_send} 个卡券内容获取失败"
                                )
                            ...
                            if not delivery_steps:
                                delivery_steps = self._build_delivery_steps(
                                    delivery_content,
                                    rule_meta.get("card_description", ""),
                                )
                            if not delivery_steps:
                                failure_reason = f"第 {unit_index}/{quantity_to_send} 个卡券发货步骤构建失败"
                                last_delivery_error = failure_reason
                                self._release_data_reservation_if_needed(
                                    rule_meta, error=failure_reason
                                )
                                self._record_delivery_log(
                                    order_id=order_id,
                                    item_id=item_id,
                            ...
                            self._release_data_reservation_if_needed(
                                rule_meta, error=f"准备发货失败: {self._safe_str(e)}"
                            )
                    ...
                    send_groups = self._build_delivery_send_groups(
                        prepared_units, quantity_to_send
                    )
                    ...
                    for group_index, send_group in enumerate(send_groups, start=1):
                        ...
                        for prepared_unit in group_units:
                            ...
                            unit_index = prepared_unit.get("unit_index") or 1
                            unit_delivery_steps = (
                                prepared_unit.get("delivery_steps") or []
                            )
                            ...
                            try:
                                ...
                                if not finalize_result.get("success"):
                                    last_delivery_error = (
                                        finalize_result.get("error")
                                        or f"第 {unit_index} 条消息发送成功但提交发货副作用失败"
                                    )
                                    self._persist_delivery_finalization_state(
                                        order_id=order_id,
                                        item_id=item_id,
                                ...
                                has_image_step = any(
                                    step.get("type") == "image"
                                    for step in unit_delivery_steps
                                )
                                if has_image_step:
                    ...
                    if progress_summary and progress_summary.get(
                        "aggregate_status"
                    ) in {"partial_success", "partial_pending_finalize", "shipped"}:
                        self._activate_delivery_lock(lock_key, delay_minutes=10)
                    ...
                    if successful_send_count > 0:
                        if progress_summary and quantity_to_send > 1:
                            ...
                            pending_finalize_count = progress_summary.get(
                                "pending_finalize_count", 0
                            )
                        ...
                        logger.warning(
                            f"[{msg_time}] 【自动发货】未找到匹配的发货规则或获取发货内容失败"
                        )
                        self._record_delivery_log(
                            order_id=order_id,
                            item_id=item_id,
                ...
                logger.info(
                    f"[{msg_time}] 【{self.cookie_id}】订单锁释放: {lock_key}，自动发货处理完成"
                )
    ...
    def _reload_latest_cookies_from_db(self, reason: str = "") -> bool:
        ...
        try:
            ...
            if new_cookies_str and new_cookies_str != self.cookies_str:
                ...
                logger.info(
                    f"【{self.cookie_id}】检测到数据库中的cookie已更新，重新加载cookie{suffix}"
                )
            ...
            logger.warning(
                f"【{self.cookie_id}】从数据库重新加载cookie失败，继续使用当前cookie: {self._safe_str(reload_e)}"
            )
        return False
    ...
    def protected_merge_cookie_dicts(cls, existing_cookies_dict, incoming_cookies_dict):
        ...
        account_switched = bool(
            existing_unb and incoming_unb and existing_unb != incoming_unb
        )
        ...
        if account_switched:
            ...
            preserved_protected_fields = [
                key
                for key in would_remove_fields
                if key in PROTECTED_SESSION_COOKIE_FIELDS and existing.get(key)
            ]

        missing_protected_fields = [
            key for key in PROTECTED_SESSION_COOKIE_FIELDS if not merged.get(key)
        ]
        missing_required_fields = [
            key for key in REQUIRED_SESSION_COOKIE_FIELDS if not merged.get(key)
        ]
        incoming_missing_protected_fields = [
            key for key in PROTECTED_SESSION_COOKIE_FIELDS if not incoming.get(key)
        ]
        incoming_missing_required_fields = [
            key for key in REQUIRED_SESSION_COOKIE_FIELDS if not incoming.get(key)
        ]
    ...
    def _merge_cookie_dicts(self, incoming_cookies_dict, existing_cookies_dict=None):
        ...
        merge_result = self.protected_merge_cookie_dicts(
            ...
            if existing_cookies_dict is not None
            else trans_cookies(self.cookies_str),
    ...
    def _log_protected_merge_event(self, event_name: str, merge_result: Dict[str, Any]):
        ...
        protected_preserved_fields = (
            merge_result.get("preserved_protected_fields") or []
        )
    ...
    def _log_cookie_merge_summary(
        ...
        preserved_protected_fields=None,
        would_remove_fields=None,
        removed_fields=None,
        missing_protected_fields=None,
        missing_required_fields=None,
        incoming_missing_protected_fields=None,
        account_switched: bool = False,
    ):
        """打印 Cookie 合并结果，重点关注会话关键字段。"""
        context_prefix = f"{context}：" if context else ""
        logger.info(
            f"【{self.cookie_id}】{context_prefix}合并后cookies包含 {len(merged_cookies_dict)} 个字段"
        )

        if updated_fields:
            logger.info(
                f"【{self.cookie_id}】{context_prefix}更新的cookie字段: {', '.join(updated_fields)}"
            )
        else:
        ...
        if account_switched:
            logger.warning(
                f"【{self.cookie_id}】{context_prefix}检测到unb变化，按账号切换处理，不保留旧账号Cookie字段"
            )
        ...
        for key in important_keys:
            if key in merged_cookies_dict:
                ...
                logger.info(
                    f"【{self.cookie_id}】  ✅ {key}: {'存在' if val else '为空'} (长度: {len(str(val)) if val else 0}){marker}"
                )
            else:
    ...
    async def preflight_token_after_manual_refresh(self) -> str:
        ...
        for attempt in range(1, max_preflight_retries + 1):
            ...
            if token:
                self.cache_auth_prewarmed_token(
                    self.cookie_id, token, source="manual_refresh_handoff"
                )
                logger.info(
                    f"【{self.cookie_id}】手动刷新后的Token预检成功（第{attempt}次），已缓存预热token供新实例复用"
                )
                return token
        ...
        raise InitAuthError(
            f"手动刷新后的Token预检失败，状态: {self.last_token_refresh_status or 'unknown'}"
        )

    async def refresh_token(
        self, captcha_retry_count: int = 0, allow_password_login_recovery: bool = True
    ):
        if self.token_refresh_lock.locked():
            logger.info(
                f"【{self.cookie_id}】Token刷新已有执行中任务，等待当前流程完成后复用结果"
            )
        ...
        async with self.token_refresh_lock:
            if (
                ...
                and self.last_token_refresh_status == "success"
                and (time.time() - self.last_token_refresh_time) < 15
            ):
                logger.info(
                    f"【{self.cookie_id}】最近15秒内已有成功的Token刷新结果，直接复用当前Token"
                )
    ...
    async def _refresh_token_impl(
        ...
        post_slider_session_grace_used: bool = False,
        ...
        try:
            logger.info(
                f"【{self.cookie_id}】开始刷新token... (滑块验证重试次数: {captcha_retry_count})"
            )
            # 标记本次刷新状态
            ...
            if captcha_retry_count >= self.max_captcha_verification_count:
                logger.error(
                    f"【{self.cookie_id}】滑块验证重试次数已达上限 ({self.max_captcha_verification_count})，停止重试"
                )
            ...
            if (
                ...
                remaining_time = (
                    self.message_cookie_refresh_cooldown - time_since_last_message
                )
                remaining_minutes = int(remaining_time // 60)
                remaining_seconds = int(remaining_time % 60)
                logger.info(
                    f"【{self.cookie_id}】收到消息后冷却中，放弃本次token刷新，还需等待 {remaining_minutes}分{remaining_seconds}秒"
                )
                # 标记为因冷却而跳过（正常情况）
            ...
            # 获取token
            token = (
                trans_cookies(self.cookies_str).get("_m_h5_tk", "").split("_")[0]
                if trans_cookies(self.cookies_str).get("_m_h5_tk")
                else ""
            )
            ...
            # 详细调试信息（仅debug级别）
            logger.debug(
                f"【{self.cookie_id}】Token刷新参数: timestamp={params['t']}, sign={sign[:16]}..."
            )
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    ...
                    logger.debug(
                        f"【{self.cookie_id}】Token刷新响应: status={response.status}, ret={ret_info}"
                    )
                    ...
                    if isinstance(res_json, dict):
                        ...
                        if any("SUCCESS::调用成功" in ret for ret in ret_value):
                            if "data" in res_json and "accessToken" in res_json["data"]:
                                ...
                                self.last_message_received_time = 0
                                logger.warning(
                                    f"【{self.cookie_id}】Token刷新成功，已重置消息接收时间标识"
                                )
                    ...
                    if self._need_captcha_verification(res_json):
                        ...
                        if qr_login_grace and not qr_login_grace.get(
                            "captcha_buffer_used"
                        ):
                            logger.warning(
                                f"【{self.cookie_id}】扫码登录后的首轮Token刷新命中风控，先执行浏览器侧Cookie稳定化"
                            )
                            log_captcha_event(
                                self.cookie_id,
                            ...
                            stabilization_success = (
                                await self._refresh_cookies_via_browser_page(
                                    self.cookies_str, restart_on_success=False
                                )
                            )
                            if stabilization_success:
                                ...
                                logger.info(
                                    f"【{self.cookie_id}】浏览器侧Cookie稳定化完成，重新尝试Token刷新"
                                )
                                return await self._refresh_token_impl(
                                    captcha_retry_count,
                            ...
                            logger.warning(
                                f"【{self.cookie_id}】浏览器侧Cookie稳定化未消除风控，继续进入滑块验证"
                            )

                        if self.is_manual_refresh_active(
                            self.cookie_id, allow_handoff_recovery=True
                        ):
                            logger.warning(
                                f"【{self.cookie_id}】检测到手动刷新进行中，跳过自动滑块处理"
                            )
                            log_captcha_event(
                                self.cookie_id,
                                "手动刷新进行中，跳过自动滑块处理",
                        ...
                        logger.warning(
                            f"【{self.cookie_id}】检测到需要滑块验证，开始处理..."
                        )

                        # 记录滑块验证检测到日志文件
                        verification_url = res_json.get("data", {}).get(
                            "url", "Token刷新时检测"
                        )
                        log_captcha_event(
                            self.cookie_id,
                            "检测到滑块验证",
                            None,
                            f"触发场景: Token刷新, URL: {verification_url}",
                        )
                        ...
                        try:
                            ...
                            if log_id:
                                logger.info(
                                    f"【{self.cookie_id}】风控日志记录成功，ID: {log_id}"
                                )
                        except Exception as log_e:
                            logger.error(
                                f"【{self.cookie_id}】记录风控日志失败: {log_e}"
                            )
                        ...
                        try:
                            ...
                            if new_cookies_str:
                                logger.info(
                                    f"【{self.cookie_id}】滑块验证成功，准备重启实例..."
                                )
                            ...
                            logger.error(
                                f"【{self.cookie_id}】滑块验证处理异常: {self._safe_str(captcha_e)}"
                            )

                            # 更新风控日志为异常状态
                            captcha_duration = (
                                time.time() - captcha_start_time
                                if "captcha_start_time" in locals()
                                else 0
                            )
                            if "log_id" in locals() and log_id:
                                self._update_risk_log(
                    ...
                    if isinstance(res_json, dict):
                        res_json_str = json.dumps(
                            res_json, ensure_ascii=False, separators=(",", ":")
                        )
                        if "令牌过期" in res_json_str or "Session过期" in res_json_str:
                            # 记录令牌/Session过期到风控日志
                            token_expired_log_id = None
                            token_expired_session_id = self._new_risk_session_id(
                                "token"
                            )
                            ...
                            try:
                                ...
                                stale_count = (
                                    db_manager.mark_stale_risk_control_logs_failed(
                                        timeout_minutes=15, cookie_id=self.cookie_id
                                    )
                                )
                                if stale_count > 0:
                                    logger.warning(
                                        f"【{self.cookie_id}】检测到{stale_count}条超时processing风控日志，已自动标记failed"
                                    )
                            ...
                            if self.is_manual_refresh_active(
                                ...
                                logger.warning(
                                    f"【{self.cookie_id}】检测到手动刷新进行中，跳过自动密码登录刷新"
                                )
                                if token_expired_log_id:
                                    self._update_risk_log(
                                        ...
                                        duration_ms=max(
                                            0,
                                            int(
                                                (time.time() - token_expired_started_at)
                                                * 1000
                                            ),
                                        ),
                                        event_meta=self._build_risk_event_meta(
                            ...
                            if (
                                self._has_recent_slider_success()
                                and not post_slider_session_grace_used
                            ):
                            ...
                            if allow_password_login_recovery:
                                ...
                                logger.warning(
                                    f"【{self.cookie_id}】当前为预检模式，跳过密码登录恢复，直接返回Token刷新失败"
                                )
                            ...
                            if token_expired_log_id:
                                self._update_risk_log(
                                    ...
                                    session_id=token_expired_session_id,
                                    trigger_scene=token_trigger_scene,
                                    result_code="token_refresh_recovered"
                                    if refresh_success
                                    else "token_refresh_recovery_failed",
                                    processing_status="success"
                                    if refresh_success
                                    else "failed",
                    ...
                    if not notification_sent:
                        ...
                        if is_ws_connected:
                            logger.info(
                                f"【{self.cookie_id}】WebSocket连接正常，Token刷新失败可能是暂时的，跳过失败通知"
                            )
                        else:
                            logger.warning(
                                f"【{self.cookie_id}】WebSocket未连接，发送Token刷新失败通知"
                            )
                            await self.send_token_refresh_notification(
                                f"Token刷新失败: {res_json}", "token_refresh_failed"
                            )
                    else:
                        logger.info(
                            f"【{self.cookie_id}】已发送滑块验证相关通知，跳过Token刷新失败通知"
                        )
                    return None
            ...
            if not notification_sent:
                ...
                if is_ws_connected:
                    logger.info(
                        f"【{self.cookie_id}】WebSocket连接正常，Token刷新异常可能是暂时的，跳过失败通知"
                    )
                else:
                    logger.warning(
                        f"【{self.cookie_id}】WebSocket未连接，发送Token刷新异常通知"
                    )
                    await self.send_token_refresh_notification(
                ...
                logger.info(
                    f"【{self.cookie_id}】已发送滑块验证相关通知，跳过Token刷新异常通知"
                )
            return None
        ...
        try:
            ...
            for keyword in captcha_keywords:
                if keyword in error_msg:
                    logger.info(
                        f"【{self.cookie_id}】检测到需要滑块验证的关键词: {keyword}"
                    )
                    return True
            ...
            logger.error(
                f"【{self.cookie_id}】检查是否需要滑块验证时出错: {self._safe_str(e)}"
            )
            return False
    ...
    async def _handle_captcha_verification(self, res_json: dict) -> str:
        ...
        try:
            ...
            if not verification_url:
                logger.info(
                    f"【{self.cookie_id}】未找到验证URL，认为不需要滑块验证，返回正常"
                )
                return None
            ...
            try:
                ...
                logger.info(
                    f"【{self.cookie_id}】XianyuSliderStealth导入成功，使用滑块验证"
                )
                ...
                if success and cookies:
                    ...
                    for cookie_name, cookie_value in cookies.items():
                        ...
                        if (
                            cookie_name_lower.startswith("x5")
                            or "x5sec" in cookie_name_lower
                        ):
                            x5sec_cookies[cookie_name] = cookie_value

                    logger.info(
                        f"【{self.cookie_id}】找到{len(x5sec_cookies)}个x5相关cookies: {list(x5sec_cookies.keys())}"
                    )

                    merge_result = self.protected_merge_cookie_dicts(
                        current_cookies_dict, cookies
                    )
                    ...
                    preserved_protected_fields = merge_result[
                        "preserved_protected_fields"
                    ]
                    ...
                    incoming_missing_protected_fields = merge_result[
                        "incoming_missing_protected_fields"
                    ]
                    account_switched = merge_result["account_switched"]
                    cookies_str = "; ".join(
                        [f"{k}={v}" for k, v in updated_cookies.items()]
                    )
                    qr_login_grace = self.get_qr_login_grace(self.cookie_id)
                    merge_event_name = (
                        "slider_post_qr_protected_merge"
                        if qr_login_grace
                        else "captcha_protected_merge"
                    )
                    ...
                    if missing_required_fields:
                        logger.error(
                            f"【{self.cookie_id}】滑块验证后的Cookie仍缺失核心字段，放弃写回数据库: {', '.join(missing_required_fields)}"
                        )
                        return None
                    ...
                    try:
                        ...
                        logger.error(
                            f"【{self.cookie_id}】自动更新数据库cookies失败: {self._safe_str(update_e)}"
                        )
                    ...
                    if is_ws_connected:
                        logger.info(
                            f"【{self.cookie_id}】WebSocket连接正常，滑块验证失败可能是暂时的，跳过通知"
                        )
                    else:
                        logger.warning(
                            f"【{self.cookie_id}】WebSocket未连接，发送滑块验证失败通知"
                        )
                        await self.send_token_refresh_notification(
                ...
                logger.error(
                    f"【{self.cookie_id}】XianyuSliderStealth导入失败: {import_e}"
                )
                logger.error(
                    f"【{self.cookie_id}】请安装Playwright库: pip install playwright"
                )

                # 记录导入失败到日志文件
                log_captcha_event(
                    self.cookie_id,
                    "XianyuSliderStealth导入失败",
                    False,
                ...
                logger.error(
                    f"【{self.cookie_id}】滑块验证异常: {self._safe_str(stealth_e)}"
                )
                ...
                if is_ws_connected:
                    logger.info(
                        f"【{self.cookie_id}】WebSocket连接正常，滑块验证执行异常可能是暂时的，跳过通知"
                    )
                else:
                    logger.warning(
                        f"【{self.cookie_id}】WebSocket未连接，发送滑块验证执行异常通知"
                    )
                    await self.send_token_refresh_notification(
        ...
        try:
            ...
            try:
                ...
                if not new_cookies_dict:
                    ...
                    return False
                logger.info(
                    f"【{self.cookie_id}】新cookies解析成功，包含 {len(new_cookies_dict)} 个字段"
                )
            except Exception as parse_e:
                logger.error(
                    f"【{self.cookie_id}】新cookies解析异常: {self._safe_str(parse_e)}"
                )
                return False
            ...
            try:
                merge_result = self.protected_merge_cookie_dicts(
                    trans_cookies(self.cookies_str), new_cookies_dict
                )
                merged_cookies_dict = merge_result["merged_cookies_dict"]
                updated_fields = merge_result["updated_fields"]
                changed_fields = merge_result["changed_fields"]
                new_fields = merge_result["new_fields"]
                self._log_protected_merge_event(
                    "password_refresh_protected_merge", merge_result
                )
                ...
                new_cookies_str = "; ".join(
                    [f"{k}={v}" for k, v in merged_cookies_dict.items()]
                )
                new_cookies_dict = merged_cookies_dict

            except Exception as merge_e:
                logger.error(
                    f"【{self.cookie_id}】cookies合并异常: {self._safe_str(merge_e)}"
                )
            ...
            try:
                ...
                try:
                    ...
                    logger.error(
                        f"【{self.cookie_id}】cookies回滚失败: {self._safe_str(rollback_e)}"
                    )
            ...
            logger.error(
                f"【{self.cookie_id}】更新cookies并重启任务时出错: {self._safe_str(e)}"
            )
            return False
    ...
    async def update_config_cookies(self):
        ...
        try:
            ...
            if hasattr(self, "cookie_id") and self.cookie_id:
                try:
                    ...
                    if not success:
                        ...
                        logger.warning(
                            f"更新Cookie到数据库失败: {self.cookie_id}，但不使用save_cookie避免覆盖账号密码"
                        )
                    else:
                    ...
                    # 发送数据库更新失败通知
                    await self.send_token_refresh_notification(
                        f"数据库Cookie更新失败: {str(e)}", "db_update_failed"
                    )
            else:
            ...
            # 发送Cookie更新失败通知
            await self.send_token_refresh_notification(
                f"Cookie更新失败: {str(e)}", "cookie_update_failed"
            )
    ...
    async def _try_password_login_refresh(
        ...
        """
        logger.warning(
            f"【{self.cookie_id}】检测到{trigger_reason}，准备刷新Cookie并重启实例..."
        )
        trigger_scene = trigger_scene or self._normalize_risk_trigger_scene(
            trigger_reason, default="auto_cookie_refresh"
        )
        ...
        try:
            ...
            refresh_risk_log_id = self._create_risk_log(
                ...
                processing_status="processing",
                event_meta=self._build_risk_event_meta(
                    trigger_scene=trigger_scene, extra=base_event_meta
                ),
            )
        except Exception as log_e:
        ...
        if failure_backoff:
            ...
            if remaining_time > 0:
                ...
                if backoff_reason == "slider_failed" and (
                    ignore_slider_failed_backoff
                    or self.consume_manual_refresh_slider_failed_bypass(self.cookie_id)
                ):
                    logger.warning(
                    ...
                    if refresh_risk_log_id:
                        self._update_risk_log(
                            ...
                            duration_ms=max(
                                0, int((time.time() - risk_log_started_at) * 1000)
                            ),
                            event_meta=self._build_risk_event_meta(
                                trigger_scene=trigger_scene,
                                extra={
                                    **base_event_meta,
                                    ...
                                    "backoff_seconds": failure_backoff.get("seconds"),
                                },
                            ),
                        )
                    return False

        last_password_login = XianyuLive._last_password_login_time.get(
            self.cookie_id, 0
        )
        ...
        if (
            ...
            logger.warning(
                f"【{self.cookie_id}】距离上次密码登录仅 {time_since_last_login:.1f} 秒，仍在冷却期内（还需等待 {remaining_time:.1f} 秒），跳过密码登录"
            )
            logger.warning(
                f"【{self.cookie_id}】提示：如果新Cookie仍然无效，请检查账号状态或手动更新Cookie"
            )
            if refresh_risk_log_id:
                self._update_risk_log(
                    refresh_risk_log_id,
        ...
        if not recovery_lock_acquired:
            ...
            logger.warning(
                f"【{self.cookie_id}】认证恢复流程已在执行中，跳过本次重复触发: owner={existing_owner}"
            )
            if refresh_risk_log_id:
                self._update_risk_log(
                    refresh_risk_log_id,
        ...
        log_captcha_event(
            self.cookie_id,
            f"{trigger_reason}触发Cookie刷新和实例重启",
            None,
        ...
        try:
            ...
            if db_cookie_value and db_cookie_value != self.cookies_str:
                logger.info(
                    f"【{self.cookie_id}】检测到数据库中的cookie已更新，重新加载cookie"
                )
                self.cookies_str = db_cookie_value
                self.cookies = trans_cookies(self.cookies_str)
                logger.info(
                    f"【{self.cookie_id}】Cookie已从数据库重新加载，跳过密码登录刷新"
                )
                if refresh_risk_log_id:
                    self._update_risk_log(
                        refresh_risk_log_id,
            ...
            if not username or not password:
                logger.warning(
                    f"【{self.cookie_id}】未配置用户名或密码，跳过密码登录刷新"
                )
                await self.send_token_refresh_notification(
            ...
            logger.info(
                f"【{self.cookie_id}】开始使用{browser_mode}浏览器进行密码登录刷新Cookie..."
            )
            logger.info(f"【{self.cookie_id}】使用账号: {username}")

            # 创建一个通知回调包装函数，支持接收截图路径和验证链接
            async def notification_callback_wrapper(
                message: str, screenshot_path: str = None, verification_url: str = None
            ):
            ...
            if result:
                ...
                # 打印密码登录获取的Cookie字段详情
                logger.info(
                    f"【{self.cookie_id}】========== 密码登录Cookie字段详情 =========="
                )
                ...
                for i, (key, value) in enumerate(result.items(), 1):
                    if len(str(value)) > 50:
                        logger.info(
                            f"【{self.cookie_id}】  {i:2d}. {key}: {str(value)[:30]}...{str(value)[-20:]} (长度: {len(str(value))})"
                        )
                    else:
                ...
                for key in important_keys:
                    if key in result:
                        val = result[key]
                        logger.info(
                            f"【{self.cookie_id}】  ✅ {key}: {'存在' if val else '为空'} (长度: {len(str(val)) if val else 0})"
                        )
                    else:
                        logger.info(f"【{self.cookie_id}】  ❌ {key}: 缺失")
                logger.info(
                ...
                logger.info(
                    f"【{self.cookie_id}】Cookie字符串摘要: {self._summarize_cookie_string(new_cookies_str)}"
                )

                # 记录密码登录时间，防止重复登录
                XianyuLive._last_password_login_time[self.cookie_id] = time.time()
                logger.warning(
                    f"【{self.cookie_id}】已记录密码登录时间，冷却期 {XianyuLive._password_login_cooldown} 秒"
                )
                ...
                try:
                    ...
                    logger.warning(
                        f"【{self.cookie_id}】发送通知失败: {self._safe_str(notify_e)}"
                    )
                ...
                login_error = (
                    getattr(slider, "last_login_error", "")
                    or "密码登录失败，未获取到Cookie"
                )
                backoff_reason, backoff_seconds = (
                    XianyuLive.classify_password_login_failure(login_error)
                )
                XianyuLive.set_password_login_failure_backoff(
                    self.cookie_id, backoff_reason, backoff_seconds
                )
                logger.warning(
                    f"【{self.cookie_id}】密码登录失败，未获取到Cookie: {login_error}"
                )
                logger.warning(
                    f"【{self.cookie_id}】已进入失败退避期: {backoff_reason}, {backoff_seconds}秒"
                )
                if refresh_risk_log_id:
                    self._update_risk_log(
                        refresh_risk_log_id,
            ...
            backoff_reason, backoff_seconds = (
                XianyuLive.classify_password_login_failure(str(refresh_e))
            )
            XianyuLive.set_password_login_failure_backoff(
                self.cookie_id, backoff_reason, backoff_seconds
            )
            logger.error(
                f"【{self.cookie_id}】Cookie刷新或实例重启失败: {self._safe_str(refresh_e)}"
            )
            import traceback
            ...
            if recovery_lock_acquired:
                XianyuLive.release_auth_recovery_lock(
                    self.cookie_id, recovery_lock_owner
                )
    ...
    async def _verify_cookie_validity(self) -> dict:
        ...
        try:
            ...
            async with self.session.get(
                ...
                redirected_to_login = (
                    "passport.goofish.com" in final_url
                    or "mini_login" in final_url
                    or (
                        "mini_login.htm" in page_text
                        and "alibaba-login-box" in page_text
                    )
                )

                if redirected_to_login or response.status in (401, 403):
                    logger.warning(
                        f"【{self.cookie_id}】❌ 网页登录态验证失败: 已进入登录/验证页 ({final_url})"
                    )
                    ...
                    logger.warning(
                        f"【{self.cookie_id}】⚠️ 网页登录态验证遇到服务端异常: HTTP {response.status}"
                    )
                    ...
                    result["details"].append(
                        f"网页登录态: 服务端异常，结果不确定 (HTTP {response.status})"
                    )
                elif response.status == 200:
                    logger.info(
                        f"【{self.cookie_id}】✅ 网页登录态验证通过: {final_url}"
                    )
                    result["web_session_api"] = True
                    result["details"].append("网页登录态: 通过验证")
                else:
                    logger.warning(
                        f"【{self.cookie_id}】⚠️ 网页登录态验证结果不明确: HTTP {response.status}, URL={final_url}"
                    )
                    result["web_session_api"] = None
                    result["inconclusive"] = True
                    if result["valid"]:
                        result["relogin_recommended"] = False
                    result["details"].append(
                        f"网页登录态: 结果不明确 (HTTP {response.status})"
                    )
            ...
            if result["valid"]:
                result["relogin_recommended"] = False
            result["details"].append(
                f"网页登录态: 网络异常，结果不确定 ({error_str[:50]})"
            )
        except Exception as e:
            error_str = self._safe_str(e)
            ...
            if result["valid"]:
                result["relogin_recommended"] = False
            result["details"].append(
                f"网页登录态: 验证异常，结果不确定 - {error_str[:50]}"
            )

        # 3. 测试图片上传API - 创建测试图片并实际上传
        try:
            logger.info(
                f"【{self.cookie_id}】测试图片上传API（使用测试图片实际上传）..."
            )
            ...
            test_image_path = os.path.join(
                temp_dir, f"cookie_test_{self.cookie_id}.png"
            )
            ...
            try:
                ...
                try:
                    ...
                    for attempt in range(2):
                        ...
                        error_message = (
                            getattr(uploader, "last_error_message", None) or "未知原因"
                        )
                        is_retryable_auth = (
                            error_type == "auth"
                            and error_message == "返回登录页面"
                            and result["web_session_api"] is not False
                        )
                ...
                if upload_result:
                    # 上传成功，Cookie有效
                    logger.info(
                        f"【{self.cookie_id}】✅ 图片上传API验证通过: 上传成功 ({upload_result[:50]}...)"
                    )
                    ...
                    if error_type == "network":
                        logger.warning(
                            f"【{self.cookie_id}】⚠️ 图片上传API验证遇到网络异常，不判定为Cookie失效: {error_message}"
                        )
                        ...
                        if result["valid"]:
                            result["relogin_recommended"] = False
                        result["details"].append(
                            f"图片上传API: 网络异常，结果不确定 ({error_message[:50]})"
                        )
                    elif (
                        error_type == "http"
                        and getattr(uploader, "last_http_status", None)
                        and uploader.last_http_status >= 500
                    ):
                        logger.warning(
                            f"【{self.cookie_id}】⚠️ 图片上传API返回服务端异常，不判定为Cookie失效: HTTP {uploader.last_http_status}"
                        )
                        result["image_api"] = None
                        result["inconclusive"] = True
                        if result["valid"]:
                            result["relogin_recommended"] = False
                        result["details"].append(
                            f"图片上传API: 服务端异常，结果不确定 (HTTP {uploader.last_http_status})"
                        )
                        ...
                        logger.warning(
                            f"【{self.cookie_id}】❌ 图片上传API验证失败: {error_message}"
                        )
                        result["image_api"] = False
                        result["valid"] = False
            ...
            if any(keyword.lower() in error_lower for keyword in auth_keywords):
                ...
                if result["valid"]:
                    result["relogin_recommended"] = False
                result["details"].append(
                    f"图片上传API: 验证异常，结果不确定 - {error_str[:50]}"
                )
        ...
        if result["image_api"] is False:
            ...
            logger.warning(
                f"【{self.cookie_id}】❌ 网页登录态与图片上传校验结果不一致，按严格策略判定Cookie失效"
            )
            result["valid"] = False
        ...
        if result["valid"]:
            if result["inconclusive"]:
                logger.warning(
                    f"【{self.cookie_id}】⚠️ Cookie验证结果不确定: 未发现明确失效证据，但部分校验存在波动或结果矛盾"
                )
            else:
    ...
    async def _restart_instance(self):
        ...
        try:
            ...
            if cookie_manager:
                ...
                def trigger_restart():
                    ...
                    try:
                        ...
                        cookie_manager.update_cookie(
                            self.cookie_id, self.cookies_str, save_to_db=False
                        )
                ...
                logger.warning(
                    f"【{self.cookie_id}】注意：重启请求已发送，CookieManager将在2秒后取消当前任务并启动新实例"
                )
            ...
            try:
                await self.send_token_refresh_notification(
                    f"实例重启失败: {str(e)}", "instance_restart_failed"
                )
            except Exception as notify_e:
                logger.error(
                    f"【{self.cookie_id}】发送重启失败通知时出错: {self._safe_str(notify_e)}"
                )

    async def save_item_info_to_db(
        self, item_id: str, item_detail: str = None, item_title: str = None
    ):
        """保存商品信息到数据库
    ...
    async def save_item_detail_only(self, item_id, item_detail):
        ...
        try:
            ...
            success = db_manager.update_item_detail(
                self.cookie_id, item_id, item_detail
            )
    ...
    async def fetch_item_detail_from_api(
        ...
        try:
            ...
            if detail_from_browser:
                ...
                logger.info(
                    f"成功通过浏览器获取商品详情: {item_id}, 长度: {len(detail_from_browser)}"
                )
    ...
    async def _add_to_item_cache(self, item_id: str, detail: str):
        ...
        async with self._item_detail_cache_lock:
            ...
            logger.warning(
                f"添加商品详情到缓存: {item_id}, 当前缓存大小: {len(self._item_detail_cache)}"
            )
    ...
    async def _fetch_item_detail_from_browser(self, item_id: str) -> str:
        ...
        try:
            ...
            try:
                    ...
                    try:
                        ...
                        if detail_element:
                            ...
                            if detail_text and len(detail_text.strip()) > 0:
                                logger.info(
                                    f"成功获取商品详情（选择器: {selector}）: {item_id}, 长度: {len(detail_text)}"
                                )
                ...
                if body_text:
                    logger.info(
                        f"获取到页面整体内容: {item_id}, 长度: {len(body_text)}"
                    )
                ...
                logger.warning(
                    f"获取商品详情元素失败: {item_id}, 错误: {self._safe_str(e)}"
                )
            ...
            logger.error(
                f"浏览器获取商品详情异常: {item_id}, 错误: {self._safe_str(e)}"
            )
            return ""
        finally:
            # 确保资源被正确清理
    ...
    async def save_items_list_to_db(self, items_list, sync_item_details=False):
        ...
        try:
                ...
                if existing_item:
                    ...
                    if sync_item_details:
                        items_need_detail.append(
                            {"item_id": item_id, "item_title": item.get("title", "")}
                        )
            ...
            if batch_update_data:
                update_count = db_manager.batch_update_item_title_price(
                    batch_update_data
                )
                logger.info(
                    f"更新商品标题和价格: {update_count}/{len(batch_update_data)} 个"
                )
            ...
            if items_need_detail:
                ...
                if auto_fetch_config.get("enabled", True):
                    action_text = (
                        "同步最新详情" if sync_item_details else "获取缺失详情"
                    )
                    logger.info(
                        f"准备为 {len(items_need_detail)} 个商品{action_text}..."
                    )
                    ...
                    logger.info(
                        f"成功为 {detail_success_count}/{len(items_need_detail)} 个商品{action_text}"
                    )
                else:
                    logger.info(
                        f"有 {len(items_need_detail)} 个商品需要获取详情，但自动获取功能已禁用"
                    )
            ...
            async def fetch_single_item_detail(item_info):
                async with semaphore:
                    try:
                        ...
                        if item_detail_text:
                            # 保存详情到数据库
                            success = await self.save_item_detail_only(
                                item_id, item_detail_text
                            )
                            if success:
                                logger.info(
                                    f"✅ 成功获取并保存商品详情: {item_id} - {item_title}"
                                )
                                return 1
                            else:
                        ...
                        logger.error(
                            f"获取单个商品详情异常: {item_info.get('item_id', 'unknown')}, 错误: {self._safe_str(e)}"
                        )
                        return 0

            # 并发获取所有商品详情
            tasks = [
                fetch_single_item_detail(item_info) for item_info in items_need_detail
            ]
    ...
    async def get_item_info(self, item_id, retry_count=0):
        ...
        if token:
            logger.warning(
                f"使用cookies中的_m_h5_tk token: {self._mask_secret_value(token, head=6, tail=4)}"
            )
        else:
        ...
        try:
            ...
            if isinstance(message_3, dict):
                ...
                if isinstance(content, str) and content:
                    ...
                    if id_match:
                        logger.info(
                            f"【{self.cookie_id}】从消息内容中提取商品ID: {id_match.group(1)}"
                        )
            ...
            def find_item_id_recursive(obj, path=""):
                if isinstance(obj, dict):
                    ...
                    if "itemId=" in obj:
                        ...
                        if id_match:
                            logger.info(
                                f"从{path}的URL参数中提取商品ID: {id_match.group(1)}"
                            )
                            return id_match.group(1)
        ...
        try:
            ...
            if isinstance(message, dict):
                for key, value in message.items():
                    logger.warning(
                        f"  键 '{key}': {type(value)} - {str(value)[:100]}..."
                    )
                    ...
                    if key in ["1", "3"] and isinstance(value, dict):
                        ...
                        for sub_key, sub_value in value.items():
                            logger.warning(
                                f"      '{sub_key}': {type(sub_value)} - {str(sub_value)[:50]}..."
                            )
            else:
        ...
        try:
            ...
            if item_id:
                ...
                if item_reply and item_reply.get("reply_content"):
                    ...
                    logger.info(
                        f"【{self.cookie_id}】使用指定商品回复: 商品ID={item_id}"
                    )
                    ...
                    try:
                        ...
                        logger.info(
                            f"【{self.cookie_id}】指定商品回复内容: {formatted_reply}"
                        )
                        return formatted_reply
                    except Exception as format_error:
                        logger.error(
                            f"指定商品回复变量替换失败: {self._safe_str(format_error)}"
                        )
                        # 如果变量替换失败，返回原始内容
                        return reply_content
                else:
                    logger.warning(
                        f"【{self.cookie_id}】商品ID {item_id} 没有配置指定回复，使用默认回复"
                    )

            # 2. 获取当前账号的默认回复设置
            default_reply_settings = db_manager.get_default_reply(self.cookie_id)

            if not default_reply_settings or not default_reply_settings.get(
                "enabled", False
            ):
            ...
            if default_reply_settings.get("reply_once", False) and chat_id:
                ...
                if db_manager.has_default_reply_record(self.cookie_id, chat_id):
                    logger.info(
                        f"【{self.cookie_id}】chat_id {chat_id} 已使用过默认回复，跳过（只回复一次）"
                    )
                    return None
        ...
        try:
            ...
            if item_id:
                for keyword_data in keywords:
                    ...
                    if (
                        keyword_item_id == item_id
                        and keyword.lower() in send_message.lower()
                    ):
                        logger.info(
                            f"商品ID关键词匹配成功: 商品{item_id} '{keyword}' (类型: {keyword_type})"
                        )

                        # 根据关键词类型处理
                        if keyword_type == "image" and image_url:
                            # 图片类型关键词，发送图片
                            return await self._handle_image_keyword(
                                keyword,
                                image_url,
                                send_user_name,
                                send_user_id,
                            ...
                            if not reply or (reply and reply.strip() == ""):
                                logger.info(
                                    f"商品ID关键词 '{keyword}' 回复内容为空，不进行回复"
                                )
                            ...
                            try:
                                ...
                                logger.error(
                                    f"关键词回复变量替换失败: {self._safe_str(format_error)}"
                                )
                                # 如果变量替换失败，返回原始内容
                                return reply
            ...
            for keyword_data in keywords:
                ...
                if not keyword_item_id and keyword.lower() in send_message.lower():
                    logger.info(
                        f"通用关键词匹配成功: '{keyword}' (类型: {keyword_type})"
                    )
    ...
    async def _handle_image_keyword(
        ...
        try:
            ...
            elif image_url.startswith("/static/uploads/") or image_url.startswith(
                "static/uploads/"
            ):
                # 本地图片，需要上传到闲鱼CDN
    ...
    def _is_cdn_url(self, url: str) -> bool:
        ...
        if url_lower.startswith("https://") and any(
            ext in url_lower for ext in [".jpg", ".jpeg", ".png", ".gif", ".webp"]
        ):
            return True
    ...
    async def _update_keyword_image_url(self, keyword: str, new_image_url: str):
        ...
        try:
            ...
            success = db_manager.update_keyword_image_url(
                self.cookie_id, keyword, new_image_url
            )
            if success:
    ...
    async def send_notification(
        ...
        try:
            ...
            notification_hash = hashlib.md5(
                notification_key.encode("utf-8")
            ).hexdigest()
            ...
            async with self.notification_lock:
                ...
                if notification_hash in self.last_notification_time:
                    time_since_last = (
                        current_time - self.last_notification_time[notification_hash]
                    )
                    if time_since_last < self.notification_cooldown:
                        remaining_seconds = int(
                            self.notification_cooldown - time_since_last
                        )
                        logger.warning(
                            f"📱 通知在冷却期内（剩余 {remaining_seconds} 秒），跳过重复发送 - 账号: {self.cookie_id}, 买家: {send_user_name}, 消息: {send_message[:30]}..."
                        )
                        return
                if reservation_key in self.pending_notification_keys:
                    logger.warning(
                        f"📱 相同消息通知正在发送中，跳过重复发送 - 账号: {self.cookie_id}, 买家: {send_user_name}"
                    )
                    return
            ...
            try:
                logger.info(
                    f"📱 开始发送消息通知 - 账号: {self.cookie_id}, 买家: {send_user_name}"
                )
                ...
                if not notification_sent:
                    logger.warning(
                        f"📱 消息通知未发送成功，不进入冷却 - 账号: {self.cookie_id}, 买家: {send_user_name}"
                    )
                    return
    ...
    async def _send_dingtalk_notification(self, config_data: dict, message: str):
        ...
        try:
            ...
            # 解析配置
            webhook_url = config_data.get("webhook_url") or config_data.get(
                "config", ""
            )
            ...
            data = {
                "msgtype": "markdown",
                "markdown": {"title": "闲鱼管理系统通知", "text": message},
            }
                    ...
                    if response.status == 200:
                        try:
                            ...
                            if response_json.get("code") == 0:
                                ...
                                logger.warning(
                                    f"📱 飞书通知发送失败: {response_json.get('msg', '未知错误')}"
                                )
                                return False
                        ...
                        logger.warning(
                            f"📱 飞书通知发送失败: HTTP {response.status}, 响应: {response_text}"
                        )
                        return False
    ...
    async def _send_bark_notification(self, config_data: dict, message: str):
        ...
        try:
            ...
            logger.info(
                f"📱 Bark通知 - 设备密钥: {device_key[:10]}..."
                if device_key
                else "📱 Bark通知 - 设备密钥: 未设置"
            )
                    ...
                    if response.status == 200:
                        try:
                            ...
                            if response_json.get("code") == 200:
                                ...
                                logger.warning(
                                    f"📱 Bark通知发送失败: {response_json.get('message', '未知错误')}"
                                )
                                return False
                        except json.JSONDecodeError:
                            # 某些Bark服务器可能返回纯文本
                            if (
                                ...
                                logger.warning(
                                    f"📱 Bark通知响应格式异常: {response_text}"
                                )
                                return False
                    else:
                        logger.warning(
                            f"📱 Bark通知发送失败: HTTP {response.status}, 响应: {response_text}"
                        )
                        return False
    ...
    async def _send_email_notification(
        self, config_data: dict, message: str, attachment_path: str = None
    ):
        """发送邮件通知（支持附件）
            ...
            try:
                ...
                try:
                    ...
                    error_code = (
                        ...
                        if hasattr(auth_error, "smtp_code")
                        else None
                    )
                    ...
                    if "qq.com" in email_user.lower() or "qq" in smtp_server.lower():
                        ...
                        suggestions.append(
                            "请到QQ邮箱设置 -> 账户 -> 开启SMTP服务 -> 生成授权码"
                        )
                    elif (
                        "gmail.com" in email_user.lower()
                        or "gmail" in smtp_server.lower()
                    ):
                        suggestions.append("Gmail需要使用应用专用密码")
                        suggestions.append(
                            "请到Google账户 -> 安全性 -> 两步验证 -> 应用专用密码"
                        )
                        suggestions.append("或启用'允许不够安全的应用访问'（不推荐）")
                    elif (
                        "163.com" in email_user.lower()
                        or "126.com" in email_user.lower()
                        or "yeah.net" in email_user.lower()
                    ):
                        suggestions.append("网易邮箱需要使用授权码")
                        suggestions.append(
                            "请到邮箱设置 -> POP3/SMTP/IMAP -> 开启SMTP服务 -> 生成授权码"
                        )
                    else:
    ...
    async def send_token_refresh_notification(
        ...
        try:
            ...
            if (
                notification_type != "token_scheduled_refresh_failed"
            ...
            async with self.notification_lock:
                ...
                if notification_key in self.pending_notification_keys:
                    logger.warning(
                        f"Token刷新通知正在发送中，跳过重复发送: {notification_type}"
                    )
                    return
                if current_time - last_time < cooldown_time:
                    ...
                    logger.warning(
                        f"Token刷新通知在冷却期内，跳过发送: {notification_type} (还需等待 {time_desc})"
                    )
                    return
            ...
            if "滑块验证成功" in error_message:
                ...
                "密码登录成功" in error_message
                or notification_type == "password_login_success"
            ):
                notification_msg = render_notification_template(
                ...
                "刷新Cookie成功" in error_message
                or notification_type == "cookie_refresh_success"
            ):
                notification_msg = render_notification_template(
                    "cookie_refresh_success",
                    account_id=self.cookie_id,
                ...
                or "短信验证" in error_message
                or "二维码验证" in error_message
                or "身份验证" in error_message
                or (verification_url and "passport" in verification_url)
            ):
                notification_msg = render_notification_template(
                    "face_verify",
                    account_id=self.cookie_id,
                    ...
                    verification_type=guess_verification_type(
                        error_message, verification_url
                    ),
                )
            ...
            if notification_sent:
                ...
                if self._is_token_related_error(error_message):
                    next_send_time = (
                        current_time + self.token_refresh_notification_cooldown
                    )
                    cooldown_desc = "3小时"
                else:
                ...
                next_send_time_str = time.strftime(
                    "%Y-%m-%d %H:%M:%S", time.localtime(next_send_time)
                )
                logger.info(
                    f"Token刷新通知已发送，下次可发送时间: {next_send_time_str} (冷却时间: {cooldown_desc})"
                )
            else:
                logger.warning(
                    f"【{self.cookie_id}】Token刷新通知未发送成功，不进入冷却: {notification_type}"
                )
    ...
    async def auto_confirm(self, order_id, item_id=None, retry_count=0):
        ...
        try:
            ...
            # 创建确认实例，传入主界面类实例
            secure_confirm = SecureConfirm(
                self.session, self.cookies_str, self.cookie_id, self
            )
            ...
            logger.error(
                f"【{self.cookie_id}】加密确认模块调用失败: {self._safe_str(e)}"
            )
            return {
                "error": f"加密确认模块调用失败: {self._safe_str(e)}",
                "order_id": order_id,
            }
    ...
    async def auto_freeshipping(self, order_id, item_id, buyer_id, retry_count=0):
        ...
        try:
            ...
            # 创建免拼发货实例
            secure_freeshipping = SecureFreeshipping(
                self.session, self.cookies_str, self.cookie_id
            )
            ...
            # 调用免拼发货方法
            return await secure_freeshipping.auto_freeshipping(
                order_id, item_id, buyer_id, retry_count
            )

        except Exception as e:
            logger.error(
                f"【{self.cookie_id}】免拼发货模块调用失败: {self._safe_str(e)}"
            )
            return {
                "error": f"免拼发货模块调用失败: {self._safe_str(e)}",
                "order_id": order_id,
            }
    ...
    async def fetch_order_detail_info(
        ...
        try:
            ...
            if lock_loop is not None and lock_loop is not current_loop:
                ...
                logger.info(
                    f"【{self.cookie_id}】订单详情锁 {order_id} 事件循环不匹配，已重建"
                )
        except RuntimeError:
            pass
        ...
        async with order_detail_lock:
            logger.info(
                f"🔍 【{self.cookie_id}】获取订单详情锁 {order_id}，开始处理..."
            )

            try:
                logger.info(
                    f"【{self.cookie_id}】开始获取订单详情: {order_id}, sid={sid}"
                )
                ...
                logger.warning(
                    f"【{self.cookie_id}】使用Cookie长度: {len(cookie_string) if cookie_string else 0}"
                )
                ...
                if result:
                    ...
                    if (
                        ...
                        self.order_detail_retry_tasks.pop(order_id, None)
                        logger.info(
                            f"【{self.cookie_id}】订单详情已成功获取，取消待执行的补抓任务: {order_id}"
                        )

                    logger.info(f"【{self.cookie_id}】订单详情获取成功: {order_id}")
                    logger.info(
                        f"【{self.cookie_id}】页面标题: {result.get('title', '未知')}"
                    )
                    ...
                    # 获取解析后的规格信息
                    spec_parse_mode = (
                        str(result.get("spec_parse_mode") or "").strip() or "no_spec"
                    )
                    ...
                    amount_source = (
                        _normalize_optional_text(result.get("amount_source"))
                        or "unknown"
                    )
                    item_config = (
                    ...
                    item_config_multi_spec = bool(
                        item_config and item_config.get("is_multi_spec")
                    )
                    item_config_detail = (
                        _normalize_optional_text(item_config.get("item_detail"))
                        if item_config
                        else None
                    )
                    is_coin_deduction_item = bool(
                        item_config_detail and "闲鱼币抵扣" in item_config_detail
                    )
                    configured_item_amount = (
                        _normalize_amount_text(item_config.get("item_price"))
                        if item_config
                        else None
                    )
                    configured_item_amount_value = _parse_amount_float(
                        configured_item_amount
                    )

                    if (
                        item_config is not None
                        and not item_config_multi_spec
                        and any([spec_name, spec_value, spec_name_2, spec_value_2])
                    ):
                        logger.warning(
                    ...
                    if (
                        spec_parse_mode == "one_spec"
                        and spec_name
                        and spec_value
                        and not (spec_name_2 or spec_value_2)
                    ):
                        spec_name_2 = ""
                        spec_value_2 = ""
                        logger.info(
                    ...
                    # 获取订单状态（从闲鱼页面解析）
                    raw_order_status = _normalize_optional_text(
                        result.get("order_status")
                    )
                    order_status_source = (
                        _normalize_optional_text(result.get("order_status_source"))
                        or "unknown"
                    )
                    # unknown 视为解析失败，不覆盖已有状态
                    order_status = (
                        raw_order_status
                        if raw_order_status and raw_order_status.lower() != "unknown"
                        else None
                    )
                    if order_status:
                        logger.info(
                            f"【{self.cookie_id}】📊 订单状态: {order_status} (source={order_status_source})"
                        )
                    elif raw_order_status and raw_order_status.lower() == "unknown":
                        logger.warning(
                            f"【{self.cookie_id}】订单状态解析为unknown，跳过状态字段写库"
                        )
                    ...
                    if spec_name and spec_value:
                        ...
                        if spec_name_2 and spec_value_2:
                            logger.info(
                                f"【{self.cookie_id}】📋 规格2名称: {spec_name_2}"
                            )
                            logger.info(
                                f"【{self.cookie_id}】📝 规格2值: {spec_value_2}"
                            )
                            print(
                                f"🛍️ 【{self.cookie_id}】订单 {order_id} 规格信息: {spec_name} -> {spec_value}, {spec_name_2} -> {spec_value_2}"
                            )
                        else:
                            print(
                                f"🛍️ 【{self.cookie_id}】订单 {order_id} 规格信息: {spec_name} -> {spec_value}"
                            )
                    else:
                    ...
                    if amount:
                        logger.info(
                            f"【{self.cookie_id}】💰 订单金额: {amount} (source={amount_source})"
                        )
                    ...
                    try:
                        ...
                        existing_amount = (
                            existing_order.get("amount") if existing_order else None
                        )
                        ...
                        if (
                            is_coin_deduction_item
                            and existing_amount_value is not None
                            and incoming_amount_value is not None
                            and configured_item_amount_value is not None
                            and existing_amount_value + 0.009
                            < configured_item_amount_value
                            and abs(
                                incoming_amount_value - configured_item_amount_value
                            )
                            <= 0.009
                        ):
                            logger.warning(
                        ...
                        if (
                            amount
                            and amount_source in low_confidence_amount_sources
                            and not has_valid_spec
                            and not order_status
                        ):
                            ...
                            and existing_amount_value is not None
                            and incoming_amount_value is not None
                            and abs(existing_amount_value - incoming_amount_value)
                            > 0.009
                            and not has_valid_spec
                            and not order_status
                            and amount_source
                            not in {
                                "selector_keyword_high",
                                "selector_keyword_low",
                                "text_keyword_high",
                                "text_keyword_low",
                                "cache",
                            }
                        ...
                        normalized_current_order_status = (
                            db_manager._normalize_order_status(current_order_status)
                        )
                        normalized_incoming_order_status = (
                            db_manager._normalize_order_status(order_status)
                        )
                        ...
                        if (
                            order_status
                            and existing_order
                            and order_status_to_save is None
                            and normalized_current_order_status
                            != normalized_incoming_order_status
                        ):
                            logger.info(
                        ...
                        if not cookie_info:
                            logger.warning(
                                f"Cookie ID {self.cookie_id} 不存在于cookies表中，丢弃订单 {order_id}"
                            )
                        else:
                            ...
                            # 使用订单状态处理器设置状态
                            logger.info(
                                f"【{self.cookie_id}】检查订单状态处理器调用条件: success={success}, handler_exists={self.order_status_handler is not None}"
                            )
                            if success and self.order_status_handler:
                                logger.info(
                                    f"【{self.cookie_id}】准备调用订单状态处理器.handle_order_detail_fetched_status: {order_id}"
                                )
                                try:
                                    ...
                                    logger.info(
                                        f"【{self.cookie_id}】订单状态处理器.handle_order_detail_fetched_status返回结果: {handler_result}"
                                    )

                                    # 处理待处理队列
                                    logger.info(
                                        f"【{self.cookie_id}】准备调用订单状态处理器.on_order_details_fetched: {order_id}"
                                    )
                                    self.order_status_handler.on_order_details_fetched(
                                        order_id
                                    )
                                    logger.info(
                                        f"【{self.cookie_id}】订单状态处理器.on_order_details_fetched调用成功: {order_id}"
                                    )
                                except Exception as e:
                                    logger.error(
                                        f"【{self.cookie_id}】订单状态处理器调用失败: {self._safe_str(e)}"
                                    )
                                    import traceback
                                ...
                                logger.warning(
                                    f"【{self.cookie_id}】订单状态处理器调用条件不满足: success={success}, handler_exists={self.order_status_handler is not None}"
                                )

                            if success:
                                logger.info(
                                    f"【{self.cookie_id}】订单信息已保存到数据库: {order_id}"
                                )
                                print(
                                    f"💾 【{self.cookie_id}】订单 {order_id} 信息已保存到数据库"
                                )
                            else:
                                logger.warning(
                                    f"【{self.cookie_id}】订单信息保存失败: {order_id}"
                                )

                    except Exception as db_e:
                        logger.error(
                            f"【{self.cookie_id}】保存订单信息到数据库失败: {self._safe_str(db_e)}"
                        )
                ...
                logger.error(
                    f"【{self.cookie_id}】获取订单详情异常: {self._safe_str(e)}"
                )
                return None
    ...
    async def _auto_delivery(
        ...
        data_preview_index: int = 0,
        delivery_unit_index: int = 1,
    ):
        ...
        try:
            ...
            def build_result(
                ...
                delivery_steps_value: list = None,
            ):
                ...
                try:
                    rule_spec_mode_value = (
                        _get_rule_spec_mode(matched_rule) if matched_rule else None
                    )
                except Exception:
                    pass

                try:
                    item_config_mode_value = (
                        "spec_enabled" if item_config_multi_spec else "no_spec"
                    )
                except Exception:
                    pass
                ...
                if include_meta:
                    return {
                        ...
                        "rule_keyword": matched_rule.get("keyword")
                        if matched_rule
                        else None,
                        ...
                        "card_id": matched_rule.get("card_id")
                        if matched_rule
                        else None,
                        "card_description": matched_rule.get("card_description")
                        if matched_rule
                        else None,
                ...
                try:
                    ...
                    if db_item_info:
                        ...
                        if not item_detail_db.strip():
                            ...
                            auto_fetch_config = config.get("ITEM_DETAIL", {}).get(
                                "auto_fetch", {}
                            )

                            if auto_fetch_config.get("enabled", True):
                                logger.info(
                                    f"数据库中商品详情为空，尝试自动获取: {item_id}"
                                )
                                try:
                                    fetched_detail = (
                                        await self.fetch_item_detail_from_api(item_id)
                                    )
                                    if fetched_detail:
                                        # 保存获取到的详情
                                        await self.save_item_detail_only(
                                            item_id, fetched_detail
                                        )
                                        item_detail_db = fetched_detail
                                        logger.info(
                                            f"成功获取并保存商品详情: {item_id}"
                                        )
                                    else:
                                        logger.warning(f"未能获取到商品详情: {item_id}")
                                except Exception as api_e:
                                    logger.warning(
                                        f"获取商品详情失败: {item_id}, 错误: {self._safe_str(api_e)}"
                                    )
                            else:
                                logger.warning(
                                    f"自动获取商品详情功能已禁用，跳过: {item_id}"
                                )
                        ...
                        if search_parts:
                            ...
                            logger.info(
                                f"使用数据库商品标题+详情作为搜索文本: 标题='{item_title_db}', 详情长度={len(item_detail_db)}"
                            )
            ...
            item_config_multi_spec = db_manager.get_item_multi_spec_status(
                self.cookie_id, item_id
            )
            spec_name = ""
            spec_value = ""
            ...
            def _get_rule_spec_mode(rule: dict) -> str:
                ...
                if (
                    rule_spec_name
                    and rule_spec_value
                    and rule_spec_name_2
                    and rule_spec_value_2
                ):
                    return "two_spec"
            ...
            if order_id:
                ...
                for attempt in range(1, max_detail_attempts + 1):
                    try:
                        ...
                        if force_refresh:
                            logger.info(
                                f"订单规格信息缺失，开始强刷重试 ({attempt}/{max_detail_attempts}): {order_id}"
                            )

                        order_detail = await self.fetch_order_detail_info(
                            order_id, item_id, send_user_id, force_refresh=force_refresh
                        )
                        ...
                        if _apply_spec_from_order_detail(order_detail):
                            ...
                            if spec_name_2 and spec_value_2:
                                logger.info(
                                    f"获取到规格2信息: {spec_name_2} = {spec_value_2}"
                                )
                            break
                ...
                if _get_order_spec_mode() == "no_spec":
                    try:
                        ...
                        logger.warning(
                            f"订单缓存规格恢复失败: {self._safe_str(cache_e)}"
                        )
            else:
                logger.warning(
                    "当前无订单ID，跳过订单详情拉取，将仅基于商品文本匹配规则"
                )
            ...
            if (
                ...
                return build_result(
                    False, error=block_reason, match_mode_value="blocked_no_spec_parsed"
                )
            ...
            if order_spec_mode == "two_spec":
                ...
                if not delivery_rules:
                    ...
                    return build_result(
                        False, error=error_message, match_mode_value="blocked_no_rule"
                    )
                ...
                if not delivery_rules:
                    ...
                    fallback_rules = db_manager.get_delivery_rules_by_keyword(
                        search_text, user_id=self.user_id, only_non_multi_spec=True
                    )
                    if not fallback_rules:
                        ...
                        return build_result(
                            False,
                            error=error_message,
                            match_mode_value="blocked_no_rule",
                        )
                    if len(fallback_rules) != 1:
                        ...
                        return build_result(
                            False,
                            error=block_reason,
                            match_mode_value="blocked_multiple_no_spec_rules",
                        )
            ...
            if rule_spec_mode != order_spec_mode and not allow_one_spec_fallback:
                ...
                return build_result(
                    ...
                    error=block_reason,
                    matched_rule=rule,
                    match_mode_value="blocked_rule_mode_mismatch",
                )
            ...
            if order_spec_mode == "two_spec":
                ...
                logger.info(
                    f"🎯 精确匹配两组规格发货规则: {rule['keyword']} -> {rule['card_name']} [{rule_spec_info}]"
                )
                logger.info(
                    f"📋 订单规格: {order_spec_info} ✅ 匹配卡券规格: {rule_spec_info}"
                )
                ...
                logger.warning(
                    f"📋 订单规格: {order_spec_info}，精确规格未命中，已降级到普通规则"
                )
            elif order_spec_mode == "one_spec":
                rule_spec_info = f"{rule['spec_name']}:{rule['spec_value']}"
                order_spec_info = f"{spec_name}:{spec_value}"
                logger.info(
                    f"🎯 精确匹配一组规格发货规则: {rule['keyword']} -> {rule['card_name']} [{rule_spec_info}]"
                )
                logger.info(
                    f"📋 订单规格: {order_spec_info} ✅ 匹配卡券规格: {rule_spec_info}"
                )
                ...
                logger.info(
                    f"✅ 匹配无规格发货规则: {rule['keyword']} -> {rule['card_name']} ({rule['card_type']})"
                )
            ...
            if order_id:
                ...
                try:
                    ...
                    if send_user_id and send_user_id == self.myid:
                        logger.info(
                            f"【{self.cookie_id}】跳过买家订单 {order_id}，buyer_id={send_user_id} 等于自己的ID"
                        )
                ...
                # 开始处理发货内容
                logger.info(
                    f"开始处理发货内容，规则: {rule['keyword']} -> {rule['card_name']} ({rule['card_type']})"
                )
                ...
                if rule["card_type"] == "api":
                    ...
                    delivery_content = await self._get_api_card_content(
                        rule, order_id, item_id, send_user_id, spec_name, spec_value
                    )

                elif rule["card_type"] == "yifan_api":
                    # 亦凡卡劵API类型：调用亦凡API获取内容
                    delivery_content = await self._get_yifan_api_card_content(
                        rule, order_id, item_id, send_user_id, chat_id
                    )
                    ...
                    if image_url:
                        delivery_content = (
                            f"__IMAGE_SEND__{rule['card_id']}|{image_url}"
                        )
                        logger.info(
                            f"准备发送图片: {image_url} (卡券ID: {rule['card_id']})"
                        )
                    else:
                ...
                if delivery_content:
                    ...
                    if (
                        len(delivery_steps) == 1
                        and delivery_steps[0].get("type") == "text"
                    ):
                    ...
                    logger.info(
                        f"自动发货内容准备成功: 规则ID={rule['id']}, 步骤数={len(delivery_steps)}"
                    )
                    ...
                    if include_meta and isinstance(result, dict):
                        ...
                        result["data_card_pending_consume"] = bool(
                            rule["card_type"] == "data"
                        )
                        result["data_line"] = data_line
                        result["data_reservation_id"] = (
                            data_reservation.get("id") if data_reservation else None
                        )
                        result["data_reservation_status"] = (
                            data_reservation.get("status") if data_reservation else None
                        )
                ...
                # 没有订单ID，记录日志但不处理发货内容
                logger.info(
                    f"⚠️ 未检测到订单ID，跳过发货内容处理。规则: {rule['keyword']} -> {rule['card_name']} ({rule['card_type']})"
                )
                return build_result(
                    False,
                    error="未检测到订单ID，跳过发货内容处理",
                    matched_rule=rule,
                    match_mode_value=match_mode,
                )
            ...
            if matched_rule_context:
                rule_label = (
                    matched_rule_context.get("keyword")
                    or f"规则ID={matched_rule_context.get('id')}"
                )
                card_type = matched_rule_context.get("card_type") or "unknown"
                error_message = (
                    f"规则已命中({rule_label})，但{card_type}发货处理异常: {error_text}"
                )
            else:
    ...
    def _process_delivery_content_with_description(
        self, delivery_content: str, card_description: str
    ) -> str:
        """处理发货内容和备注信息，实现变量替换"""
        try:
            # 如果没有备注信息，直接返回发货内容
            ...
            # 替换备注中的变量
            processed_description = card_description.replace(
    ...
    def _build_delivery_steps(self, delivery_content: str, card_description: str):
        ...
        try:
            raw_content = (
                delivery_content
                if isinstance(delivery_content, str)
                else str(delivery_content or "")
            )
            description = (card_description or "").strip()
            steps = []

            if raw_content and not raw_content.startswith("__IMAGE_SEND__"):
                final_text = self._process_delivery_content_with_description(
            ...
            if fallback_content:
                return [
                    {
                        "type": "image"
                        if fallback_content.startswith("__IMAGE_SEND__")
                        else "text",
    ...
    def _format_delivery_unit_text(
        self, text: str, unit_index: int, total_units: int
    ) -> str:
        """为批量发货文本添加全局连续序号。"""
    ...
    def _apply_delivery_unit_numbering(
        self, delivery_steps, unit_index: int, total_units: int, card_type: str = None
    ):
    ...
    def _build_delivery_send_groups(
        self,
        prepared_units,
        total_units: int,
        ...
        if max(1, int(total_units or 1)) <= 1:
            return [
                ...
                for prepared_unit in sorted(
                    prepared_units or [],
                    key=lambda unit: int(unit.get("unit_index") or 0),
                )
            ]
        ...
        def flush_current_batch():
            ...
            batched_text = "\n\n".join(
                unit["batched_text"] for unit in current_batch_units
            )
            groups.append(
                {
                    "mode": "batched_text",
        ...
        for prepared_unit in sorted(
            prepared_units or [], key=lambda unit: int(unit.get("unit_index") or 0)
        ):
            delivery_steps = prepared_unit.get("delivery_steps") or []
            ...
            exceeds_char_limit = current_batch_units and (
                current_batch_chars + separator_chars + len(numbered_text)
            ...
            current_batch_chars += (2 if len(current_batch_units) > 1 else 0) + len(
                numbered_text
            )
    ...
    async def _send_delivery_steps(
        self,
        websocket,
        chat_id: str,
        user_id: str,
        ...
        for index, step in enumerate(steps, start=1):
            ...
            if step_type == "image":
                ...
                await self.send_image_msg(
                    websocket, chat_id, user_id, image_url, card_id=image_card_id
                )
                logger.info(
    ...
    async def _get_api_card_content(
        self,
        rule,
        order_id=None,
        item_id=None,
        ...
        try:
            ...
            if not api_config:
                logger.error(
                    f"API配置为空，规则ID: {rule.get('id')}, 卡券名称: {rule.get('card_name')}"
                )
            ...
            if method == "POST" and params:
                params = await self._replace_api_dynamic_params(
                    params, order_id, item_id, buyer_id, spec_name, spec_value
                )
            ...
            if method == "POST" and params:
                logger.warning(
                    f"POST请求参数: {json.dumps(params, ensure_ascii=False)}"
                )
            ...
            if method == "GET":
                async with self.session.get(
                    url, headers=headers, params=params, timeout=timeout_obj
                ) as response:
                    status_code = response.status
                    response_text = await response.text()
            elif method == "POST":
                async with self.session.post(
                    url, headers=headers, json=params, timeout=timeout_obj
                ) as response:
                    status_code = response.status
                    response_text = await response.text()
            else:
            ...
            if status_code == 200:
                ...
                if status_code >= 500 or status_code == 408:
                    if retry_count < max_retries - 1:
                        ...
                        return await self._get_api_card_content(
                            rule,
                            order_id,
                            item_id,
                            buyer_id,
            ...
            if retry_count < max_retries - 1:
                ...
                logger.error(
                    f"API调用网络异常，已达到最大重试次数: {self._safe_str(e)}"
                )
                return None
    ...
    async def _get_yifan_api_card_content(
        self, rule, order_id=None, item_id=None, buyer_id=None, chat_id=None
    ):
        """调用亦凡卡劵API获取内容"""
        try:
            import hashlib
            ...
            if not api_config:
                logger.error(
                    f"亦凡API配置为空，规则ID: {rule.get('id')}, 卡券名称: {rule.get('card_name')}"
                )
                return None
            ...
            if require_account:
                ...
                recharge_account = await self._ask_for_recharge_account(
            ...
            logger.info(
                f"调用亦凡API: 商户ID={user_id}, 商品ID={goods_id}, 充值账号={recharge_account}, 回调URL={callback_url if callback_url else '无'}"
            )
            ...
            async with self.session.post(
                api_url, data=params, timeout=timeout_obj
            ) as response:
                status_code = response.status
                response_text = await response.text()
                ...
                if status_code == 200:
                        ...
                        if result.get("code") == 1:
                            ...
                            if order_id and order_no:
                                try:
                                    ...
                                    if chat_id:
                                        db_manager.update_order_chat_id(
                                            order_id, chat_id
                                        )
                                    logger.info(
                                        f"已记录亦凡订单信息: order_id={order_id}, yifan_orderno={order_no}"
                                    )
                                except Exception as e:
                            ...
                            logger.error(
                                f"亦凡API调用失败: code={result.get('code')}, msg={error_msg}"
                            )
                    ...
                    logger.error(
                        f"亦凡API调用失败: HTTP {status_code} - {response_text[:200]}"
                    )
                    return None
    ...
    async def _call_yifan_api_with_account(
        self, rule, account, order_id=None, item_id=None, buyer_id=None, chat_id=None
    ):
        """使用确认的账号调用亦凡API"""
        try:
            import hashlib
            ...
            logger.info(
                f"调用亦凡API: 商户ID={user_id}, 商品ID={goods_id}, 充值账号={account}, 回调URL={callback_url if callback_url else '无'}"
            )
    ...
    async def _ask_for_recharge_account(
        self, chat_id, buyer_id, rule, order_id=None, item_id=None
    ):
        """询问客户充值账号并设置等待状态（不阻塞）"""
        try:
    ...
    async def _replace_api_dynamic_params(
        self,
        params,
        order_id=None,
        item_id=None,
        ...
        try:
            while True:
                try:
                    ...
                    if cookie_manager and not cookie_manager.get_cookie_status(
                        self.cookie_id
                    ):
                        logger.info(
                            f"【{self.cookie_id}】账号已禁用，停止Token刷新循环"
                        )
                        break
                    ...
                    if (
                        ...
                        if new_token:
                            logger.info(
                                f"【{self.cookie_id}】Token刷新成功，将关闭WebSocket以使用新Token重连"
                            )
                            ...
                            if self.ws and not self.ws.closed:
                                try:
                                    logger.info(
                                        f"【{self.cookie_id}】关闭当前WebSocket连接以使用新Token重连..."
                                    )
                                    await self.ws.close()
                                    logger.info(
                                        f"【{self.cookie_id}】WebSocket连接已关闭，将自动重连"
                                    )
                                except Exception as close_e:
                                    logger.warning(
                                        f"【{self.cookie_id}】关闭WebSocket时出错: {self._safe_str(close_e)}"
                                    )

                            # 退出Token刷新循环，让main循环重新建立连接
                            # 后台任务（心跳、清理等）继续运行
                            logger.info(
                                f"【{self.cookie_id}】Token刷新完成，WebSocket将使用新Token重新连接"
                            )
                            break
                        else:
                            # 根据上一次刷新状态决定日志级别（冷却/已重启为正常情况）
                            last_refresh_status = getattr(
                                self, "last_token_refresh_status", None
                            )
                            benign_refresh_statuses = (
                                "skipped_cooldown",
                                "restarted_after_cookie_refresh",
                            )
                            ...
                            if last_refresh_status in benign_refresh_statuses:
                                ...
                                logger.error(
                                    f"【{self.cookie_id}】Token刷新失败，将在{self.token_retry_interval // 60}分钟后重试"
                                )
                    ...
                    # 收到取消信号，立即退出循环
                    logger.info(
                        f"【{self.cookie_id}】Token刷新循环收到取消信号，准备退出"
                    )
                    raise
                except Exception as e:
                    ...
                    try:
                        ...
                        logger.info(
                            f"【{self.cookie_id}】Token刷新循环在重试等待时收到取消信号，准备退出"
                        )
                        raise
        except asyncio.CancelledError:
    ...
    async def init(self, ws):
        ...
        if not self.current_token:
            ...
            if not token_refresh_attempted:
                await self.send_token_refresh_notification(
                    "初始化时无法获取有效Token", "token_init_failed"
                )
            else:
                logger.info(
                    f"【{self.cookie_id}】由于刚刚尝试过token刷新，跳过重复的初始化失败通知"
                )
            raise InitAuthError(
                f"Token获取失败(status={self.last_init_failure_reason})"
            )
    ...
    async def heartbeat_loop(self, ws, heartbeat_generation: Optional[int] = None):
        """心跳循环"""
        consecutive_failures = 0
        max_failures = 3  # 连续失败3次后停止心跳
        if heartbeat_generation is None:
            heartbeat_generation = getattr(self, "_heartbeat_generation", 0)

        try:
            while True:
                try:
                    if heartbeat_generation != getattr(
                        self, "_heartbeat_generation", 0
                    ):
                        logger.info(
                            f"【{self.cookie_id}】检测到旧心跳任务世代 {heartbeat_generation} 已失效，停止当前循环"
                        )
                        break
                    ...
                    if ws.closed:
                        logger.warning(
                            f"【{self.cookie_id}】WebSocket连接已关闭，停止心跳循环"
                        )
                        break
                    ...
                    logger.error(
                        f"心跳发送失败 ({consecutive_failures}/{max_failures}): {self._safe_str(e)}"
                    )

                    if consecutive_failures >= max_failures:
                        logger.error(
                            f"【{self.cookie_id}】心跳连续失败{max_failures}次，停止心跳循环"
                        )
                        break
                    ...
                    try:
                        ...
                        # 在等待重试时收到取消信号，立即退出
                        logger.info(
                            f"【{self.cookie_id}】心跳循环在重试等待时收到取消信号，准备退出"
                        )
                        raise
        except asyncio.CancelledError:
            # 确保CancelledError被正确传播
            ...
            while True:
                try:
                    ...
                    try:
                        ...
                        if cleaned_count > 0:
                            logger.info(
                                f"【{self.cookie_id}】清理了 {cleaned_count} 个过期的商品详情缓存"
                            )
                    except asyncio.CancelledError:
                        raise
                    except Exception as cache_clean_e:
                        logger.warning(
                            f"【{self.cookie_id}】清理商品详情缓存时出错: {cache_clean_e}"
                        )
                    ...
                    try:
                        ...
                        logger.warning(
                            f"【{self.cookie_id}】清理QR登录会话时出错: {qr_clean_e}"
                        )
                    ...
                    try:
                        ...
                        logger.warning(
                            f"【{self.cookie_id}】清理Playwright缓存时出错: {pw_clean_e}"
                        )
                    ...
                    try:
                        ...
                        logger.warning(
                            f"【{self.cookie_id}】清理日志文件时出错: {log_clean_e}"
                        )
                    ...
                    try:
                        if hasattr(self.__class__, "_last_risk_log_cleanup_time"):
                            last_risk_cleanup = (
                                self.__class__._last_risk_log_cleanup_time
                            )
                        else:
                        ...
                        if current_time - last_risk_cleanup > 600:
                            try:
                                ...
                                if cleaned_count > 0:
                                    logger.warning(
                                        f"【{self.cookie_id}】风控日志超时兜底清理完成，自动关闭 {cleaned_count} 条processing记录"
                                    )
                                self.__class__._last_risk_log_cleanup_time = (
                                    current_time
                                )
                                ...
                                logger.warning(
                                    f"【{self.cookie_id}】风控日志超时兜底清理被取消"
                                )
                                raise
                    except asyncio.CancelledError:
                        raise
                        ...
                        logger.error(
                            f"【{self.cookie_id}】清理超时风控日志时出错: {risk_clean_e}"
                        )
                    ...
                    try:
                        ...
                        if current_time - last_cleanup > 86400:
                            logger.info(
                                f"【{self.cookie_id}】开始执行数据库历史数据清理..."
                            )
                            ...
                            try:
                                ...
                                if "error" not in stats:
                                    logger.info(
                                        f"【{self.cookie_id}】数据库清理完成: {stats}"
                                    )
                                    self.__class__._last_db_cleanup_time = current_time
                                else:
                                    logger.error(
                                        f"【{self.cookie_id}】数据库清理失败: {stats['error']}"
                                    )
                            except asyncio.CancelledError:
                        ...
                        logger.error(
                            f"【{self.cookie_id}】清理数据库历史数据时出错: {db_clean_e}"
                        )
                    ...
                    logger.error(
                        f"【{self.cookie_id}】清理任务失败: {self._safe_str(e)}"
                    )
                    ...
                    try:
                        ...
                        logger.info(
                            f"【{self.cookie_id}】清理循环在重试等待时收到取消信号，准备退出"
                        )
                        raise
        except asyncio.CancelledError:
            # 确保CancelledError被正确传播
            ...
            while True:
                try:
                    ...
                    if cookie_manager and not cookie_manager.get_cookie_status(
                        ...
                        logger.info(
                            f"【{self.cookie_id}】账号已禁用，停止Cookie刷新循环"
                        )
                        break
                    ...
                    if not self.cookie_refresh_enabled:
                        logger.warning(
                            f"【{self.cookie_id}】Cookie刷新功能已禁用，跳过执行"
                        )
                    ...
                    if self.is_manual_refresh_active(self.cookie_id):
                        logger.warning(
                            f"【{self.cookie_id}】手动刷新进行中，跳过自动Cookie刷新"
                        )
                        await self._interruptible_sleep(60)
                        continue
                    ...
                    current_time = time.time()
                    if (
                        current_time - self.last_cookie_refresh_time
                        >= self.cookie_refresh_interval
                    ):
                        # 检查是否在消息接收后的冷却时间内
                        time_since_last_message = (
                            current_time - self.last_message_received_time
                        )
                        if (
                            time_since_last_message
                            < self.message_cookie_refresh_cooldown
                        ):
                            remaining_time = (
                                self.message_cookie_refresh_cooldown
                                - time_since_last_message
                            ...
                            remaining_seconds = int(remaining_time % 60)
                            logger.warning(
                                f"【{self.cookie_id}】收到消息后冷却中，还需等待 {remaining_minutes}分{remaining_seconds}秒 才能执行Cookie刷新"
                            )
                        # 检查是否已有Cookie刷新任务在执行
                        elif self.cookie_refresh_lock.locked():
                            logger.warning(
                                f"【{self.cookie_id}】Cookie刷新任务已在执行中，跳过本次触发"
                            )
                        else:
                            logger.info(
                                f"【{self.cookie_id}】开始执行Cookie刷新任务..."
                            )
                    ...
                    # 收到取消信号，立即退出循环
                    logger.info(
                        f"【{self.cookie_id}】Cookie刷新循环收到取消信号，准备退出"
                    )
                    raise
                except Exception as e:
                    logger.error(
                        f"【{self.cookie_id}】Cookie刷新循环失败: {self._safe_str(e)}"
                    )
                    # 出错后也等待1分钟再重试，使用可中断的sleep
                    try:
                        await self._interruptible_sleep(60)
                        ...
                        logger.info(
                            f"【{self.cookie_id}】Cookie刷新循环在重试等待时收到取消信号，准备退出"
                        )
                        raise
        except asyncio.CancelledError:
            # 确保CancelledError被正确传播
    ...
    async def _execute_cookie_refresh(self, current_time):
        ...
        async with self.cookie_refresh_lock:
            try:
                ...
                if self.is_manual_refresh_active(self.cookie_id):
                    logger.warning(
                        f"【{self.cookie_id}】手动刷新进行中，取消当前自动Cookie刷新任务"
                    )
                    return

                logger.info(
                    f"【{self.cookie_id}】开始Cookie刷新任务，暂时暂停心跳以避免连接冲突..."
                )
                ...
                if self.heartbeat_task and not self.heartbeat_task.done():
                    ...
                    self._invalidate_heartbeat_generation("Cookie刷新前暂停心跳")
                    self.heartbeat_task.cancel()
                ...
                if heartbeat_was_running and self.ws and not self.ws.closed:
                    ...
                    await self._start_heartbeat_task(
                        self.ws, reason="Cookie刷新成功后恢复心跳"
                    )
                ...
                if success:
                    ...
                    try:
                        ...
                        if not validation_result["valid"]:
                            logger.warning(
                                f"【{self.cookie_id}】❌ Cookie验证失败: {validation_result['details']}"
                            )
                            if validation_result.get("relogin_recommended", True):
                                logger.warning(
                                    f"【{self.cookie_id}】检测到Cookie可能无法用于关键API，尝试通过密码登录重新获取..."
                                )

                                # 触发密码登录刷新
                                password_refresh_success = (
                                    await self._try_password_login_refresh(
                                        "Cookie验证失败(关键API不可用)"
                                    )
                                )
                                ...
                                if password_refresh_success:
                                    logger.info(
                                        f"【{self.cookie_id}】✅ 密码登录刷新成功，Cookie已更新"
                                    )
                                    clear_message_received_flag = True
                                else:
                                    logger.warning(
                                        f"【{self.cookie_id}】⚠️ 密码登录刷新失败，Cookie可能仍然无效"
                                    )
                                    # 发送通知
                                    await self.send_token_refresh_notification(
                                ...
                                logger.warning(
                                    f"【{self.cookie_id}】Cookie验证失败，但当前错误更像网络/环境问题，跳过密码登录刷新"
                                )
                        else:
                            if validation_result.get("inconclusive"):
                                logger.warning(
                                    f"【{self.cookie_id}】⚠️ Cookie验证结果不确定，保留当前消息冷却标志，等待后续保活再次确认: {validation_result['details']}"
                                )
                            else:
                                logger.info(
                                    f"【{self.cookie_id}】✅ Cookie验证通过: {validation_result['details']}"
                                )
                                clear_message_received_flag = True

                    except Exception as verify_e:
                        logger.error(
                            f"【{self.cookie_id}】Cookie验证过程异常: {self._safe_str(verify_e)}"
                        )
                        import traceback

                        logger.error(
                            f"【{self.cookie_id}】详细堆栈:\n{traceback.format_exc()}"
                        )
                else:
                ...
                logger.error(
                    f"【{self.cookie_id}】执行Cookie刷新任务异常: {self._safe_str(e)}"
                )
                # 异常也要更新时间，避免频繁重试
                self.last_cookie_refresh_time = current_time
            finally:
                ...
                if (
                    self.ws
                    and not self.ws.closed
                    and (not self.heartbeat_task or self.heartbeat_task.done())
                ):
                    logger.info(f"【{self.cookie_id}】Cookie刷新完成，心跳任务正常运行")
                    await self._start_heartbeat_task(
                        self.ws, reason="Cookie刷新finally兜底恢复心跳"
                    )
                ...
                if clear_message_received_flag:
                    ...
                    self.last_message_received_time = 0
                    logger.warning(
                        f"【{self.cookie_id}】Cookie刷新完成，已清空消息接收标志"
                    )
                else:
                    logger.warning(
                        f"【{self.cookie_id}】Cookie刷新未确认恢复可用，保留消息接收标志"
                    )
    ...
    async def refresh_cookies_from_qr_login(
        self, qr_cookies_str: str, cookie_id: str = None, user_id: int = None
    ):
        ...
        try:
            ...
            logger.info(
                f"【{target_cookie_id}】开始使用扫码登录cookie获取真实cookie..."
            )
            ...
            for i, cookie in enumerate(cookies, 1):
                logger.info(
                    f"【{target_cookie_id}】{i:2d}. {cookie['name']}: {cookie['value'][:50]}{'...' if len(cookie['value']) > 50 else ''}"
                )
            ...
            try:
                ...
                if "timeout" in str(e).lower():
                    logger.warning(
                        f"【{target_cookie_id}】页面访问超时，尝试降级策略..."
                    )
                    try:
                        # 降级策略：只等待基本加载
                        ...
                        logger.warning(
                            f"【{target_cookie_id}】降级策略也失败，尝试最基本访问..."
                        )
                        # 最后尝试：不等待任何加载完成
            ...
            try:
                ...
                if "timeout" in str(e).lower():
                    logger.warning(
                        f"【{target_cookie_id}】页面刷新超时，使用降级策略..."
                    )
            ...
            if existing_cookie_value:
                try:
                    ...
                    logger.warning(
                        f"【{target_cookie_id}】解析现有账号Cookie失败，按空基线继续: {self._safe_str(merge_e)}"
                    )

            merge_result = self.protected_merge_cookie_dicts(
                existing_cookies_dict, real_cookies_dict
            )
            real_cookies_dict = merge_result["merged_cookies_dict"]
            if target_cookie_id == self.cookie_id:
                self._log_protected_merge_event(
                    "qr_login_protected_merge", merge_result
                )
            else:
                logger.info(
            ...
            if merge_result["updated_fields"]:
                logger.info(
                    f"【{target_cookie_id}】扫码登录合并更新Cookie字段: {', '.join(merge_result['updated_fields'])}"
                )
            if merge_result["preserved_fields"]:
                logger.info(
                    f"【{target_cookie_id}】扫码登录保留现有Cookie字段 ({len(merge_result['preserved_fields'])}个): {', '.join(merge_result['preserved_fields'])}"
                )
            if merge_result["preserved_protected_fields"]:
                logger.warning(
                    f"【{target_cookie_id}】扫码登录保护性保留关键字段: {', '.join(merge_result['preserved_protected_fields'])}"
                )
            if merge_result["account_switched"]:
                logger.warning(
                    f"【{target_cookie_id}】扫码登录检测到unb变化，按账号切换处理，不保留旧账号Cookie字段"
                )

            missing_required_fields = merge_result["missing_required_fields"]
            if missing_required_fields:
                logger.error(
                    f"【{target_cookie_id}】扫码登录真实Cookie仍缺失核心字段，放弃保存: {', '.join(missing_required_fields)}"
                )
                return False

            # 生成真实cookie字符串
            real_cookies_str = "; ".join(
                [f"{k}={v}" for k, v in real_cookies_dict.items()]
            )

            logger.info(
                f"【{target_cookie_id}】真实Cookie已获取，包含 {len(real_cookies_dict)} 个字段"
            )

            # 打印扫码登录获取的真实Cookie字段详情
            logger.info(
                f"【{target_cookie_id}】========== 扫码登录真实Cookie字段详情 =========="
            )
            ...
            for i, (key, value) in enumerate(real_cookies_dict.items(), 1):
                if len(str(value)) > 50:
                    logger.info(
                        f"【{target_cookie_id}】  {i:2d}. {key}: {str(value)[:30]}...{str(value)[-20:]} (长度: {len(str(value))})"
                    )
                else:
            ...
            for key in important_keys:
                if key in real_cookies_dict:
                    ...
                    logger.info(
                        f"【{target_cookie_id}】  ✅ {key}: {'存在' if val else '为空'} (长度: {len(str(val)) if val else 0})"
                    )
                else:
            ...
            logger.info(
                f"【{target_cookie_id}】Cookie字符串长度: {len(real_cookies_str)}"
            )
            logger.info(
                f"【{target_cookie_id}】Cookie摘要: {self._summarize_cookie_string(real_cookies_str)}"
            )
            ...
            logger.info(
                f"【{target_cookie_id}】真实Cookie长度: {len(real_cookies_str)}"
            )
            logger.info(
                f"【{target_cookie_id}】真实Cookie字段数: {len(real_cookies_dict)}"
            )
            logger.info(
                f"【{target_cookie_id}】长度增加: {len(real_cookies_str) - len(qr_cookies_str)} 字符"
            )
            logger.info(
                f"【{target_cookie_id}】字段增加: {len(real_cookies_dict) - len(qr_cookies_dict)} 个"
            )
            ...
            if changed_cookies:
                logger.info(
                    f"【{target_cookie_id}】发生变化的Cookie字段 ({len(changed_cookies)}个): {', '.join(changed_cookies)}"
                )
            if new_cookies:
                logger.info(
                    f"【{target_cookie_id}】新增的Cookie字段 ({len(new_cookies)}个): {', '.join(new_cookies)}"
                )
            ...
            for cookie_name in important_cookies:
                if cookie_name in real_cookies_dict:
                    ...
                    change_mark = (
                        " [已变化]"
                        if cookie_name in changed_cookies
                        else " [新增]"
                    ...
                    logger.info(
                        f"【{target_cookie_id}】  值: {self._mask_secret_value(cookie_value, head=8, tail=6)}"
                    )
                    ...
                    if cookie_name in qr_cookies_dict:
                        ...
                        if old_value != cookie_value:
                            logger.info(
                                f"【{target_cookie_id}】  原值: {self._mask_secret_value(old_value, head=8, tail=6)}"
                            )
                            logger.info(
                                f"【{target_cookie_id}】  原长度: {len(old_value)}"
                            )
            ...
            if existing_cookie:
                ...
                success = db_manager.update_cookie_account_info(
                    target_cookie_id, cookie_value=real_cookies_str
                )
            else:
                # 新账号，使用 save_cookie
                success = db_manager.save_cookie(
                    target_cookie_id, real_cookies_str, target_user_id
                )
            ...
            if success:
                ...
                logger.info(
                    f"【{target_cookie_id}】已更新扫码登录Cookie刷新时间标志，_refresh_cookies_via_browser将等待{self.qr_cookie_refresh_cooldown // 60}分钟后执行"
                )
            ...
            logger.error(
                f"【{target_cookie_id}】使用扫码cookie获取真实cookie失败: {self._safe_str(e)}"
            )
            return False
        finally:
            # 确保资源清理
            try:
                ...
                if browser:
                    try:
                        ...
                        logger.warning(
                            f"【{target_cookie_id}】浏览器关闭超时（5秒），资源可能未完全释放"
                        )
                        # 尝试取消浏览器相关的任务
                        try:
                        ...
                        logger.warning(
                            f"【{target_cookie_id}】关闭浏览器时出错: {self._safe_str(e)}"
                        )
                ...
                if playwright:
                    try:
                        ...
                        logger.warning(
                            f"【{target_cookie_id}】Playwright关闭超时（2秒），进程可能仍在运行"
                        )
                        logger.warning(
                            f"【{target_cookie_id}】提示：如果后续Playwright启动失败，可能需要手动清理残留进程"
                        )
                        # 尝试清理Playwright的内部状态
                        try:
                        ...
                        logger.warning(
                            f"【{target_cookie_id}】关闭Playwright时出错: {self._safe_str(e)}"
                        )
            except Exception as cleanup_e:
                logger.warning(
                    f"【{target_cookie_id}】清理浏览器资源时出错: {self._safe_str(cleanup_e)}"
                )

    async def _refresh_cookies_via_browser_page(
        self, current_cookies_str: str, restart_on_success: bool = True
    ):
        ...
        try:
            ...
            logger.info(
                f"【{self.cookie_id}】开始使用当前cookie访问指定页面获取真实cookie..."
            )
            logger.info(
                f"【{self.cookie_id}】当前cookie长度: {len(current_cookies_str)}"
            )

            # 解析当前的cookie
            current_cookies_dict = trans_cookies(current_cookies_str)
            logger.info(
                f"【{self.cookie_id}】当前cookie字段数: {len(current_cookies_dict)}"
            )
            ...
            try:
                ...
                if "timeout" in str(e).lower():
                    ...
                    try:
                        ...
                        logger.warning(
                            f"【{self.cookie_id}】降级策略也失败，尝试最基本访问..."
                        )
                        # 最后尝试：不等待任何加载完成
                        await page.goto(target_url, timeout=25000)
            ...
            merge_result = self.protected_merge_cookie_dicts(
                current_cookies_dict, real_cookies_dict
            )
            real_cookies_dict = merge_result["merged_cookies_dict"]
            self._log_protected_merge_event(
                "browser_stabilization_protected_merge", merge_result
            )
            ...
            logger.info(
                f"【{self.cookie_id}】真实Cookie已获取，包含 {len(real_cookies_dict)} 个字段"
            )
            logger.info(
                f"【{self.cookie_id}】真实Cookie摘要: {self._summarize_cookie_string(real_cookies_str)}"
            )
            ...
            if merge_result["missing_required_fields"]:
                logger.error(
                    f"【{self.cookie_id}】浏览器稳定化后的Cookie仍缺失核心字段，放弃写回数据库: {', '.join(merge_result['missing_required_fields'])}"
                )
                return False
            ...
            if not changed_cookies and not new_cookies:
                if restart_on_success:
                    logger.warning(
                        f"【{self.cookie_id}】Cookie无变化，可能当前cookie已失效"
                    )
                    return False
                logger.info(
                    f"【{self.cookie_id}】Cookie字段无变化，但浏览器稳定化访问已完成"
                )

            logger.info(
                f"【{self.cookie_id}】发生变化的Cookie字段 ({len(changed_cookies)}个): {', '.join(changed_cookies[:10])}"
            )
            if new_cookies:
                logger.info(
                    f"【{self.cookie_id}】新增的Cookie字段 ({len(new_cookies)}个): {', '.join(new_cookies[:10])}"
                )
            ...
            if restart_on_success:
                ...
                if update_success:
                    logger.info(
                        f"【{self.cookie_id}】通过访问指定页面成功更新Cookie并重启任务"
                    )
                    return True
                else:
            ...
            try:
                ...
                await self.update_config_cookies()
                logger.info(
                    f"【{self.cookie_id}】通过访问指定页面成功稳定当前Cookie（不重启任务）"
                )
                return True
            except Exception as update_e:
                self.cookies_str = old_cookies_str
                self.cookies = old_cookies_dict
                logger.error(
                    f"【{self.cookie_id}】稳定Cookie时更新数据库失败: {self._safe_str(update_e)}"
                )
                return False

        except Exception as e:
            logger.error(
                f"【{self.cookie_id}】使用当前cookie访问指定页面获取真实cookie失败: {self._safe_str(e)}"
            )
            return False
        finally:
            # 确保资源清理
            try:
                ...
                if browser:
                    try:
                        ...
                        logger.warning(
                            f"【{self.cookie_id}】浏览器关闭超时（5秒），资源可能未完全释放"
                        )
                    except Exception as e:
                        logger.warning(
                            f"【{self.cookie_id}】关闭浏览器时出错: {self._safe_str(e)}"
                        )
                ...
                if playwright:
                    try:
                        ...
                        logger.warning(
                            f"【{self.cookie_id}】Playwright关闭超时（2秒），进程可能仍在运行"
                        )
                    except Exception as e:
                        logger.warning(
                            f"【{self.cookie_id}】关闭Playwright时出错: {self._safe_str(e)}"
                        )
            except Exception as cleanup_e:
                logger.warning(
                    f"【{self.cookie_id}】清理浏览器资源时出错: {self._safe_str(cleanup_e)}"
                )
    ...
    async def _refresh_cookies_via_browser(
        self, triggered_by_refresh_token: bool = False
    ):
        """通过浏览器访问指定页面刷新Cookie
        ...
        try:
            ...
            if time_since_qr_refresh < self.qr_cookie_refresh_cooldown:
                ...
                logger.info(
                    f"【{self.cookie_id}】扫码登录Cookie刷新冷却中，还需等待 {remaining_minutes}分{remaining_seconds}秒"
                )
            ...
            logger.info(
                f"【{self.cookie_id}】刷新前Cookie长度: {len(self.cookies_str)}"
            )
            ...
            try:
                ...
                if "timeout" in str(e).lower():
                    logger.warning(
                        f"【{self.cookie_id}】第一次刷新超时，使用降级策略..."
                    )
                    await page.reload(wait_until="load", timeout=15000)
            ...
            try:
                ...
                if "timeout" in str(e).lower():
                    logger.warning(
                        f"【{self.cookie_id}】第二次刷新超时，使用降级策略..."
                    )
                    await page.reload(wait_until="load", timeout=15000)
            ...
            merged_cookies_dict = merge_result["merged_cookies_dict"]
            self._log_protected_merge_event(
                "browser_refresh_protected_merge", merge_result
            )
            ...
            logger.info(
                f"【{self.cookie_id}】Cookie已更新，包含 {len(new_cookies_dict)} 个字段"
            )

            # 显示Cookie变化统计
            if changed_cookies:
                logger.info(
                    f"【{self.cookie_id}】发生变化的Cookie字段 ({len(changed_cookies)}个): {', '.join(changed_cookies)}"
                )
            if new_cookies:
                logger.info(
                    f"【{self.cookie_id}】新增的Cookie字段 ({len(new_cookies)}个): {', '.join(new_cookies)}"
                )
            if not changed_cookies and not new_cookies:
            ...
            logger.info(
                f"【{self.cookie_id}】更新后的Cookie摘要: {self._summarize_cookie_string(self.cookies_str)}"
            )
            ...
            for cookie_name in important_cookies:
                if cookie_name in new_cookies_dict:
                    ...
                    logger.info(
                        f"【{self.cookie_id}】  {cookie_name}: {display_value}{change_mark}"
                    )
            ...
            if triggered_by_refresh_token:
                ...
                logger.info(
                    f"【{self.cookie_id}】由refresh_token触发，浏览器Cookie刷新成功标志已设置为True"
                )
                ...
                try:
                    ...
                    logger.info(
                        f"【{self.cookie_id}】Cookie刷新成功，准备重启实例...(via _refresh_cookies_via_browser)"
                    )
                    await self._restart_instance()
                    ...
                    # 不要等待或执行耗时操作
                    logger.info(
                        f"【{self.cookie_id}】重启请求已触发(via _refresh_cookies_via_browser)"
                    )
                    ...
                    logger.error(
                        f"【{self.cookie_id}】兜底重启失败: {self._safe_str(e)}"
                    )
            else:
                logger.info(
                    f"【{self.cookie_id}】由定时任务触发，不设置浏览器Cookie刷新成功标志"
                )
            ...
            logger.error(
                f"【{self.cookie_id}】通过浏览器刷新Cookie失败: {self._safe_str(e)}"
            )
            return False
        finally:
            ...
            try:
                if browser or playwright:
                    ...
                    try:
                        ...
                        logger.warning(
                            f"【{self.cookie_id}】浏览器关闭任务超时（15秒），强制继续"
                        )
                        # 取消任务，避免资源泄漏
                        ...
                        logger.warning(
                            f"【{self.cookie_id}】等待浏览器关闭任务时出错: {self._safe_str(wait_e)}"
                        )
                        # 确保任务被取消
                ...
                logger.warning(
                    f"【{self.cookie_id}】创建浏览器关闭任务时出错: {self._safe_str(cleanup_e)}"
                )
                # 如果创建任务失败，尝试直接关闭
                if browser or playwright:
                    try:
    ...
    async def _async_close_browser(self, browser, playwright):
        ...
        try:
            ...
            await asyncio.wait_for(
                self._normal_close_resources(browser, playwright), timeout=10.0
            )
            ...
            logger.warning(
                f"【{self.cookie_id}】异步关闭时出错，强制关闭: {self._safe_str(e)}"
            )
            await self._force_close_resources(browser, playwright)
        ...
        try:
            ...
            if playwright:
                try:
                    ...
                    logger.warning(
                        f"【{self.cookie_id}】Playwright关闭超时，将自动清理"
                    )
            ...
            if force_tasks:
                ...
                for i, result in enumerate(results):
                    if isinstance(result, (asyncio.TimeoutError, Exception)):
                        ...
                        logger.warning(
                            f"【{self.cookie_id}】{resource_name}强制关闭失败，尝试直接清理连接"
                        )
                        try:
    ...
    async def send_msg_once(self, toid, item_id, text):
        ...
        logger.info(
            f"【{self.cookie_id}】开始单次发送消息: toid={toid}, item_id={item_id}"
        )
        ...
        try:
            ...
            if "extra_headers" in error_msg:
                ...
                # 使用兼容模式
                async with websockets.connect(
                    self.base_url, additional_headers=headers, close_timeout=5
                ) as websocket:
                    result = await self._handle_websocket_connection(
                        websocket, toid, item_id, text
                    )
            ...
            logger.warning(
                f"【{self.cookie_id}】WebSocket连接关闭: {self._safe_str(e)}"
            )
            # 连接关闭但消息可能已发送，不抛出异常
        except Exception as e:
    ...
    async def send_delivery_steps_once(self, toid, item_id, delivery_steps):
        ...
        logger.info(
            f"【{self.cookie_id}】开始单次发送发货步骤: toid={toid}, item_id={item_id}, steps={len(delivery_steps or [])}"
        )

        try:
            async with websockets.connect(
                self.base_url, extra_headers=headers, close_timeout=5
            ) as websocket:
                result = await self._handle_websocket_connection_steps(
                    websocket, toid, item_id, delivery_steps
                )
                if result:
            ...
            if "extra_headers" in error_msg:
                logger.warning(
                    "websockets库不支持extra_headers参数，使用兼容模式发送发货步骤"
                )
                async with websockets.connect(
                    self.base_url, additional_headers=headers, close_timeout=5
                ) as websocket:
                    ...
                    if result:
                        logger.info(
                            f"【{self.cookie_id}】单次发送发货步骤成功(兼容模式)"
                        )
                    else:
                        raise Exception("发货步骤发送失败")
            else:
            ...
            logger.error(
                f"【{self.cookie_id}】单次发送发货步骤异常: {self._safe_str(e)}"
            )
            raise

    async def _handle_websocket_connection_steps(
        self, websocket, toid, item_id, delivery_steps
    ):
        ...
        try:
            ...
            async for message in websocket:
                try:
                    ...
                    cid = cid.split("@")[0]
                    await self._send_delivery_steps(
                        websocket, cid, toid, delivery_steps, log_prefix="单次手动发货"
                    )
                    ...
                    logger.warning(
                        f"【{self.cookie_id}】处理消息异常: {self._safe_str(e)}"
                    )
                    continue
            ...
            logger.error(
                f"【{self.cookie_id}】WebSocket发货步骤处理异常: {self._safe_str(e)}"
            )
            return False
    ...
    async def _create_websocket_connection(self, headers):
        ...
        if proxy_url:
            proxy_type = self.proxy_config.get("proxy_type", "none")
            logger.info(
                f"【{self.cookie_id}】WebSocket将通过代理连接: {proxy_type}://{self.proxy_config.get('proxy_host')}:{self.proxy_config.get('proxy_port')}"
            )
            ...
            try:
                ...
                if socks_type:
                    ...
                    dest_port = parsed_url.port or (
                        443 if parsed_url.scheme == "wss" else 80
                    )
                    ...
                    if parsed_url.scheme == "wss":
                        ...
                        proxy_sock = ssl_context.wrap_socket(
                            proxy_sock, server_hostname=dest_host
                        )
                ...
                logger.warning(
                    f"【{self.cookie_id}】代理连接需要安装 python-socks: pip install python-socks[asyncio]"
                )
                logger.warning(f"【{self.cookie_id}】将尝试不使用代理进行WebSocket连接")
                proxy_sock = None
            except Exception as e:
                logger.error(
                    f"【{self.cookie_id}】通过代理建立连接失败: {self._safe_str(e)}"
                )
                logger.warning(f"【{self.cookie_id}】将尝试不使用代理进行WebSocket连接")
                proxy_sock = None
        ...
        try:
            ...
            if (
                "extra_headers" in error_msg
                or "unexpected keyword argument" in error_msg
            ):
                logger.warning(
                    f"【{self.cookie_id}】websockets库不支持extra_headers参数，尝试additional_headers"
                )
                ...
                try:
                    ...
                    logger.warning(
                        f"【{self.cookie_id}】additional_headers参数失败: {error_msg2}"
                    )

                    if (
                        "additional_headers" in error_msg2
                        or "unexpected keyword argument" in error_msg2
                    ):
                        # 如果都不支持，则不传递headers
                        logger.warning(
                            f"【{self.cookie_id}】websockets库不支持headers参数，使用基础连接模式"
                        )
                        if proxy_sock:
    ...
    async def _handle_websocket_connection(self, websocket, toid, item_id, text):
        ...
        try:
            ...
            logger.error(
                f"【{self.cookie_id}】WebSocket连接处理异常: {self._safe_str(e)}"
            )
            return False

    def _extract_chat_message_text(self, message_10: Any) -> str:
        """统一提取聊天消息文本，兼容 reminderContent/detailNotice/reminderNotice。"""
        if not isinstance(message_10, dict):
            return ""

        for key in ("reminderContent", "detailNotice", "reminderNotice"):
            text = str(message_10.get(key, "") or "").strip()
            if text:
                return text
        return ""
    ...
    def is_chat_message(self, message):
        ...
        try:
            return (
                ...
                and "10" in message["1"]
                and isinstance(message["1"]["10"], dict)
                and bool(self._extract_chat_message_text(message["1"]["10"]))
            )
        except Exception:
            return False
    ...
    async def create_session(self):
        ...
        if not self.session:
            ...
            if proxy_url:
                proxy_type = self.proxy_config.get("proxy_type", "none")
                logger.info(
                    f"【{self.cookie_id}】创建带代理的Session: {proxy_type}://{self.proxy_config.get('proxy_host')}:{self.proxy_config.get('proxy_port')}"
                )
                ...
                if proxy_type == "socks5":
                    ...
                    try:
                        ...
                        logger.error(
                            f"【{self.cookie_id}】SOCKS5代理需要安装 aiohttp-socks: pip install aiohttp-socks"
                        )
                        connector = None
                else:
            ...
            self._http_proxy_url = (
                proxy_url
                if proxy_url
                and self.proxy_config.get("proxy_type") in ["http", "https"]
                else None
            )
    ...
    async def _handle_message_with_semaphore(
        self, message_data, websocket, msg_id="unknown"
    ):
        ...
        async with self.message_semaphore:
            ...
            try:
                ...
                if (
                    self.active_message_tasks % 100 == 0
                    and self.active_message_tasks > 0
                ):
                    logger.info(
                        f"【{self.cookie_id}】当前活跃消息处理任务数: {self.active_message_tasks}"
                    )
        ...
        try:
            ...
            if isinstance(message_data, dict) and "1" in message_data:
                ...
                if isinstance(message_1, dict) and "10" in message_1:
                    message_10 = message_1.get("10")
                    if isinstance(message_10, dict):
                        biz_tag_dict = self._load_json_dict(
                            message_10.get("bizTag", "")
                        )
                        biz_tag_message_id = str(
                            biz_tag_dict.get("messageId") or ""
                        ).strip()
                        if biz_tag_message_id:
                            return biz_tag_message_id

                        ext_json_dict = self._load_json_dict(
                            message_10.get("extJson", "")
                        )
                        ext_json_message_id = str(
                            ext_json_dict.get("messageId") or ""
                        ).strip()
                        if ext_json_message_id:
                            return ext_json_message_id
        except Exception as e:
    ...
    def _split_sync_package_packets(self, message_data: dict) -> list[dict]:
        ...
        try:
            ...
            body = (
                message_data.get("body", {}) if isinstance(message_data, dict) else {}
            )
            sync_push_package = (
                body.get("syncPushPackage", {}) if isinstance(body, dict) else {}
            )
            sync_data_list = (
                sync_push_package.get("data", [])
                if isinstance(sync_push_package, dict)
                else []
            )

            if not isinstance(sync_data_list, list) or len(sync_data_list) <= 1:
                return [message_data]
            ...
            for sync_data in sync_data_list:
                single_packet = {
                    **message_data,
                    "headers": dict(message_data.get("headers", {})),
                    "body": dict(body),
                }
                single_sync_push_package = dict(sync_push_package)
                single_sync_push_package["data"] = [sync_data]
                single_packet["body"]["syncPushPackage"] = single_sync_push_package
                packets.append(single_packet)

            return packets or [message_data]
        except Exception as e:
            logger.warning(
                f"【{self.cookie_id}】拆分同步包失败，回退原始处理: {self._safe_str(e)}"
            )
            return [message_data]

    async def _mark_message_processed_if_new(self, message_id: str) -> bool:
        """记录消息ID去重状态；返回True表示允许继续处理，False表示近期已处理。"""
        if not message_id:
            return True
        ...
        async with self.processed_message_ids_lock:
            ...
            if message_id in self.processed_message_ids:
                ...
                if time_elapsed < self.message_expire_time:
                    ...
                    logger.warning(
                        f"【{self.cookie_id}】消息ID {message_id[:50]}... 已处理过，距离可重复回复还需 {remaining_time} 秒"
                    )
                    return False

                logger.info(
                    f"【{self.cookie_id}】消息ID {message_id[:50]}... 已超过 {int(time_elapsed / 60)} 分钟，允许重新回复"
                )
            ...
            if len(self.processed_message_ids) > self.processed_message_ids_max_size:
                expired_ids = [
                    msg_id
                    for msg_id, timestamp in self.processed_message_ids.items()
                ...
                logger.info(
                    f"【{self.cookie_id}】已清理 {len(expired_ids)} 个过期消息ID"
                )
                ...
                if (
                    ...
                    sorted_ids = sorted(
                        self.processed_message_ids.items(), key=lambda x: x[1]
                    )
                    remove_count = len(sorted_ids) // 2
                    for msg_id, _ in sorted_ids[:remove_count]:
                        del self.processed_message_ids[msg_id]
                    logger.info(
                        f"【{self.cookie_id}】消息ID去重字典过大，已清理 {remove_count} 个最旧记录"
                    )
    ...
    def _build_fallback_message_id(
        ...
        *,
        ...
        """在缺少稳定 messageId 时，尽量基于消息内容生成更稳妥的去重键。"""
        fallback_create_time = 0
        try:
            fallback_create_time = int(create_time or 0)
        except Exception:
            fallback_create_time = 0

        if not fallback_create_time and isinstance(message_data, dict):
            message_1 = message_data.get("1")
            if isinstance(message_1, dict):
                try:
                    fallback_create_time = int(message_1.get("5", 0) or 0)
                except Exception:
                    fallback_create_time = 0

        if fallback_create_time:
            return f"{chat_id}_{send_message}_{fallback_create_time}"
        ...
        try:
            ...
            normalized_payload = repr(message_data)

        payload_hash = hashlib.sha1(
            normalized_payload.encode("utf-8", errors="ignore")
        ).hexdigest()[:16]
        return f"{chat_id}_{send_message}_{payload_hash}"
        ...
        async with self.message_debounce_lock:
            ...
            if chat_id in self.message_debounce_tasks:
                ...
                if old_task and not old_task.done():
                    old_task.cancel()
                    logger.warning(
                        f"【{self.cookie_id}】取消chat_id {chat_id} 的旧防抖任务"
                    )
                ...
                try:
                    ...
                    async with self.message_debounce_lock:
                        ...
                        if saved_timer != debounce_info["timer"]:
                            logger.warning(
                                f"【{self.cookie_id}】chat_id {chat_id} 在防抖期间有新消息，跳过旧消息处理"
                            )
                            return
                    ...
                    # 处理最后一条消息
                    logger.info(
                        f"【{self.cookie_id}】防抖延迟结束，开始处理chat_id {chat_id} 的最后一条消息: {last_msg['send_message'][:30]}..."
                    )
                    ...
                    logger.warning(
                        f"【{self.cookie_id}】chat_id {chat_id} 的防抖任务被取消"
                    )
                except Exception as e:
                    logger.error(
                        f"【{self.cookie_id}】处理防抖回复时发生错误: {self._safe_str(e)}"
                    )
                    # 确保从防抖任务中移除
                    async with self.message_debounce_lock:
                        if chat_id in self.message_debounce_tasks:
            ...
            logger.warning(
                f"【{self.cookie_id}】为chat_id {chat_id} 创建防抖任务，延迟 {self.message_debounce_delay} 秒"
            )

    async def _process_chat_message_reply(
        self,
        message_data: dict,
        websocket,
        send_user_name: str,
        ...
        try:
            ...
            if pause_manager.is_chat_paused(chat_id):
                ...
                logger.info(
                    f"[{msg_time}] 【{self.cookie_id}】【系统】chat_id {chat_id} 自动回复已暂停，剩余时间: {remaining_minutes}分{remaining_seconds}秒"
                )
                return
            ...
            if AUTO_REPLY.get("api", {}).get("enabled", False):
                ...
                if not reply:
                    logger.error(
                        f"[{msg_time}] 【API调用失败】用户: {send_user_name} (ID: {send_user_id}), 商品({item_id}): {send_message}"
                    )
            ...
            if not reply:
                ...
                if reply == "EMPTY_REPLY":
                    ...
                    logger.info(
                        f"[{msg_time}] 【{self.cookie_id}】匹配到空回复关键词，跳过自动回复"
                    )
                    return
                elif reply:
                    ...
                    if reply:
                        ...
                        reply = await self.get_default_reply(
                            send_user_name, send_user_id, send_message, chat_id, item_id
                        )
                        if reply == "EMPTY_REPLY":
                            # 默认回复内容为空，不进行任何回复
                            logger.info(
                                f"[{msg_time}] 【{self.cookie_id}】默认回复内容为空，跳过自动回复"
                            )
                            return
            ...
            if reply:
                ...
                if reply.startswith("__IMAGE_SEND__"):
                    ...
                    try:
                        ...
                        logger.info(
                            f"[{msg_time}] 【{reply_source}图片发出】用户: {send_user_name} (ID: {send_user_id}), 商品({item_id}): 图片 {image_url}"
                        )
                    except Exception as e:
                        # 图片发送失败，发送错误提示
                        ...
                        msg_time = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
                        logger.error(
                            f"[{msg_time}] 【{reply_source}图片发送失败】用户: {send_user_name} (ID: {send_user_id}), 商品({item_id})"
                        )
                else:
                    # 普通文本消息
                    ...
                    msg_time = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
                    logger.info(
                        f"[{msg_time}] 【{reply_source}发出】用户: {send_user_name} (ID: {send_user_id}), 商品({item_id}): {reply}"
                    )
            else:
                msg_time = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
                logger.info(
                    f"[{msg_time}] 【{self.cookie_id}】【系统】未找到匹配的回复规则，不回复"
                )
        except Exception as e:
    ...
    async def handle_message(
        ...
        try:
            ...
            if cookie_manager and not cookie_manager.get_cookie_status(self.cookie_id):
                logger.warning(
                    f"【{self.cookie_id}】[{msg_id}] ⏹️ 账号已禁用，消息处理结束"
                )
                return
            ...
            sync_packets = self._split_sync_package_packets(message_data)
            if not skip_ack and len(sync_packets) > 1:
                logger.info(
                    f"【{self.cookie_id}】[{msg_id}] 检测到同步包包含 {len(sync_packets)} 条数据，拆分后逐条处理"
                )
                for index, single_packet in enumerate(sync_packets, start=1):
                    await self.handle_message(
                        single_packet,
                        websocket,
                ...
                logger.info(f"【{self.cookie_id}】[{msg_id}] ⏹️ 多条同步数据处理完成")
                return
            ...
            if "data" not in sync_data:
                logger.warning(
                    f"【{self.cookie_id}】[{msg_id}] ⚠️ 同步包中无data字段，消息内容: {sync_data}"
                )
                logger.warning(
                    f"【{self.cookie_id}】[{msg_id}] ⏹️ 消息处理结束（缺少data字段）"
                )
                return
            ...
            try:
                ...
                try:
                    ...
                    if isinstance(parsed_data, dict) and "chatType" in parsed_data:
                        logger.warning(
                            f"【{self.cookie_id}】[{msg_id}] ⚠️ 检测到chatType消息，完整内容: {parsed_data}"
                        )
                        if (
                            "operation" in parsed_data
                            and "content" in parsed_data["operation"]
                        ):
                            content = parsed_data["operation"]["content"]
                            if "sessionArouse" in content:
                                # 处理系统引导消息
                                logger.info(
                                    f"[{msg_time}] 【{self.cookie_id}】[{msg_id}] 【系统】小闲鱼智能提示:"
                                )
                                if "arouseChatScriptInfo" in content["sessionArouse"]:
                                    for qa in content["sessionArouse"][
                                        "arouseChatScriptInfo"
                                    ]:
                                        logger.info(f"  - {qa['chatScrip']}")
                                logger.info(
                                    f"[{msg_time}] 【{self.cookie_id}】[{msg_id}] ⏹️ 系统引导消息处理完成"
                                )
                                return
                            elif "contentType" in content:
                                # 其他类型的未加密消息
                                logger.warning(
                                    f"[{msg_time}] 【{self.cookie_id}】[{msg_id}] 【系统】其他类型消息: {content}"
                                )
                        # ⚠️ 修复：不能直接return，应该继续处理这条消息
                        # 因为付款消息可能也包含chatType字段
                        logger.warning(
                            f"【{self.cookie_id}】[{msg_id}] ⚠️ chatType消息但不是引导消息，继续处理..."
                        )
                        message = parsed_data
                    else:
                        # 如果不是系统消息，将解析的数据作为message
                        logger.debug(
                            f"【{self.cookie_id}】[{msg_id}] 解密成功，正常消息"
                        )
                        message = parsed_data
                except Exception as e:
                    # 如果JSON解析失败，尝试解密
                    logger.debug(
                        f"【{self.cookie_id}】[{msg_id}] JSON解析失败，尝试解密..."
                    )
                ...
                logger.error(
                    f"【{self.cookie_id}】[{msg_id}] ❌ 消息解密失败: {self._safe_str(e)}"
                )
                if msg_size > 3000:
                    logger.error(
                        f"【{self.cookie_id}】[{msg_id}] ⚠️⚠️⚠️ 大消息({msg_size}字节)解密失败，完整sync_data: {sync_data}"
                    )
                    # 尝试记录base64数据的前后部分
                    try:
                        raw_data = sync_data.get("data", "")
                        logger.error(
                            f"【{self.cookie_id}】[{msg_id}] Base64数据长度: {len(raw_data)}"
                        )
                        logger.error(
                            f"【{self.cookie_id}】[{msg_id}] Base64前100字符: {raw_data[:100]}"
                        )
                        logger.error(
                            f"【{self.cookie_id}】[{msg_id}] Base64后100字符: {raw_data[-100:]}"
                        )
                    except Exception:
                        pass
                logger.error(
                    f"【{self.cookie_id}】[{msg_id}] ⏹️ 消息处理结束（解密失败）"
                )
                return
            ...
            if message is None:
                ...
                if msg_size > 3000:
                    logger.error(
                        f"【{self.cookie_id}】[{msg_id}] ⚠️⚠️⚠️ 大消息({msg_size}字节)解析后为空！"
                    )
                logger.error(
                    f"【{self.cookie_id}】[{msg_id}] ⏹️ 消息处理结束（解析后为空）"
                )
                return
            ...
            if not isinstance(message, dict):
                logger.error(
                    f"【{self.cookie_id}】[{msg_id}] ❌ 消息格式错误，期望字典但得到: {type(message)}"
                )
                logger.warning(f"【{self.cookie_id}】[{msg_id}] 消息内容: {message}")
                logger.error(
                    f"【{self.cookie_id}】[{msg_id}] ⏹️ 消息处理结束（格式错误）"
                )
                return
            ...
            try:
                logger.info(
                    f"【{self.cookie_id}】[{msg_id}] 🔍 开始提取订单ID，消息类型: {type(message)}"
                )
                order_id = self._extract_order_id(message, raw_message_data)
                if order_id:
                    msg_time = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
                    logger.info(
                        f"[{msg_time}] 【{self.cookie_id}】[{msg_id}] ✅ 检测到订单ID: {order_id}，开始获取订单详情"
                    )

                    order_context = self._extract_order_message_context(
                    ...
                    if self.order_status_handler:
                        logger.info(
                            f"【{self.cookie_id}】准备调用订单状态处理器.on_order_id_extracted: {order_id}"
                        )
                        try:
                            ...
                            logger.info(
                                f"【{self.cookie_id}】订单状态处理器.on_order_id_extracted调用成功: {order_id}"
                            )
                        except Exception as e:
                            logger.error(
                                f"【{self.cookie_id}】通知订单状态处理器订单ID提取失败: {self._safe_str(e)}"
                            )
                            import traceback
                        ...
                        logger.warning(
                            f"【{self.cookie_id}】订单状态处理器为None，跳过订单ID提取通知: {order_id}"
                        )
                    ...
                    try:
                        ...
                        if order_detail:
                            logger.info(
                                f"[{msg_time}] 【{self.cookie_id}】✅ 订单详情获取成功: {order_id}"
                            )
                        else:
                            logger.warning(
                                f"[{msg_time}] 【{self.cookie_id}】⚠️ 订单详情获取失败: {order_id}"
                            )
                        ...
                        logger.error(
                            f"[{msg_time}] 【{self.cookie_id}】❌ 获取订单详情异常: {self._safe_str(detail_e)}"
                        )
                        if basic_order_saved:
                            self._schedule_order_detail_retry(
                                order_id,
                ...
                logger.error(
                    f"【{self.cookie_id}】[{msg_id}] 提取订单ID失败: {self._safe_str(e)}"
                )
            ...
            try:
                ...
                if red_reminder == "等待买家付款":
                    user_url = f"https://www.goofish.com/personal?userId={user_id}"
                    logger.info(
                        f"[{msg_time}] 【{self.cookie_id}】[{msg_id}] 【系统】等待买家 {user_url} 付款"
                    )
                    logger.info(
                        f"【{self.cookie_id}】[{msg_id}] ⏹️ 处理结束（等待买家付款）"
                    )
                    return
                elif red_reminder == "交易关闭":
                    user_url = f"https://www.goofish.com/personal?userId={user_id}"
                    logger.info(
                        f"[{msg_time}] 【{self.cookie_id}】[{msg_id}] 【系统】买家 {user_url} 交易关闭"
                    )
                    ...
                    if self.order_status_handler:
                        try:
                            ...
                            logger.error(
                                f"【{self.cookie_id}】更新交易关闭订单状态失败: {self._safe_str(e)}"
                            )

                    logger.info(
                        f"【{self.cookie_id}】[{msg_id}] ⏹️ 处理结束（交易关闭）"
                    )
                    return
                elif red_reminder == "等待卖家发货":
                    user_url = f"https://www.goofish.com/personal?userId={user_id}"
                    logger.info(
                        f"[{msg_time}] 【{self.cookie_id}】[{msg_id}] 【系统】交易成功 {user_url} 等待卖家发货"
                    )
                    ...
                    if isinstance(message.get("1"), str):
                        logger.info(
                            f"[{msg_time}] 【{self.cookie_id}】[{msg_id}] 🔔 检测到简化结构的发货通知消息，延迟处理"
                        )
                        await asyncio.sleep(30)
                        logger.info(
                            f"[{msg_time}] 【{self.cookie_id}】[{msg_id}] 🔔 延迟30秒后处理简化发货"
                        )
                        # 检查是否启用自动确认发货
                        if self.is_auto_confirm_enabled():
                            logger.info(
                                f"[{msg_time}] 【{self.cookie_id}】[{msg_id}] ✅ 自动确认发货已启用，开始处理"
                            )
                            ...
                            session_id_str = (
                                simple_sid.split("@")[0]
                                if "@" in str(simple_sid)
                                else simple_sid
                            )

                            logger.info(
                                f"[{msg_time}] 【{self.cookie_id}】[{msg_id}] 🔍 简化消息解析: sid={simple_sid}, session_id={session_id_str}"
                            )
                            ...
                            if recent_order and sid_match_type in {
                                ...
                                simple_user_id = recent_order.get(
                                    "buyer_id", user_id
                                )  # 从订单中获取buyer_id
                                logger.info(
                                    f"[{msg_time}] 【{self.cookie_id}】[{msg_id}] ✅ 通过sid从数据库找到订单: order_id={order_id}, item_id={real_item_id}, buyer_id={simple_user_id}"
                                )
                                ...
                                if not self.can_auto_delivery(order_id):
                                    logger.info(
                                        f"[{msg_time}] 【{self.cookie_id}】[{msg_id}] 🔒 订单 {order_id} 已在冷却期内（可能完整消息已处理），跳过简化消息发货"
                                    )
                                    logger.info(
                                        f"【{self.cookie_id}】[{msg_id}] ⏹️ 处理结束（订单已处理）"
                                    )
                                    return
                                ...
                                if self.is_lock_held(order_id):
                                    logger.info(
                                        f"[{msg_time}] 【{self.cookie_id}】[{msg_id}] 🔒 订单 {order_id} 延迟锁已被持有（可能完整消息正在处理），跳过简化消息发货"
                                    )
                                    logger.info(
                                        f"【{self.cookie_id}】[{msg_id}] ⏹️ 处理结束（订单正在处理）"
                                    )
                                    return
                                ...
                                logger.info(
                                    f"【{self.cookie_id}】[{msg_id}] ⏹️ 处理结束（简化消息自动发货）"
                                )
                                return
                            elif recent_order:
                                order_id = recent_order.get("order_id")
                                order_status = (
                                    recent_order.get("order_status") or "unknown"
                                )
                                if sid_match_type == "already_processed":
                                    logger.info(
                                    ...
                                    logger.info(
                                        f"【{self.cookie_id}】[{msg_id}] ⏹️ 处理结束（订单已关闭）"
                                    )
                                else:
                                    logger.info(
                                    ...
                                    logger.info(
                                        f"【{self.cookie_id}】[{msg_id}] ⏹️ 处理结束（订单状态未就绪）"
                                    )
                                return
                            else:
                                logger.warning(
                                    f"[{msg_time}] 【{self.cookie_id}】[{msg_id}] ❌ 未找到sid {simple_sid} 的最近订单，跳过自动发货"
                                )
                                logger.info(
                                    f"【{self.cookie_id}】[{msg_id}] ⏹️ 处理结束（未找到订单）"
                                )
                                return
                        else:
                            logger.info(
                                f"[{msg_time}] 【{self.cookie_id}】[{msg_id}] ⚠️ 未启用自动确认发货，跳过"
                            )
                            logger.info(
                                f"【{self.cookie_id}】[{msg_id}] ⏹️ 处理结束（未启用自动发货）"
                            )
                            return
                    # 如果不是简化结构，继续走正常流程
            except Exception:
            ...
            try:
                ...
                if not (
                    ...
                    logger.error(
                        f"【{self.cookie_id}】[{msg_id}] ❌ 消息格式错误：缺少必要的字段结构"
                    )
                    logger.error(
                        f"【{self.cookie_id}】[{msg_id}] ⏹️ 处理结束（格式错误）"
                    )
                    return
                ...
                if not isinstance(message_1.get("10"), dict):
                    logger.error(
                        f"【{self.cookie_id}】[{msg_id}] ❌ 消息格式错误：缺少消息详情字段"
                    )
                    logger.error(
                        f"【{self.cookie_id}】[{msg_id}] ⏹️ 处理结束（缺少详情字段）"
                    )
                    return

                create_time = int(message_1.get("5", 0))
                message_10 = message_1["10"]
                send_user_name = message_10.get(
                    "senderNick", message_10.get("reminderTitle", "未知用户")
                )
                send_user_id = message_10.get("senderUserId", "unknown")
                send_message = self._extract_chat_message_text(message_10)
                ...
                logger.error(
                    f"【{self.cookie_id}】[{msg_id}] ❌ 提取聊天消息信息失败: {self._safe_str(e)}"
                )
                logger.error(
                    f"【{self.cookie_id}】[{msg_id}] ⏹️ 处理结束（提取信息失败）"
                )
                return
            ...
            if send_user_id == self.myid and not is_system_message:
                logger.info(
                    f"[{msg_time}] 【{self.cookie_id}】[{msg_id}] 【手动发出】 商品({item_id}): {send_message}"
                )
                ...
                logger.info(
                    f"【{self.cookie_id}】[{msg_id}] ⏹️ 处理结束（手动发出消息）"
                )
                return
                        ...
                        if time.time() - waiting_info["create_time"] > 1800:
                            ...
                            try:
                                ...
                                if isinstance(message_10, dict):
                                    ...
                                    if biz_tag and (
                                        "SECURITY" in biz_tag
                                        or "taskName" in biz_tag
                                        or "taskId" in biz_tag
                                    ):
                                        is_system_msg = True
                            except Exception:
                                pass
                            ...
                            if (
                                ...
                                logger.info(
                                    f"【{self.cookie_id}】[{msg_id}] 收到系统消息，跳过账号确认处理（direction={message_direction}, contentType={content_type}, isSystem={is_system_msg}）"
                                )
                                logger.info(
                                    f"【{self.cookie_id}】[{msg_id}] ⏹️ 处理结束（系统消息）"
                                )
                                return
                            ...
                            if waiting_info["state"] == "waiting_account":
                                ...
                                if account:
                                    ...
                                    await self.send_msg(
                                        self.ws, chat_id, send_user_id, confirm_msg
                                    )
                                    logger.info(
                                        f"【{self.cookie_id}】[{msg_id}] 已保存充值账号: {account}，等待用户确认"
                                    )
                                    logger.info(
                                        f"【{self.cookie_id}】[{msg_id}] ⏹️ 处理结束（等待账号确认）"
                                    )
                                ...
                                if user_reply == "是":
                                    ...
                                    try:
                                        # 直接调用亦凡API下单
                                        delivery_content = (
                                            await self._call_yifan_api_with_account(
                                                rule,
                                                account,
                                                order_id_saved,
                                                item_id_saved,
                                        ...
                                        if delivery_content:
                                            ...
                                            if not finalize_result.get("success"):
                                                self._record_delivery_log(
                                                    ...
                                                    status="failed",
                                                    reason=finalize_result.get("error")
                                                    or "亦凡账号确认发货发送成功但提交副作用失败",
                                                    channel="auto",
                                                    rule_meta={
                                                        "rule_id": rule.get("id"),
                                                        "rule_keyword": rule.get(
                                                            "keyword"
                                                        ),
                                                        "card_type": rule.get(
                                                            "card_type"
                                                ...
                                                await self.send_msg(
                                                    ...
                                                    chat_id,
                                                    send_user_id,
                                                    "发货消息已发送，但确认发货失败，请稍后刷新订单状态。",
                                                )
                                                logger.error(
                                                    f"亦凡API自动发货副作用提交失败: {finalize_result.get('error')}"
                                                )
                                                return

                                            if order_id_saved:
                                                self.mark_delivery_sent(
                                                    order_id_saved,
                                                    context="亦凡账号确认发货发送成功",
                                                )
                                                self._activate_delivery_lock(
                                                    order_id_saved, delay_minutes=10
                                                )
                                    ...
                                    if new_account:
                                        ...
                                        if waiting_info["retry_count"] >= 5:
                                            logger.warning(
                                                f"【{self.cookie_id}】[{msg_id}] 账号确认重试次数过多，取消发货"
                                            )
                                            del self.yifan_account_waiting[chat_id]
                                            await self.send_msg(
                                                self.ws,
                                                chat_id,
                                                send_user_id,
                                                "账号确认失败次数过多，已取消发货，请重新下单。",
                                            )
                                            logger.info(
                                                f"【{self.cookie_id}】[{msg_id}] ⏹️ 处理结束（重试次数过多）"
                                            )
                                            return
                                        ...
                                        logger.info(
                                            f"【{self.cookie_id}】[{msg_id}] 用户重新输入账号: {new_account}，再次等待确认"
                                        )
                                        logger.info(
                                            f"【{self.cookie_id}】[{msg_id}] ⏹️ 处理结束（等待账号重新确认）"
                                        )
                                        return
                ...
                try:
                    if is_group_message:
                        logger.info(
                            f"📱 检测到群组消息（sessionType=30），跳过消息通知"
                        )
            ...
            if self.order_status_handler:
                try:
                    ...
                    try:
                        ...
                        logger.error(
                            f"【{self.cookie_id}】处理系统消息失败: {self._safe_str(e)}"
                        )
                        handled = False
                            ...
                            if (
                                ...
                                if red_reminder:
                                    try:
                                        ...
                                        logger.error(
                                            f"【{self.cookie_id}】处理红色提醒消息失败: {self._safe_str(e)}"
                                        )
                        except Exception as red_e:
                            logger.warning(
                                f"处理红色提醒消息失败: {self._safe_str(red_e)}"
                            )
            ...
            if order_id and order_status_signal in {
                "pending_ship",
                "shipped",
                "completed",
                "cancelled",
            ...
            if send_message == "[我已拍下，待付款]":
                logger.info(
                    f"[{msg_time}] 【{self.cookie_id}】[{msg_id}] 系统消息不处理"
                )
                logger.info(
                    f"【{self.cookie_id}】[{msg_id}] ⏹️ 处理结束（系统消息：待付款）"
                )
                return
                ...
                logger.info(
                    f"【{self.cookie_id}】[{msg_id}] ⏹️ 处理结束（系统消息：订单关闭）"
                )
                return
                ...
                # 检测到评价提醒消息，尝试自动好评
                logger.info(
                    f"[{msg_time}] 【{self.cookie_id}】[{msg_id}] 🌟 检测到评价提醒消息: {send_message}"
                )
                await self.handle_auto_comment(message, msg_time, msg_id)
                logger.info(
                    f"【{self.cookie_id}】[{msg_id}] ⏹️ 处理结束（评价提醒消息）"
                )
                return
            elif message_route == "system_notice" or any(
                keyword in send_message for keyword in fallback_ignore_keywords
            ):
                logger.info(
                ...
                return
            # 简化消息通过 sid 查找订单，更可靠
            elif message_route == "order_status" and self._is_auto_delivery_trigger(
                send_message
            ):
                logger.info(
                    f"[{msg_time}] 【{self.cookie_id}】[{msg_id}] 检测到自动发货触发消息: {send_message}"
                )
                ...
                if not is_system_message:
                    ...
                    logger.info(
                        f"【{self.cookie_id}】[{msg_id}] ⏹️ 处理结束（非系统触发）"
                    )
                    return
                ...
                if not self.is_auto_confirm_enabled():
                    logger.info(
                        f"[{msg_time}] 【{self.cookie_id}】[{msg_id}] 未启用自动确认发货，跳过自动发货"
                    )
                    logger.info(
                        f"【{self.cookie_id}】[{msg_id}] ⏹️ 处理结束（未启用自动发货）"
                    )
                ...
                logger.info(
                    f"【{self.cookie_id}】[{msg_id}] ⏹️ 处理结束（自动发货完成）"
                )
                return
            # 【重要】检查小刀流程卡片消息 - 即使在人工接入暂停期间也要处理
                ...
                try:
                    ...
                    if not card_title and isinstance(card_message_1, dict):
                        if "6" in card_message_1 and isinstance(
                            card_message_1["6"], dict
                        ):
                            ...
                            if "3" in message_6 and isinstance(message_6["3"], dict):
                                ...
                                if "5" in message_6_3:
                                    ...
                                    try:
                                        ...
                                        if (
                                            "dxCard" in card_content
                                            and "item" in card_content["dxCard"]
                                        ):
                                            card_item = card_content["dxCard"]["item"]
                                            if (
                                                "main" in card_item
                                                and "exContent" in card_item["main"]
                                            ):
                                                ex_content = card_item["main"][
                                                    "exContent"
                                                ]
                    ...
                    card_message_direction = (
                        card_message_1.get("7", 0)
                    ...
                    is_system_card_message = (
                        card_message_direction == 1
                        or card_content_type == 6
                        or card_is_system_biz
                    )
                    ...
                    if card_title in waiting_bargain_titles:
                        logger.info(
                            f'[{msg_time}] 【{self.cookie_id}】【系统】检测到"{card_title}"，执行免拼流程'
                        )

                        # 检查是否启用自动确认发货
                        if not self.is_auto_confirm_enabled():
                            logger.info(
                                f"[{msg_time}] 【{self.cookie_id}】未启用自动确认发货，跳过自动小刀和自动发货"
                            )
                            return
                        ...
                        if item_id and item_id != "未知商品":
                            try:
                                if not await self._ensure_item_owned_by_current_account(
                                    ...
                                    logger.warning(
                                        f"[{msg_time}] 【{self.cookie_id}】❌ 商品 {item_id} 不属于当前账号，跳过免拼发货"
                                    )
                                    return
                                logger.warning(
                                    f"[{msg_time}] 【{self.cookie_id}】✅ 商品 {item_id} 归属验证通过"
                                ...
                                logger.error(
                                    f"[{msg_time}] 【{self.cookie_id}】检查商品归属失败: {self._safe_str(e)}，跳过免拼发货"
                                )
                                return
                        ...
                        if not order_id:
                            logger.warning(
                                f"[{msg_time}] 【{self.cookie_id}】❌ 未能提取到订单ID，无法执行免拼发货"
                            )
                            return
                        ...
                        # 延迟2秒后执行免拼发货
                        logger.info(
                            f"[{msg_time}] 【{self.cookie_id}】延迟2秒后执行免拼发货..."
                        )
                        await asyncio.sleep(2)
                        # 调用自动免拼发货方法
                        result = await self.auto_freeshipping(
                            order_id, item_id, send_user_id
                        )
                        if result.get("success"):
                            self._mark_order_bargain_flow(
                            ...
                            logger.info(
                                f"[{msg_time}] 【{self.cookie_id}】✅ 自动免拼发货成功"
                            )
                            logger.info(
                                f'[{msg_time}] 【{self.cookie_id}】⏳ 已完成免拼，等待"我已成功小刀，待发货"卡片后再自动发货'
                            )
                            return
                        else:
                            logger.warning(
                                f"[{msg_time}] 【{self.cookie_id}】❌ 自动免拼发货失败: {result.get('error', '未知错误')}"
                            )
                            logger.info(
                                f"[{msg_time}] 【{self.cookie_id}】⏹️ 免拼失败，不执行自动发货"
                            )
                            return
                    ...
                    # 第二阶段：成功小刀待发货，触发自动发货
                        ...
                        logger.info(
                            f'[{msg_time}] 【{self.cookie_id}】【系统】检测到"{card_title}"，开始自动发货'
                        )
                        ...
                        if not self.is_auto_confirm_enabled():
                            logger.info(
                                f"[{msg_time}] 【{self.cookie_id}】未启用自动确认发货，跳过自动发货"
                            )
                            return
                        ...
                        logger.info(
                            f"[{msg_time}] 【{self.cookie_id}】⏹️ 小刀成功待发货卡片处理完成"
                        )
                        return
                    else:
                        logger.info(
                            f"[{msg_time}] 【{self.cookie_id}】收到卡片消息，标题: {card_title or '未知'}"
                        )
                ...
                if valid_buyer_nick:
                    try:
                        ...
                        db_manager.update_buyer_nick_by_buyer_id(
                            send_user_id, valid_buyer_nick, self.cookie_id
                        )
                    except Exception as e:
            ...
            logger.error(
                f"【{self.cookie_id}】[{msg_id}] ❌ 处理消息时发生异常: {self._safe_str(e)}"
            )
            if msg_size > 3000:
                logger.error(
                    f"【{self.cookie_id}】[{msg_id}] ⚠️⚠️⚠️ 大消息({msg_size}字节)处理异常！"
                )
            logger.warning(f"【{self.cookie_id}】[{msg_id}] 原始消息: {message_data}")
            import traceback

            logger.error(
                f"【{self.cookie_id}】[{msg_id}] 异常堆栈: {traceback.format_exc()}"
            )
        finally:
            # 确保每条消息都有明确的处理结束标记
            logger.info(
                f"【{self.cookie_id}】[{msg_id}] 🏁 消息处理完成 ({msg_size}字节)"
            )
    ...
    async def main(self):
        ...
        try:
            ...
            logger.info(
                f"【{self.cookie_id}】Session创建完成，开始WebSocket连接循环..."
            )
            ...
            while True:
                try:
                    ...
                    init_auth_state = (
                        self.get_init_auth_failure_state(self.cookie_id) or {}
                    )
                    ...
                    if circuit_until and time.time() < circuit_until:
                        ...
                        self._set_connection_state(
                            ...
                            f"初始化鉴权冷静期剩余{remaining_seconds}秒",
                        )
                        logger.warning(
                    ...
                    # 更新连接状态为连接中
                    self._set_connection_state(
                        ConnectionState.CONNECTING, "准备建立WebSocket连接"
                    )
                    logger.info(
                        f"【{self.cookie_id}】WebSocket目标地址: {self.base_url}"
                    )
                    ...
                    async with await self._create_websocket_connection(
                        ...
                        self.ws = websocket
                        logger.info(
                            f"【{self.cookie_id}】WebSocket连接建立成功，开始初始化..."
                        )
                        ...
                        try:
                            ...
                            # 初始化完成后才设置为已连接状态
                            self._set_connection_state(
                                ConnectionState.CONNECTED, "初始化完成，连接就绪"
                            )
                            ...
                            # 记录后台任务启动前的状态
                            logger.warning(
                                f"【{self.cookie_id}】准备启动后台任务 - 当前状态: heartbeat={self.heartbeat_task}, token_refresh={self.token_refresh_task}, cleanup={self.cleanup_task}, cookie_refresh={self.cookie_refresh_task}"
                            )

                            # 启动心跳任务（依赖WebSocket，每次重连都需要重启）
                            logger.info(f"【{self.cookie_id}】启动心跳任务...")
                            await self._start_heartbeat_task(
                                websocket, reason="WebSocket连接建立成功"
                            )
                            ...
                            if (
                                not self.token_refresh_task
                                or self.token_refresh_task.done()
                            ):
                                ...
                                logger.info(
                                    f"【{self.cookie_id}】Token刷新任务已在运行，跳过启动"
                                )

                            if not self.cleanup_task or self.cleanup_task.done():
                                logger.info(
                                    f"【{self.cookie_id}】启动暂停记录清理任务..."
                                )
                                ...
                                logger.info(
                                    f"【{self.cookie_id}】暂停记录清理任务已在运行，跳过启动"
                                )

                            if (
                                not self.cookie_refresh_task
                                or self.cookie_refresh_task.done()
                            ):
                                logger.info(
                                    f"【{self.cookie_id}】启动Cookie刷新任务..."
                                )
                                self.cookie_refresh_task = asyncio.create_task(
                                    self.cookie_refresh_loop()
                                )
                                tasks_started.append("Cookie刷新")
                            else:
                                logger.info(
                                    f"【{self.cookie_id}】Cookie刷新任务已在运行，跳过启动"
                                )
                            ...
                            if tasks_started:
                                logger.info(
                                    f"【{self.cookie_id}】✅ 新启动的任务: {', '.join(tasks_started)}"
                                )
                            logger.info(
                                f"【{self.cookie_id}】✅ 所有后台任务状态: 心跳(已启动), Token刷新({'运行中' if self.token_refresh_task and not self.token_refresh_task.done() else '已启动'}), 暂停清理({'运行中' if self.cleanup_task and not self.cleanup_task.done() else '已启动'}), Cookie刷新({'运行中' if self.cookie_refresh_task and not self.cookie_refresh_task.done() else '已启动'})"
                            )

                            logger.info(f"【{self.cookie_id}】开始监听WebSocket消息...")
                            logger.info(
                                f"【{self.cookie_id}】WebSocket连接状态正常，等待服务器消息..."
                            )
                                ...
                                try:
                                    ...
                                    try:
                                        ...
                                        if (
                                            ...
                                            msg_id = message_data["headers"].get(
                                                "mid", "unknown"
                                            )
                                        ...
                                        if (
                                            ...
                                            if (
                                                "syncPushPackage"
                                                in message_data["body"]
                                            ):
                                                msg_preview = "[同步包]"
                                            elif (
                                                "ack"
                                                in str(message_data["body"]).lower()
                                            ):
                                                msg_preview = "[确认]"
                                    except Exception:
                                        pass
                                    ...
                                    logger.info(
                                        f"【{self.cookie_id}】📨 收到消息 [ID:{msg_id}] {msg_preview} {len(message) if message else 0}字节"
                                    )

                                    # 处理心跳响应（高优先级，直接处理）
                                    if await self.handle_heartbeat_response(
                                        message_data
                                    ):
                                        continue
                                    ...
                                    if (
                                        self.message_queue_enabled
                                        and self.message_queue_running
                                    ):
                                        # 消息队列模式：快速入队，由工作协程异步处理
                                        await self._enqueue_message(
                                            message_data, websocket, msg_id
                                        )
                                    else:
                                        # 传统模式：使用追踪的异步任务处理消息
                                        self._create_tracked_task(
                                            self._handle_message_with_semaphore(
                                                message_data, websocket, msg_id
                                            )
                                        )
                    ...
                    self.current_token = None
                    self.connection_failures = 0
                    init_auth_state = self.record_init_auth_failure(
                        self.cookie_id, error_msg
                    )
                    self.init_auth_failures = int(init_auth_state.get("count", 0))
                    self._set_connection_state(
                        ConnectionState.RECONNECTING,
                        f"初始化鉴权失败第{self.init_auth_failures}次",
                    )
                    logger.error(
                        f"【{self.cookie_id}】初始化鉴权失败 ({self.init_auth_failures}/{self._init_auth_failure_threshold})"
                    )
                    ...
                    if circuit_until and time.time() < circuit_until:
                        ...
                        logger.warning(
                            f"【{self.cookie_id}】将在 {retry_delay} 秒后重试初始化鉴权..."
                        )

                    self._reset_background_tasks()
                    await self._interruptible_sleep(retry_delay)
                    logger.info(
                        f"【{self.cookie_id}】初始化鉴权重试等待完成，准备重新建立连接..."
                    )
                    continue
                    ...
                    is_connection_closed = (
                        ...
                        or "no close frame received or sent" in error_msg
                    ...
                    if is_connection_closed:
                        logger.warning(
                            f"【{self.cookie_id}】WebSocket连接已关闭 ({self.connection_failures + 1}/{self.max_connection_failures})"
                        )
                    ...
                    self._set_connection_state(
                        ConnectionState.RECONNECTING,
                        f"第{self.connection_failures}次失败",
                    )
                    logger.error(
                        f"【{self.cookie_id}】WebSocket连接异常 ({self.connection_failures}/{self.max_connection_failures})"
                    )
                    logger.error(f"【{self.cookie_id}】异常类型: {error_type}")
                    logger.error(f"【{self.cookie_id}】异常信息: {error_msg}")
                    logger.warning(
                        f"【{self.cookie_id}】异常堆栈:\n{traceback.format_exc()}"
                    )
                    ...
                    if is_connection_closed:
                        ...
                        self._set_connection_state(
                            ConnectionState.RECONNECTING,
                            f"连接关闭，第{self.connection_failures}次重连",
                        )
                    ...
                    if self.connection_failures >= self.max_connection_failures:
                        ...
                        logger.warning(
                            f"【{self.cookie_id}】连续失败{self.max_connection_failures}次，尝试通过密码登录刷新Cookie..."
                        )
                        ...
                        try:
                            ...
                            if refresh_success:
                                logger.info(
                                    f"【{self.cookie_id}】✅ 密码登录刷新成功，将重置失败计数并继续重连"
                                )
                                ...
                                logger.warning(
                                    f"【{self.cookie_id}】❌ 密码登录刷新失败，将重启实例..."
                                )
                        except Exception as refresh_e:
                            logger.error(
                                f"【{self.cookie_id}】密码登录刷新过程异常: {self._safe_str(refresh_e)}"
                            )
                        ...
                        try:
                            ...
                            logger.warning(
                                f"【{self.cookie_id}】后台任务清理超时，强制继续重启"
                            )
                        except Exception as cleanup_e:
                            logger.error(
                                f"【{self.cookie_id}】后台任务清理失败: {self._safe_str(cleanup_e)}"
                            )
                        ...
                        logger.info(
                            f"【{self.cookie_id}】重启请求已触发，主程序即将退出，新实例将自动启动"
                        )
                    ...
                    retry_delay = self._calculate_retry_delay(error_msg)
                    logger.warning(
                        f"【{self.cookie_id}】将在 {retry_delay} 秒后重试连接..."
                    )
                    ...
                    try:
                        ...
                        if self.current_token:
                            logger.warning(
                                f"【{self.cookie_id}】清空当前token，重新连接时将重新获取"
                            )
                            self.current_token = None
                        ...
                        # 这样可以避免等待任务取消导致的阻塞问题
                        logger.info(
                            f"【{self.cookie_id}】准备重置后台任务引用（快速重连模式）..."
                        )
                        self._reset_background_tasks()
                        logger.info(
                            f"【{self.cookie_id}】后台任务引用已重置，可以立即重连"
                        )
                        ...
                        while remaining > 0:
                            ...
                            try:
                                ...
                                if remaining > 0:
                                    logger.info(
                                        f"【{self.cookie_id}】等待中... 已等待 {elapsed:.1f} 秒，剩余 {remaining:.1f} 秒"
                                    )
                                    # 定期刷新日志
                                    try:
                                        sys.stdout.flush()
                                ...
                                logger.warning(
                                    f"【{self.cookie_id}】等待期间收到取消信号"
                                )
                                raise
                            except Exception as sleep_error:
                                logger.error(
                                    f"【{self.cookie_id}】等待期间发生异常: {self._safe_str(sleep_error)}"
                                )
                                logger.warning(
                                    f"【{self.cookie_id}】等待异常堆栈:\n{traceback.format_exc()}"
                                )
                                # 即使出错也继续等待剩余时间
                                if remaining > 0:
                        ...
                        logger.info(
                            f"【{self.cookie_id}】等待完成（总耗时 {time.time() - start_time:.1f} 秒），准备重新连接..."
                        )
                        # 再次强制刷新日志
                        try:
                            sys.stdout.flush()
                        ...
                        logger.error(
                            f"【{self.cookie_id}】清理过程出错: {self._safe_str(cleanup_error)}"
                        )
                        logger.warning(
                            f"【{self.cookie_id}】清理异常堆栈:\n{traceback.format_exc()}"
                        )
                        ...
                        self.cookie_refresh_task = None
                        logger.warning(
                            f"【{self.cookie_id}】清理失败，已强制重置所有任务引用"
                        )
                        # 使用可中断的sleep，并定期输出日志
                        logger.info(
                            f"【{self.cookie_id}】清理失败后开始等待 {retry_delay} 秒..."
                        )
                        chunk_size = 5.0
                        remaining = retry_delay
                        start_time = time.time()
                        ...
                        while remaining > 0:
                            ...
                            try:
                                ...
                                if remaining > 0:
                                    logger.info(
                                        f"【{self.cookie_id}】清理失败后等待中... 剩余 {remaining:.1f} 秒"
                                    )
                            except asyncio.CancelledError:
                                logger.warning(
                                    f"【{self.cookie_id}】清理失败后等待期间收到取消信号"
                                )
                                raise
                            except Exception as sleep_error:
                                logger.error(
                                    f"【{self.cookie_id}】清理失败后等待期间发生异常: {self._safe_str(sleep_error)}"
                                )
                                if remaining > 0:
                                    await asyncio.sleep(remaining)
                                break
                        ...
                        logger.info(
                            f"【{self.cookie_id}】清理失败后等待完成（总耗时 {time.time() - start_time:.1f} 秒）"
                        )
            ...
            if has_pending_tasks:
                ...
                try:
                    await asyncio.wait_for(
                        self._cancel_background_tasks(), timeout=10.0
                    )
                except asyncio.TimeoutError:
                    logger.error(
                        f"【{self.cookie_id}】程序退出时任务取消超时，强制继续"
                    )
                except Exception as e:
                    logger.error(
                        f"【{self.cookie_id}】程序退出时任务取消失败: {self._safe_str(e)}"
                    )
                finally:
                    # 确保任务引用被重置
                    self.heartbeat_task = None
            ...
            if self.background_tasks:
                logger.info(
                    f"【{self.cookie_id}】等待 {len(self.background_tasks)} 个后台任务完成..."
                )
                try:
                    await asyncio.wait_for(
    ...
    async def get_item_list_info(
        self, page_number=1, page_size=20, retry_count=0, sync_item_details=False
    ):
        """获取商品信息，自动处理token失效的情况
        ...
        try:
            async with self.session.post(
                ...
                if res_json.get("ret") and res_json["ret"][0] == "SUCCESS::调用成功":
                    ...
                    for card in card_list:
                        ...
                        if card_data:
                            ...
                            item_info = {
                                ...
                                "price": card_data.get("priceInfo", {}).get(
                                    "price", ""
                                ),
                                "price_text": card_data.get("priceInfo", {}).get(
                                    "preText", ""
                                )
                                + card_data.get("priceInfo", {}).get("price", ""),
                    ...
                    print("\n" + "=" * 80)
                    print(
                        f"📦 账号 {self.myid} 的商品列表 (第{page_number}页，{len(items_list)} 个商品)"
                    )
                    print("=" * 80)
                    ...
                    for i, item in enumerate(items_list, 1):
                        ...
                        if item.get("pic_info"):
                            ...
                            print(
                                f"   图片信息: {pic_info.get('width', 'N/A')}x{pic_info.get('height', 'N/A')}"
                            )
                            print(f"   图片链接: {pic_info.get('picUrl', 'N/A')}")
                        print(
                            f"   完整信息: {json.dumps(item, ensure_ascii=False, indent=2)}"
                        )
                    ...
                    # 检查是否是token失效
                    error_msg = (
                        res_json.get("ret", [""])[0] if res_json.get("ret") else ""
                    )
                    if (
                        "FAIL_SYS_TOKEN_EXOIRED" in error_msg
    ...
    async def get_all_items(
        self, page_size=20, max_pages=None, sync_item_details=False
    ):
        """获取所有商品信息（自动分页）
        ...
        while True:
            ...
            if len(current_items) < page_size:
                logger.info(
                    f"第 {page_number} 页商品数量({len(current_items)})少于页面大小({page_size})，获取完成"
                )
                break
        ...
        logger.info(
            f"所有商品获取完成，共 {len(all_items)} 个商品，保存了 {total_saved} 个"
        )
    ...
    async def send_image_msg(
        self, ws, cid, toid, image_url, width=800, height=600, card_id=None
    ):
        """发送图片消息"""
        try:
            # 检查图片URL是否需要上传到CDN
            ...
            if self._is_cdn_url(image_url):
                ...
                if os.path.exists(local_image_path):
                    logger.info(
                        f"【{self.cookie_id}】准备上传本地图片到闲鱼CDN: {local_image_path}"
                    )
                    ...
                    async with uploader:
                        ...
                        if cdn_url:
                            logger.info(
                                f"【{self.cookie_id}】图片上传成功，CDN URL: {cdn_url}"
                            )
                            image_url = cdn_url
                            ...
                            try:
                                actual_width, actual_height = (
                                    image_manager.get_image_size(local_image_path)
                                )
                                if actual_width and actual_height:
                                    width, height = actual_width, actual_height
                                    logger.info(
                                        f"【{self.cookie_id}】获取到实际图片尺寸: {width}x{height}"
                                    )
                            except Exception as e:
                                logger.warning(
                                    f"【{self.cookie_id}】获取图片尺寸失败，使用默认尺寸: {e}"
                                )
                        else:
                            logger.error(
                                f"【{self.cookie_id}】图片上传失败: {local_image_path}"
                            )
                            logger.error(
                                f"【{self.cookie_id}】❌ Cookie可能已失效！请检查配置并更新Cookie"
                            )
                            raise Exception(
                                f"图片上传失败（Cookie可能已失效）: {local_image_path}"
                            )
                else:
                    logger.error(
                        f"【{self.cookie_id}】本地图片文件不存在: {local_image_path}"
                    )
            ...
            content_base64 = str(
                base64.b64encode(content_json.encode("utf-8")), "utf-8"
            )
    ...
    async def send_image_from_file(self, ws, cid, toid, image_path):
        ...
        try:
            ...
            if image_url:
                ...
                logger.info(
                    f"【{self.cookie_id}】图片发送完成: {image_path} -> {image_url}"
                )
                return True
            else:
