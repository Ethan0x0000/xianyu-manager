...
class DBManager:
    ...
    def __init__(self, db_path: str = None):
        ...
        if os.getenv("SQL_LOG_ENABLED"):
            self.sql_log_enabled = (
                os.getenv("SQL_LOG_ENABLED", "true").lower() == "true"
            )
    ...
    def _migrate_plaintext_cookie_secrets(self):
        ...
        with self.lock:
            ...
            self._execute_sql(
                cursor, "SELECT id, value, password, proxy_pass FROM cookies"
            )
    ...
    def resolve_external_order_status(
        self, current_status: str, incoming_status: str, source: str = "external_sync"
    ) -> str:
        ...
        if (
            ...
            and normalized_incoming
            not in {"refunding", "cancelled", "refund_cancelled"}
        ):
            logger.warning(
    ...
    def init_db(self):
        ...
        try:
            ...
            try:
                ...
                self._execute_sql(
                    cursor, "CREATE INDEX IF NOT EXISTS idx_orders_sid ON orders(sid)"
                )
            ...
            try:
                ...
                self._execute_sql(
                    cursor, "ALTER TABLE orders ADD COLUMN buyer_nick TEXT"
                )
            ...
            try:
                self._execute_sql(
                    cursor, "SELECT pre_refund_status FROM orders LIMIT 1"
                )
            except sqlite3.OperationalError:
                logger.info("正在为 orders 表添加 pre_refund_status 列...")
                self._execute_sql(
                    cursor, "ALTER TABLE orders ADD COLUMN pre_refund_status TEXT"
                )
            ...
            try:
                self._execute_sql(
                    cursor, "SELECT bargain_flow_detected FROM orders LIMIT 1"
                )
            except sqlite3.OperationalError:
                logger.info("正在为 orders 表添加 bargain_flow_detected 列...")
                self._execute_sql(
                    cursor,
                    "ALTER TABLE orders ADD COLUMN bargain_flow_detected INTEGER DEFAULT 0",
                )
            ...
            try:
                self._execute_sql(
                    cursor, "SELECT bargain_success_detected FROM orders LIMIT 1"
                )
            except sqlite3.OperationalError:
                logger.info("正在为 orders 表添加 bargain_success_detected 列...")
                self._execute_sql(
                    cursor,
                    "ALTER TABLE orders ADD COLUMN bargain_success_detected INTEGER DEFAULT 0",
                )
            ...
            try:
                ...
                self._execute_sql(
                    cursor, "ALTER TABLE keywords ADD COLUMN item_id TEXT"
                )
            ...
            try:
                self._execute_sql(
                    cursor, "SELECT multi_quantity_delivery FROM item_info LIMIT 1"
                )
            except sqlite3.OperationalError:
                ...
                self._execute_sql(
                    cursor,
                    "ALTER TABLE item_info ADD COLUMN multi_quantity_delivery BOOLEAN DEFAULT FALSE",
                )
            ...
            self._execute_sql(
                cursor,
                "CREATE INDEX IF NOT EXISTS idx_delivery_logs_user_time ON delivery_logs(user_id, created_at)",
            )
            self._execute_sql(
                cursor,
            ...
            self._execute_sql(
                cursor,
                "CREATE INDEX IF NOT EXISTS idx_delivery_finalization_states_status ON delivery_finalization_states(status, updated_at)",
            )
            ...
            self._execute_sql(
                cursor,
                "CREATE INDEX IF NOT EXISTS idx_data_card_reservations_card_status ON data_card_reservations(card_id, status)",
            )
            self._execute_sql(
                cursor,
                "CREATE INDEX IF NOT EXISTS idx_data_card_reservations_order_status ON data_card_reservations(order_id, status)",
            )
            self._execute_sql(
                cursor,
                "CREATE INDEX IF NOT EXISTS idx_data_card_reservations_card_order_unit ON data_card_reservations(card_id, order_id, unit_index)",
            )
            ...
            try:
                cursor.execute(
                    "ALTER TABLE default_replies ADD COLUMN reply_once BOOLEAN DEFAULT FALSE"
                )
                self.conn.commit()
...
账号已可正常使用。')
    ...
    def _migrate_database(self, cursor):
        ...
        try:
            ...
            if "auto_comment" not in cookie_columns:
                ...
                cursor.execute(
                    "ALTER TABLE cookies ADD COLUMN auto_comment INTEGER DEFAULT 0"
                )
            ...
            if "api_type" not in ai_columns:
                ...
                cursor.execute(
                    "ALTER TABLE ai_reply_settings ADD COLUMN api_type TEXT DEFAULT ''"
                )
            ...
            if "api_type" not in preset_columns:
                ...
                cursor.execute(
                    "ALTER TABLE ai_config_presets ADD COLUMN api_type TEXT NOT NULL DEFAULT ''"
                )
            ...
            for column_name, column_type in risk_log_column_defs.items():
                if column_name not in risk_log_columns:
                    ...
                    cursor.execute(
                        f"ALTER TABLE risk_control_logs ADD COLUMN {column_name} {column_type}"
                    )
                    logger.info(
                        f"数据库迁移完成：添加risk_control_logs.{column_name}列"
                    )

            self._execute_sql(
                cursor,
                "CREATE INDEX IF NOT EXISTS idx_risk_control_logs_cookie_created ON risk_control_logs(cookie_id, created_at DESC)",
            )
            self._execute_sql(
                cursor,
                "CREATE INDEX IF NOT EXISTS idx_risk_control_logs_type_status_created ON risk_control_logs(event_type, processing_status, created_at DESC)",
            )
            self._execute_sql(
                cursor,
                "CREATE INDEX IF NOT EXISTS idx_risk_control_logs_session_id ON risk_control_logs(session_id)",
            )
    ...
    def _update_cards_table_constraints(self, cursor):
        ...
        try:
            ...
            if "CHECK constraint failed" in str(e) or "constraint" in str(e).lower():
                logger.info(
                    "检测到旧的CHECK约束，开始更新cards表以支持yifan_api类型..."
                )
    ...
    def _migrate_notification_templates(self, cursor):
        ...
        try:
            ...
            cursor.execute(
                "SELECT COUNT(*) FROM notification_templates WHERE type = 'cookie_refresh_success'"
            )
...
账号已可正常使用。')
        ...
        try:
            ...
            if admin_user:
                ...
                try:
                    ...
                    self._execute_sql(
                        cursor, "ALTER TABLE cookies ADD COLUMN user_id INTEGER"
                    )
                    self._execute_sql(
                        cursor,
                        "UPDATE cookies SET user_id = ? WHERE user_id IS NULL",
                        (admin_user_id,),
                    )
                else:
                    # user_id列存在，更新NULL值
                ...
                try:
                    self._execute_sql(
                        cursor, "SELECT auto_confirm FROM cookies LIMIT 1"
                    )
                except sqlite3.OperationalError:
                    # auto_confirm列不存在，需要添加并设置默认值
                    self._execute_sql(
                        cursor,
                        "ALTER TABLE cookies ADD COLUMN auto_confirm INTEGER DEFAULT 1",
                    )
                    self._execute_sql(
                        cursor,
                        "UPDATE cookies SET auto_confirm = 1 WHERE auto_confirm IS NULL",
                    )
                else:
                ...
                try:
                    self._execute_sql(
                        cursor, "SELECT user_id FROM delivery_rules LIMIT 1"
                    )
                except sqlite3.OperationalError:
                    # user_id列不存在，需要添加并更新历史数据
                    self._execute_sql(
                        cursor, "ALTER TABLE delivery_rules ADD COLUMN user_id INTEGER"
                    )
                    self._execute_sql(
                        cursor,
                ...
                try:
                    self._execute_sql(
                        cursor, "SELECT last_delivery_date FROM delivery_rules LIMIT 1"
                    )
                except sqlite3.OperationalError:
                    # 今日发货字段不存在，需要添加
                    self._execute_sql(
                        cursor,
                        "ALTER TABLE delivery_rules ADD COLUMN last_delivery_date DATE",
                    )
                    self._execute_sql(
                        cursor,
                        "ALTER TABLE delivery_rules ADD COLUMN today_delivery_times INTEGER DEFAULT 0",
                    )
                    logger.info(
                        "已添加 last_delivery_date 和 today_delivery_times 字段到 delivery_rules 表"
                    )

                # 为notification_channels表添加user_id字段（如果不存在）
                try:
                    self._execute_sql(
                        cursor, "SELECT user_id FROM notification_channels LIMIT 1"
                    )
                except sqlite3.OperationalError:
                    # user_id列不存在，需要添加并更新历史数据
                    self._execute_sql(
                        cursor,
                        "ALTER TABLE notification_channels ADD COLUMN user_id INTEGER",
                    )
                    self._execute_sql(
                        cursor,
                        "UPDATE notification_channels SET user_id = ? WHERE user_id IS NULL",
                        (admin_user_id,),
                    )
                else:
                    # user_id列存在，更新NULL值
                ...
                try:
                    self._execute_sql(
                        cursor, "SELECT type FROM email_verifications LIMIT 1"
                    )
                except sqlite3.OperationalError:
                    # type列不存在，需要添加并更新历史数据
                    self._execute_sql(
                        cursor,
                        "ALTER TABLE email_verifications ADD COLUMN type TEXT DEFAULT 'register'",
                    )
                    self._execute_sql(
                        cursor,
                        "UPDATE email_verifications SET type = 'register' WHERE type IS NULL",
                    )
                else:
                    # type列存在，更新NULL值
                    self._execute_sql(
                ...
                try:
                    ...
                    self._execute_sql(
                        cursor, "ALTER TABLE cards ADD COLUMN spec_name TEXT"
                    )
                    self._execute_sql(
                        cursor, "ALTER TABLE cards ADD COLUMN spec_value TEXT"
                    )
                ...
                try:
                    ...
                    # 双规格字段不存在，需要添加
                    self._execute_sql(
                        cursor, "ALTER TABLE cards ADD COLUMN spec_name_2 TEXT"
                    )
                    self._execute_sql(
                        cursor, "ALTER TABLE cards ADD COLUMN spec_value_2 TEXT"
                    )
                ...
                try:
                    ...
                    # 双规格字段不存在，需要添加
                    self._execute_sql(
                        cursor, "ALTER TABLE orders ADD COLUMN spec_name_2 TEXT"
                    )
                    self._execute_sql(
                        cursor, "ALTER TABLE orders ADD COLUMN spec_value_2 TEXT"
                    )
                ...
                try:
                    self._execute_sql(
                        cursor, "SELECT is_multi_spec FROM item_info LIMIT 1"
                    )
                except sqlite3.OperationalError:
                    # 多规格字段不存在，需要添加
                    self._execute_sql(
                        cursor,
                        "ALTER TABLE item_info ADD COLUMN is_multi_spec BOOLEAN DEFAULT FALSE",
                    )
    ...
    def upgrade_cookies_table_for_account_login(self, cursor):
        ...
        try:
            ...
            try:
                ...
                # username字段不存在，需要添加
                self._execute_sql(
                    cursor, "ALTER TABLE cookies ADD COLUMN username TEXT DEFAULT ''"
                )
            ...
            try:
                ...
                # password字段不存在，需要添加
                self._execute_sql(
                    cursor, "ALTER TABLE cookies ADD COLUMN password TEXT DEFAULT ''"
                )
            ...
            try:
                ...
                self._execute_sql(
                    cursor,
                    "ALTER TABLE cookies ADD COLUMN show_browser INTEGER DEFAULT 0",
                )
    ...
    def upgrade_cookies_table_for_proxy(self, cursor):
        ...
        try:
            ...
            try:
                ...
                self._execute_sql(
                    cursor,
                    "ALTER TABLE cookies ADD COLUMN proxy_type TEXT DEFAULT 'none'",
                )
            ...
            try:
                ...
                self._execute_sql(
                    cursor, "ALTER TABLE cookies ADD COLUMN proxy_host TEXT DEFAULT ''"
                )
            ...
            try:
                ...
                self._execute_sql(
                    cursor,
                    "ALTER TABLE cookies ADD COLUMN proxy_port INTEGER DEFAULT 0",
                )
            ...
            try:
                ...
                self._execute_sql(
                    cursor, "ALTER TABLE cookies ADD COLUMN proxy_user TEXT DEFAULT ''"
                )
            ...
            try:
                ...
                self._execute_sql(
                    cursor, "ALTER TABLE cookies ADD COLUMN proxy_pass TEXT DEFAULT ''"
                )
    ...
    def upgrade_users_table_for_admin(self, cursor):
        ...
        try:
            ...
            # 将admin用户设置为管理员
            self._execute_sql(
                cursor, "UPDATE users SET is_admin = 1 WHERE username = 'admin'"
            )
    ...
    def _migrate_keywords_table_constraints(self, cursor):
        ...
        try:
            ...
            cursor.execute(
                "SELECT name FROM sqlite_master WHERE type='index' AND name='idx_keywords_unique_with_item'"
            )
            if cursor.fetchone():
    ...
    def save_cookie(
        ...
        with self.lock:
            try:
                ...
                if user_id is None:
                    self._execute_sql(
                        cursor, "SELECT user_id FROM cookies WHERE id = ?", (cookie_id,)
                    )
                    ...
                    if existing:
                        ...
                        # 获取admin用户ID作为默认值
                        self._execute_sql(
                            cursor, "SELECT id FROM users WHERE username = 'admin'"
                        )
                        admin_user = cursor.fetchone()
                ...
                if saved_user_id:
                    logger.info(
                        f"Cookie保存验证: {cookie_id} 实际绑定到用户ID: {saved_user_id[0]}"
                    )
                else:
    ...
    def delete_cookie(self, cookie_id: str) -> bool:
        ...
        with self.lock:
            try:
                ...
                # 删除关联的关键字
                self._execute_sql(
                    cursor, "DELETE FROM keywords WHERE cookie_id = ?", (cookie_id,)
                )
                # 删除Cookie
                self._execute_sql(
                    cursor, "DELETE FROM cookies WHERE id = ?", (cookie_id,)
                )
                self.conn.commit()
    ...
    def get_cookie(self, cookie_id: str) -> Optional[str]:
        ...
        with self.lock:
            try:
                cursor = self.conn.cursor()
                self._execute_sql(
                    cursor, "SELECT value FROM cookies WHERE id = ?", (cookie_id,)
                )
    ...
    def get_all_cookies(self, user_id: int = None) -> Dict[str, str]:
        ...
        with self.lock:
            try:
                ...
                return {
                    row[0]: self._decrypt_secret(row[1]) for row in cursor.fetchall()
                }
            except Exception as e:
    ...
    def update_cookie_pause_duration(self, cookie_id: str, pause_duration: int) -> bool:
        ...
        with self.lock:
            try:
                ...
                self.conn.commit()
                logger.info(
                    f"更新账号 {cookie_id} 自动回复暂停时间: {pause_duration}分钟"
                )
                return True
            except Exception as e:
    ...
    def get_cookie_pause_duration(self, cookie_id: str) -> int:
        ...
        with self.lock:
            try:
                ...
                self._execute_sql(
                    cursor,
                    "SELECT pause_duration FROM cookies WHERE id = ?",
                    (cookie_id,),
                )
                result = cursor.fetchone()
                if result:
                    if result[0] is None:
                        logger.warning(
                            f"账号 {cookie_id} 的pause_duration为NULL，使用默认值10分钟并修复数据库"
                        )
                        # 修复数据库中的NULL值
                        self._execute_sql(
                            cursor,
                            "UPDATE cookies SET pause_duration = 10 WHERE id = ?",
                            (cookie_id,),
                        )
                        self.conn.commit()
                        return 10
    ...
    def update_cookie_account_info(
        ...
        with self.lock:
            try:
                ...
                # 检查记录是否存在
                self._execute_sql(
                    cursor, "SELECT id FROM cookies WHERE id = ?", (cookie_id,)
                )
                ...
                if not exists:
                    ...
                    if cookie_value is None:
                        logger.warning(
                            f"账号 {cookie_id} 不存在，且未提供cookie_value，无法创建新记录"
                        )
                        return False
                    ...
                    insert_values = [
                        cookie_id,
                        self._encrypt_secret(cookie_value),
                        user_id,
                    ]
                    insert_placeholders = ["?", "?", "?"]
                    ...
                    self.conn.commit()
                    logger.info(
                        f"创建新账号 {cookie_id} 并保存信息成功: {insert_fields}"
                    )
                    return True
                else:
                    # 记录存在，执行更新
                    ...
                    if not update_fields:
                        logger.warning(
                            f"更新账号 {cookie_id} 信息时没有提供任何更新字段"
                        )
                        return False
    ...
    def update_cookie_proxy_config(
        ...
        with self.lock:
            try:
                ...
                if not update_fields:
                    logger.warning(
                        f"更新账号 {cookie_id} 代理配置时没有提供任何更新字段"
                    )
                    return False
                ...
                self.conn.commit()
                logger.info(
                    f"更新账号 {cookie_id} 代理配置成功: type={proxy_type}, host={proxy_host}, port={proxy_port}"
                )
                return True
            except Exception as e:
    ...
    def get_auto_confirm(self, cookie_id: str) -> bool:
        ...
        with self.lock:
            try:
                ...
                self._execute_sql(
                    cursor,
                    "SELECT auto_confirm FROM cookies WHERE id = ?",
                    (cookie_id,),
                )
                result = cursor.fetchone()
                if result:
    ...
    def get_auto_comment(self, cookie_id: str) -> bool:
        ...
        with self.lock:
            try:
                ...
                self._execute_sql(
                    cursor,
                    "SELECT auto_comment FROM cookies WHERE id = ?",
                    (cookie_id,),
                )
                result = cursor.fetchone()
        ...
        with self.lock:
            try:
                ...
                self._execute_sql(
                    ...
                    (int(auto_comment), cookie_id),
                )
                self.conn.commit()
                logger.info(
                    f"更新账号 {cookie_id} 自动好评设置: {'开启' if auto_comment else '关闭'}"
                )
                return True
            except Exception as e:
    ...
    def add_comment_template(
        self, cookie_id: str, name: str, content: str, is_active: bool = False
    ) -> Optional[int]:
        """添加好评模板"""
        with self.lock:
            try:
                ...
                if is_active:
                    self._execute_sql(
                        cursor,
                        "UPDATE comment_templates SET is_active = 0 WHERE cookie_id = ?",
                        (cookie_id,),
                    )
                ...
                self._execute_sql(
                    cursor,
                    "SELECT MAX(sort_order) FROM comment_templates WHERE cookie_id = ?",
                    (cookie_id,),
                )
                ...
                self.conn.commit()
                logger.info(
                    f"添加好评模板成功: cookie_id={cookie_id}, name={name}, id={template_id}"
                )
                return template_id
            except Exception as e:
    ...
    def update_comment_template(
        ...
        with self.lock:
            try:
                ...
                self._execute_sql(
                    cursor,
                    "SELECT cookie_id FROM comment_templates WHERE id = ?",
                    (template_id,),
                )
                result = cursor.fetchone()
                if not result:
    ...
    def delete_comment_template(self, template_id: int) -> bool:
        ...
        with self.lock:
            try:
                cursor = self.conn.cursor()
                self._execute_sql(
                    cursor, "DELETE FROM comment_templates WHERE id = ?", (template_id,)
                )
                self.conn.commit()
    ...
    def set_active_comment_template(self, cookie_id: str, template_id: int) -> bool:
        ...
        with self.lock:
            try:
                ...
                self._execute_sql(
                    cursor,
                    "UPDATE comment_templates SET is_active = 1, updated_at = CURRENT_TIMESTAMP WHERE id = ? AND cookie_id = ?",
                    (template_id, cookie_id),
                )
                self.conn.commit()
                logger.info(
                    f"设置激活好评模板: cookie_id={cookie_id}, template_id={template_id}"
                )
                return True
            except Exception as e:
    ...
    def save_keywords_with_item_id(
        self, cookie_id: str, keywords: List[Tuple[str, str, str]]
    ) -> bool:
        ...
        with self.lock:
            try:
                ...
                for keyword, reply, item_id in keywords:
                    ...
                    normalized_item_id = (
                        item_id if item_id and item_id.strip() else None
                    )
                    ...
                    try:
                        ...
                        logger.error(
                            f"关键词唯一约束冲突: Cookie={cookie_id}, 关键词='{keyword}', {item_desc}"
                        )
                        raise ie
            ...
            try:
                ...
                for keyword, reply, item_id in keywords:
                    ...
                    if cursor.fetchone():
                        ...
                        logger.warning(
                            f"文本关键词与图片关键词冲突: Cookie={cookie_id}, 关键词='{keyword}', {item_desc}"
                        )
                ...
                self.conn.commit()
                logger.info(
                    f"文本关键字保存成功: {cookie_id}, {len(keywords)}条，图片关键词已保留"
                )
                return True
            except ValueError:
                # 重新抛出友好的错误信息
    ...
    def check_keyword_duplicate(
        self, cookie_id: str, keyword: str, item_id: str = None
    ) -> bool:
        """检查关键词是否重复"""
        with self.lock:
            try:
    ...
    def save_image_keyword(
        self, cookie_id: str, keyword: str, image_url: str, item_id: str = None
    ) -> bool:
        """保存图片关键词（调用前应先检查重复）"""
        with self.lock:
            try:
                ...
                self.conn.commit()
                logger.info(
                    f"图片关键词保存成功: {cookie_id}, 关键词: {keyword}, 图片: {image_url}"
                )
                return True
            except Exception as e:
    ...
    def update_keyword_image_url(
        self, cookie_id: str, keyword: str, new_image_url: str
    ) -> bool:
        """更新关键词的图片URL"""
        with self.lock:
            try:
                ...
                if cursor.rowcount > 0:
                    logger.info(
                        f"关键词图片URL更新成功: {cookie_id}, 关键词: {keyword}, 新URL: {new_image_url}"
                    )
                    return True
                else:
                    logger.warning(
                        f"未找到匹配的图片关键词: {cookie_id}, 关键词: {keyword}"
                    )
                    return False
        ...
        with self.lock:
            try:
                ...
                if 0 <= index < len(rows):
                    rowid = rows[index][0]
                    self._execute_sql(
                        cursor, "DELETE FROM keywords WHERE rowid = ?", (rowid,)
                    )
                    self.conn.commit()
        ...
        with self.lock:
            try:
                ...
                if user_id is not None:
                    ...
                    self._execute_sql(
                        cursor, "SELECT cookie_id, keyword, reply FROM keywords"
                    )
    ...
    def save_cookie_status(self, cookie_id: str, enabled: bool):
        ...
        with self.lock:
            try:
                ...
                self.conn.commit()
                logger.debug(
                    f"保存Cookie状态: {cookie_id} -> {'启用' if enabled else '禁用'}"
                )
            except Exception as e:
    ...
    def save_ai_config_preset(
        ...
        with self.lock:
            try:
                ...
                logger.debug(
                    f"保存AI配置预设: user_id={user_id}, preset_name={preset_name}"
                )
                return preset_id
            except Exception as e:
    ...
    def delete_ai_config_preset(self, user_id: int, preset_id: int) -> bool:
        ...
        with self.lock:
            try:
                ...
                if deleted:
                    logger.debug(
                        f"删除AI配置预设: preset_id={preset_id}, user_id={user_id}"
                    )
                return deleted
            except Exception as e:
    ...
    def save_default_reply(
        ...
        with self.lock:
            try:
                ...
                self.conn.commit()
                logger.debug(
                    f"保存默认回复设置: {cookie_id} -> {'启用' if enabled else '禁用'}, 只回复一次: {'是' if reply_once else '否'}"
                )
            except Exception as e:
    ...
    def get_all_default_replies(self) -> Dict[str, Dict[str, any]]:
        ...
        with self.lock:
            try:
                cursor = self.conn.cursor()
                cursor.execute(
                    "SELECT cookie_id, enabled, reply_content, reply_once FROM default_replies"
                )
    ...
    def delete_default_reply(self, cookie_id: str) -> bool:
        ...
        with self.lock:
            try:
                ...
                self._execute_sql(
                    cursor,
                    "DELETE FROM default_replies WHERE cookie_id = ?",
                    (cookie_id,),
                )
                self.conn.commit()
    ...
    def create_notification_channel(
        self, name: str, channel_type: str, config: str, user_id: int = None
    ) -> int:
        """创建通知渠道"""
        with self.lock:
            try:
    ...
    def get_notification_channel(
        self, channel_id: int, user_id: int = None
    ...
    def set_message_notification(
        self, cookie_id: str, channel_id: int, enabled: bool = True
    ) -> bool:
        """设置账号的消息通知"""
        with self.lock:
            try:
    ...
    def delete_message_notification(
        self, notification_id: int, user_id: int = None
    ) -> bool:
        """删除消息通知配置"""
        with self.lock:
            try:
...
账号已可正常使用。""",
        ...
        return self.update_notification_template(
            template_type, default_templates[template_type]
        )
...
账号已可正常使用。""",
        ...
        with self.lock:
            try:
                ...
                if user_id is not None:
                    ...
                    # 备份用户的cookies
                    self._execute_sql(
                        cursor, "SELECT * FROM cookies WHERE user_id = ?", (user_id,)
                    )
                    ...
                    if user_cookie_ids:
                        ...
                        # 备份关键字
                        cursor.execute(
                            f"SELECT * FROM keywords WHERE cookie_id IN ({placeholders})",
                            user_cookie_ids,
                        )
                        columns = [description[0] for description in cursor.description]
                        rows = cursor.fetchall()
                        ...
                        for table in related_tables:
                            cursor.execute(
                                f"SELECT * FROM {table} WHERE cookie_id IN ({placeholders})",
                                user_cookie_ids,
                            )
                            columns = [
        ...
        with self.lock:
            try:
                ...
                if user_id is not None:
                    ...
                    # 获取用户的cookie_id列表
                    self._execute_sql(
                        cursor, "SELECT id FROM cookies WHERE user_id = ?", (user_id,)
                    )
                    ...
                    if user_cookie_ids:
                        ...
                        # 删除用户的cookies
                        self._execute_sql(
                            cursor, "DELETE FROM cookies WHERE user_id = ?", (user_id,)
                        )
                else:
    ...
    def get_system_setting(self, key: str) -> Optional[str]:
        ...
        with self.lock:
            try:
                cursor = self.conn.cursor()
                self._execute_sql(
                    cursor, "SELECT value FROM system_settings WHERE key = ?", (key,)
                )
                result = cursor.fetchone()
        ...
        with self.lock:
            try:
                ...
                if row:
                    ...
                    if has_is_admin:
                        user_data["is_admin"] = (
                            bool(row[7]) if row[7] is not None else (row[1] == "admin")
                        )
                    else:
    ...
    def save_captcha(
        self, session_id: str, captcha_text: str, expires_minutes: int = 5
    ) -> bool:
        """保存图形验证码"""
        with self.lock:
            try:
                ...
                # 删除该session的旧验证码
                cursor.execute(
                    "DELETE FROM captcha_codes WHERE session_id = ?", (session_id,)
                )
    ...
    def verify_email_code(
        self, email: str, code: str, code_type: str = "register"
    ) -> bool:
        """验证邮箱验证码"""
        with self.lock:
            try:
...
© 2026 闲鱼管理系统"""
            ...
            try:
                ...
                smtp_from = (
                    self.get_system_setting("smtp_from") or ""
                ).strip() or smtp_user
                smtp_use_tls = (
                    self.get_system_setting("smtp_use_tls") or "true"
                ).lower() == "true"
                smtp_use_ssl = (
                    self.get_system_setting("smtp_use_ssl") or "false"
                ).lower() == "true"
            except Exception as e:
    ...
    async def _send_email_via_api(
        self, email: str, subject: str, text_content: str
    ) -> bool:
        """使用API方式发送邮件"""
        try:
            logger.warning(f"API邮件发送通道已移除，跳过发送: {email}")
            return False
        except Exception as e:
    ...
    def create_card(
        ...
        logger.info(
            f"[DEBUG DB] is_multi_spec: {is_multi_spec}, type: {type(is_multi_spec)}"
        )
        ...
        logger.info(
            f"[DEBUG DB] spec_value_2: {spec_value_2}, type: {type(spec_value_2)}"
        )
        ...
        with self.lock:
            try:
                ...
                if is_multi_spec:
                    ...
                    if cursor.fetchone()[0] > 0:
                        raise ValueError(
                            f"卡券已存在：{name} - {spec_name}:{spec_value}"
                        )
                else:
                    # 检查唯一性：仅卡券名称
                    cursor = self.conn.cursor()
                ...
                if is_multi_spec:
                    logger.info(
                        f"创建多规格卡券成功: {name} - {spec_name}:{spec_value} (ID: {card_id})"
                    )
                else:
    ...
    def update_card_image_url(self, card_id: int, new_image_url: str) -> bool:
        ...
        with self.lock:
            try:
                ...
                if cursor.rowcount > 0:
                    logger.info(
                        f"卡券图片URL更新成功: 卡券ID: {card_id}, 新URL: {new_image_url}"
                    )
                    return True
                else:
    ...
    def create_delivery_rule(
        ...
        with self.lock:
            try:
                ...
                logger.info(
                    f"创建发货规则成功: {keyword} -> 卡券ID {card_id} (规则ID: {rule_id})"
                )
                return rule_id
            except Exception as e:
    ...
    def get_delivery_rules_by_keyword(
        self, keyword: str, user_id: int = None, only_non_multi_spec: bool = False
    ):
        """根据关键字获取匹配的发货规则
    ...
    def increment_delivery_times(self, rule_id: int):
        ...
        with self.lock:
            try:
                ...
                # 先查询当前规则的最后发货日期
                cursor.execute(
                    "SELECT last_delivery_date FROM delivery_rules WHERE id = ?",
                    (rule_id,),
                )
                row = cursor.fetchone()
    ...
    def upsert_delivery_finalization_state(
        self,
        order_id: str,
        unit_index: int = 1,
        cookie_id: str = None,
        ...
        with self.lock:
            try:
                ...
                finalized_at_value = (
                    "CURRENT_TIMESTAMP" if status == "finalized" else "NULL"
                )
        ...
        with self.lock:
            try:
                ...
                if expected_mode is None:
                    expected_mode = (
                        "two_spec"
                        if (normalized_spec_name_2 and normalized_spec_value_2)
                        else "one_spec"
                    )
                ...
                if expected_mode == "two_spec":
                    ...
                    if user_id is not None:
                        params = [
                            ...
                            normalized_spec_name_2,
                            normalized_spec_value_2,
                            keyword,
                        ]
                    else:
                        params = [
                ...
                if rules:
                    if expected_mode == "two_spec":
                        logger.info(
                            f"找到两组规格匹配规则: {keyword} - {spec_name}:{spec_value}, {spec_name_2}:{spec_value_2}"
                        )
                    else:
                        logger.info(
                            f"找到一组规格匹配规则: {keyword} - {spec_name}:{spec_value}"
                        )
                else:
                    if expected_mode == "two_spec":
        ...
        with self.lock:
            try:
                ...
                if user_id is not None:
                    ...
                    self._execute_sql(
                        cursor, "DELETE FROM delivery_rules WHERE id = ?", (rule_id,)
                    )
    ...
    def reserve_batch_data(
        ...
        with self.lock:
            try:
                ...
                if existing:
                    logger.info(
                        f"复用批量数据预占记录: card_id={card_id}, order_id={order_id}, unit_index={unit_index}, status={existing[7]}"
                    )
                    return {
                        "id": existing[0],
                        "card_id": existing[1],
                ...
                self._execute_sql(
                    cursor,
                    "SELECT data_content FROM cards WHERE id = ? AND type = 'data'",
                    (card_id,),
                )
                result = cursor.fetchone()
                ...
                lines = [
                    line.strip() for line in str(result[0]).split("\n") if line.strip()
                ]
                if not lines:
                ...
                self.conn.commit()
                logger.info(
                    f"批量数据预占成功: card_id={card_id}, order_id={order_id}, unit_index={unit_index}, reservation_id={reservation_id}"
                )
                return {
                    "id": reservation_id,
                    "card_id": card_id,
                ...
                logger.error(
                    f"预占批量数据失败: card_id={card_id}, order_id={order_id}, error={e}"
                )
                self.conn.rollback()
                return None
    ...
    def mark_batch_data_reservation_sent(self, reservation_id: int):
        ...
        with self.lock:
            try:
                ...
                self._execute_sql(
                    cursor,
                    "SELECT status FROM data_card_reservations WHERE id = ?",
                    (reservation_id,),
                )
                result = cursor.fetchone()
                if not result:
                ...
                if current_status != "reserved":
                    logger.warning(
                        f"批量数据预占状态不允许标记为已发送: reservation_id={reservation_id}, status={current_status}"
                    )
                    return False
                ...
                logger.error(
                    f"标记批量数据预占已发送失败: reservation_id={reservation_id}, error={e}"
                )
                self.conn.rollback()
                return False
        ...
        with self.lock:
            try:
                ...
                if current_status not in ("reserved", "sent"):
                    logger.warning(
                        f"批量数据预占状态不允许 finalize: reservation_id={reservation_id}, status={current_status}"
                    )
                    return {"success": False, "already_finalized": False}
                ...
                logger.error(
                    f"完成批量数据预占失败: reservation_id={reservation_id}, error={e}"
                )
                self.conn.rollback()
                return {"success": False, "already_finalized": False}

    def release_batch_data_reservation(
        self, reservation_id: int, error: str = None, expired: bool = False
    ):
        ...
        with self.lock:
            try:
                ...
                if current_status in ("sent", "consumed"):
                    logger.warning(
                        f"批量数据预占已发送或已完成，不能释放: reservation_id={reservation_id}, status={current_status}"
                    )
                    return False
                ...
                self.conn.commit()
                logger.info(
                    f"释放批量数据预占成功: reservation_id={reservation_id}, status={next_status}"
                )
                return True
            except Exception as e:
                logger.error(
                    f"释放批量数据预占失败: reservation_id={reservation_id}, error={e}"
                )
                self.conn.rollback()
                return False
        ...
        with self.lock:
            try:
                ...
                for reservation_id in stale_ids:
                    if self.release_batch_data_reservation(
                        reservation_id, error="预占超时自动回收", expired=True
                    ):
                        recovered += 1
        ...
        with self.lock:
            try:
                ...
                if line_index < 0 or line_index >= len(lines):
                    logger.warning(
                        f"卡券 {card_id} 预览索引越界: index={line_index}, total={len(lines)}"
                    )
                    return None

                logger.info(
                    f"预览批量数据成功: 卡券ID={card_id}, index={line_index}, 剩余={len(lines)}条"
                )
    ...
    def consume_specific_batch_data(self, card_id: int, expected_line: str):
        ...
        with self.lock:
            try:
                ...
                self.conn.commit()
                logger.info(
                    f"消费指定批量数据成功: 卡券ID={card_id}, 剩余={len(remaining_lines)}条"
                )
                return True
            except Exception as e:
    ...
    def consume_batch_data(self, card_id: int):
        ...
        with self.lock:
            try:
                ...
                logger.info(
                    f"消费批量数据成功: 卡券ID={card_id}, 剩余={len(remaining_lines)}条"
                )
                return first_line
    ...
    def save_item_basic_info(
        ...
        try:
            with self.lock:
                ...
                if item_description:
                    update_parts.append(
                        "item_description = CASE WHEN (item_description IS NULL OR item_description = '') THEN ? ELSE item_description END"
                    )
                    params.append(item_description)

                if item_category:
                    update_parts.append(
                        "item_category = CASE WHEN (item_category IS NULL OR item_category = '') THEN ? ELSE item_category END"
                    )
                    params.append(item_category)

                if item_price:
                    update_parts.append(
                        "item_price = CASE WHEN (item_price IS NULL OR item_price = '') THEN ? ELSE item_price END"
                    )
            ...
            with self.lock:
                ...
                if row:
                    ...
                    if item_info.get("item_detail"):
                        try:
                            item_info["item_detail_parsed"] = json.loads(
    ...
    def update_item_multi_spec_status(
        self, cookie_id: str, item_id: str, is_multi_spec: bool
    ) -> bool:
        """更新商品的多规格状态"""
        try:
            with self.lock:
    ...
    def update_item_multi_quantity_delivery_status(
        self, cookie_id: str, item_id: str, multi_quantity_delivery: bool
    ) -> bool:
        """更新商品的多数量发货状态"""
        try:
            with self.lock:
                ...
                if cursor.rowcount > 0:
                    self.conn.commit()
                    logger.info(
                        f"更新商品多数量发货状态成功: {item_id} -> {multi_quantity_delivery}"
                    )
                    return True
                else:
    ...
    def get_item_multi_quantity_delivery_status(
        self, cookie_id: str, item_id: str
    ) -> bool:
        """获取商品的多数量发货状态"""
        try:
            with self.lock:
    ...
    def update_item_detail(
        self, cookie_id: str, item_id: str, item_detail: str
    ) -> bool:
        """更新商品详情（不覆盖商品标题等基本信息）
    ...
    def update_item_title_only(
        self, cookie_id: str, item_id: str, item_title: str
    ) -> bool:
        """仅更新商品标题（并发安全）
            ...
            with self.lock:
                ...
                for item_data in items_data:
                    try:
                        ...
                        logger.warning(
                            f"批量保存单个商品失败 {item_data.get('item_id', 'unknown')}: {item_e}"
                        )
                        continue

                cursor.execute("COMMIT")
                logger.info(
                    f"批量保存商品信息完成: {success_count}/{len(items_data)} 个商品"
                )
            ...
            with self.lock:
                ...
                for item_data in items_data:
                    try:
                        ...
                        cursor.execute(
                            update_sql,
                            (item_title, item_price, item_category, cookie_id, item_id),
                        )
                        ...
                        logger.warning(
                            f"批量更新单个商品失败 {item_data.get('item_id', 'unknown')}: {item_e}"
                        )
                        continue

                cursor.execute("COMMIT")
                logger.info(
                    f"批量更新商品标题和价格完成: {success_count}/{len(items_data)} 个商品"
                )
                return success_count
            ...
            with self.lock:
                ...
                for item_data in items_to_delete:
                    try:
                        ...
                        logger.warning(
                            f"删除单个商品失败 {item_data.get('item_id', 'unknown')}: {item_e}"
                        )
                        continue

                cursor.execute("COMMIT")
                logger.info(
                    f"批量删除商品信息完成: {success_count}/{len(items_to_delete)} 个商品"
                )
                return success_count
        ...
        with self.lock:
            try:
                ...
                for row in cursor.fetchall():
                    ...
                    if has_is_admin:
                        user_data["is_admin"] = (
                            bool(row[5]) if row[5] is not None else (row[1] == "admin")
                        )
                    else:
                        user_data["is_admin"] = row[1] == "admin"
    ...
    def update_user_admin_status(self, user_id: int, is_admin: bool) -> bool:
        ...
        with self.lock:
            try:
                ...
                self.conn.commit()
                logger.info(
                    f"用户管理员状态更新成功: user_id={user_id}, is_admin={is_admin}"
                )
                return cursor.rowcount > 0
            except Exception as e:
    ...
    def delete_user_and_data(self, user_id: int):
        ...
        with self.lock:
            try:
                ...
                # 3. 删除用户的发货规则
                cursor.execute(
                    "DELETE FROM delivery_rules WHERE user_id = ?", (user_id,)
                )
    ...
    def insert_or_update_order(
        ...
        buyer_nick: str = None,
        pre_refund_status=...,
        clear_pre_refund_status: bool = False,
        bargain_flow_detected=...,
        bargain_success_detected=...,
    ):
        """插入或更新订单信息
        ...
        with self.lock:
            try:
                ...
                if has_pre_refund_status:
                    normalized_pre_refund_status = self._normalize_order_status(
                        pre_refund_status
                    )
                ...
                if cookie_id:
                    ...
                    if not cookie_exists:
                        logger.warning(
                            f"Cookie ID {cookie_id} 不存在于cookies表中，拒绝插入订单 {order_id}"
                        )
                        return False
                ...
                if existing:
                    ...
                    if buyer_id is not None:
                        if self._is_valid_buyer_id(buyer_id):
                            ...
                            logger.debug(
                                f"跳过无效buyer_id覆盖: order_id={order_id}, invalid_buyer_id={buyer_id}"
                            )
                    ...
                    sanitized_buyer_id = (
                        buyer_id if self._is_valid_buyer_id(buyer_id) else None
                    )
                    insert_fields = [
                        "order_id",
                        "item_id",
    ...
    def get_order_pre_refund_status(self, order_id: str) -> str:
        ...
        with self.lock:
            try:
                cursor = self.conn.cursor()
                cursor.execute(
                    "SELECT pre_refund_status FROM orders WHERE order_id = ?",
                    (order_id,),
                )
                row = cursor.fetchone()
                if not row:
    ...
    def update_buyer_nick_by_buyer_id(
        self, buyer_id: str, buyer_nick: str, cookie_id: str = None
    ):
        """根据买家ID更新所有相关订单的买家昵称
        ...
        with self.lock:
            try:
                ...
                if updated_count > 0:
                    logger.info(
                        f"已更新买家 {buyer_id} 的 {updated_count} 个订单昵称为: {buyer_nick}"
                    )
    ...
    def get_recent_order_by_buyer_id(
        self,
        buyer_id: str,
        cookie_id: str = None,
        status: str = None,
            ...
            try:
                ...
                if status:
                    ...
                    if normalized_status == "pending_ship":
                        conditions.append(
                            "(order_status = ? OR order_status = ? OR order_status = ? OR order_status = ?)"
                        )
                        params.extend(
                            [
                                "pending_ship",
                ...
                if row:
                    logger.info(
                        f"根据买家ID找到最近订单: buyer_id={buyer_id}, order_id={row[0]}, item_id={row[1]}"
                    )
                    return {
                        "order_id": row[0],
                        "item_id": row[1],
                ...
                logger.warning(
                    f"未找到买家 {buyer_id} 的最近订单 (cookie_id={cookie_id}, status={status}, minutes={minutes})"
                )
                return None
    ...
    def get_recent_order_by_sid(
        self, sid: str, cookie_id: str = None, status: str = None, minutes: int = 10
    ):
        ...
        with self.lock:
            try:
                ...
                # 添加时间限制
                conditions.append(
                    "datetime(COALESCE(updated_at, created_at)) >= datetime('now', ?)"
                )
                params.append(f"-{minutes} minutes")
                ...
                if row:
                    logger.info(
                        f"根据sid找到最近订单: sid={sid}, order_id={row[0]}, item_id={row[1]}"
                    )
                    return {
                        "order_id": row[0],
                        "item_id": row[1],
                ...
                logger.warning(
                    f"未找到sid {sid} 的最近订单 (cookie_id={cookie_id}, status={status}, minutes={minutes})"
                )
                return None
            ...
            try:
                ...
                if statuses:
                    ...
                    for status in statuses:
                        normalized_status = (
                            self._normalize_order_status(status) or status
                        )
                ...
                if not conditions:
                    logger.warning(
                        "find_recent_orders_by_match_context 缺少有效查询条件，拒绝全表扫描"
                    )
                    return []
    ...
    def update_order_yifan_status(
        self,
        order_id: str,
        yifan_orderno: str = None,
        ...
        with self.lock:
            try:
                ...
                current_order_status = (
                    existing_order[1] if len(existing_order) > 1 else None
                )
                ...
                if delivery_status is not None:
                    ...
                    merged_order_status = self.resolve_external_order_status(
                        current_order_status, delivery_status, source="yifan_status"
                    )
                    normalized_current_status = self._normalize_order_status(
                        current_order_status
                    )
                    if (
                        merged_order_status
                        and merged_order_status != normalized_current_status
                    ):
                        update_fields.append("order_status = ?")
    ...
    def update_order_chat_id(self, order_id: str, chat_id: str):
        ...
        with self.lock:
            try:
                ...
                cursor.execute(
                    "UPDATE orders SET chat_id = ? WHERE order_id = ?",
                    (chat_id, order_id),
                )
                self.conn.commit()
                return True
    ...
    def delete_table_record(self, table_name: str, record_id: str):
        ...
        with self.lock:
            try:
                ...
                # 删除记录
                cursor.execute(
                    f"DELETE FROM {table_name} WHERE {primary_key} = ?", (record_id,)
                )
                ...
                if cursor.rowcount > 0:
                    ...
                    logger.warning(
                        f"删除表记录失败，记录不存在: {table_name}.{record_id}"
                    )
                    return False
    ...
    def upgrade_keywords_table_for_image_support(self, cursor):
        ...
        try:
            ...
            if "type" not in columns:
                ...
                cursor.execute(
                    "ALTER TABLE keywords ADD COLUMN type TEXT DEFAULT 'text'"
                )
    ...
    def get_item_replay(self, item_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            with self.lock:
                ...
                if row:
                    (reply_content,) = row
                    return {"reply_content": reply_content or ""}
                return None
        except Exception as e:
    ...
    def update_item_reply(
        self, cookie_id: str, item_id: str, reply_content: str
    ) -> bool:
        """
    ...
    def _build_legacy_verification_meta(
    ...
    def _infer_legacy_risk_trigger_scene(
        ...
        combined_text = " ".join(
            part for part in (description, processing_result, error_message) if part
        )
        ...
        if (
            "连续失败5次" in description
            or "关键api不可用" in lower_text
            or "cookie验证失败" in description
        ):
            return "auto_cookie_refresh"

        if (
            "token刷新" in combined_text
            or "令牌" in combined_text
            or "session过期" in lower_text
            or "token" in lower_text
        ):
    ...
    def _get_risk_trigger_scene_label(
        self, trigger_scene: Optional[str]
    ) -> Optional[str]:
    ...
    def _compact_legacy_risk_description(self, log_info: Dict[str, Any]) -> str:
        ...
        event_type = str(log_info.get("event_type") or "").strip()
        trigger_scene = self._get_risk_trigger_scene_label(
            log_info.get("trigger_scene")
        )
        lower_description = description.lower()

        if event_type == "slider_captcha" and (
            "滑块验证" in description or "url:" in lower_description
        ):
            return (
                f"检测到滑块验证（{trigger_scene}）"
                if trigger_scene
                else "检测到滑块验证"
            )
        ...
        compacted = re.sub(
            r"[，,]?\s*URL[:：]\s*https?://\S+", "", description, flags=re.IGNORECASE
        )
    ...
    def _compact_legacy_risk_processing_result(self, log_info: Dict[str, Any]) -> str:
        ...
        if event_type == "slider_captcha":
            ...
            if reason_match:
                ...
                if (
                    "未获取到新cookies" in reason
                    or "未获取到新cookie" in reason.lower()
                ):
                    reason = "未获取到新Cookie"
            ...
            if (
                "触发闲鱼风控验证" in processing_result
                or "触发闲鱼风控验证" in error_message
            ):
        ...
        compacted = re.sub(
            r"[，,]\s*耗时[:：]\s*[0-9]+(?:\.[0-9]+)?\s*秒", "", processing_result
        )
        compacted = re.sub(
            r"[，,]\s*cookies?长度[:：]?\s*\d+", "", compacted, flags=re.IGNORECASE
        )
    ...
    def _compact_legacy_risk_error_message(self, log_info: Dict[str, Any]) -> str:
        ...
        compacted = re.sub(
            r"[，,]?\s*URL[:：]\s*https?://\S+", "", error_message, flags=re.IGNORECASE
        )
        compacted = re.sub(r"https?://\S+", "", compacted)
        compacted = compacted.replace("  ", " ")
        compacted = compacted.strip(" ，,;；")
    ...
    def _normalize_legacy_risk_log(self, log_info: Dict[str, Any]) -> Dict[str, Any]:
        ...
        is_legacy = not any(
            [session_id, trigger_scene, result_code, raw_meta, duration_ms]
        )
        ...
        normalized["event_description_display"] = (
            self._compact_legacy_risk_description(normalized)
            or normalized.get("event_description")
            or "-"
        )
        if is_legacy:
            normalized["processing_result_display"] = (
                self._compact_legacy_risk_processing_result(normalized)
                or normalized.get("processing_result")
                or ""
            )
            normalized["error_message_display"] = (
                self._compact_legacy_risk_error_message(normalized)
                or normalized.get("error_message")
                or ""
            )
        else:
            normalized["processing_result_display"] = (
                normalized.get("processing_result") or ""
            )
            normalized["error_message_display"] = normalized.get("error_message") or ""
        normalized["is_legacy"] = is_legacy
        normalized["session_display"] = session_id or (
            "历史记录" if is_legacy else "--"
        )
        return normalized

    def _normalize_risk_log_datetime_param(
    ...
    def _build_risk_control_log_filters(
        ...
        normalized_from = self._normalize_risk_log_datetime_param(
            date_from, end_of_day=False
        )
        if normalized_from:
            conditions.append(f"datetime({prefix}created_at) >= datetime(?)")
            params.append(normalized_from)

        normalized_to = self._normalize_risk_log_datetime_param(
            date_to, end_of_day=True
        )
        if normalized_to:
            conditions.append(f"datetime({prefix}created_at) <= datetime(?)")
        ...
        try:
            with self.lock:
                ...
                for row in cursor.fetchall():
                    ...
                    log_info["event_meta"] = self._decode_risk_control_event_meta(
                        log_info.get("event_meta")
                    )
    ...
    def get_slider_verification_session_stats(
        self, cookie_ids: Optional[List[str]] = None
    ) -> Dict[str, Any]:
        """获取滑块验证会话级统计数据。"""
        empty_stats = {
            "has_data": False,
        ...
        try:
            ...
            with self.lock:
                ...
                success_rate = (
                    round((success_count / completed_sessions) * 100, 1)
                    if completed_sessions > 0
                    else 0.0
                )

                def _fetch_recent_datetime(
                    extra_condition: str, extra_params: List[Any]
                ) -> Optional[str]:
                ...
                if total_sessions > 0:
                    summary_text = (
                        "已包含历史滑块成功/失败，并将账密刷新中的滑块失败计入失败次数"
                    )
                else:
                    summary_text = "暂无滑块验证记录"
                ...
                return {
                    ...
                    "recent_success": _fetch_recent_datetime(
                        "event_type = ? AND processing_status = ?",
                        ["slider_captcha", "success"],
                    ),
    ...
    def mark_stale_risk_control_logs_failed(
        self, timeout_minutes: int = 15, cookie_id: str = None
    ) -> int:
        ...
        try:
            with self.lock:
                ...
                try:
                    ...
                    if cursor.rowcount > 0:
                        logger.info(
                            f"清理了 {cursor.rowcount} 条过期的AI对话记录（{days}天前）"
                        )
                except Exception as e:
                ...
                try:
                    ...
                    if cursor.rowcount > 0:
                        logger.info(
                            f"清理了 {cursor.rowcount} 条过期的风控日志（{days}天前）"
                        )
                except Exception as e:
                ...
                try:
                    ...
                    if cursor.rowcount > 0:
                        logger.info(
                            f"清理了 {cursor.rowcount} 条过期的AI商品缓存（{cache_days}天前）"
                        )
                except Exception as e:
                ...
                if total_cleaned > 100:
                    logger.info(
                        f"共清理了 {total_cleaned} 条记录，执行VACUUM以释放磁盘空间..."
                    )
    ...
    def calculate_next_daily_run(
        self, run_hour, random_delay_max=10, include_today=True
    ):
        """计算每日定时任务的下次运行时间"""
        ...
        next_run = now.replace(
            hour=safe_hour, minute=random_min, second=0, microsecond=0
        )
        ...
        with self.lock:
            try:
                ...
                next_run_value = next_run_at or self.calculate_next_daily_run(
                    delay_minutes, random_delay_max, include_today=True
                )
                    ...
                    tasks.append(
                        {
                            ...
                            "delay_minutes": row[6],
                            "random_delay_max": row[7],
                            "next_run_at": row[8],
                            "last_run_at": row[9],
                            "last_run_result": row[10],
                            "user_id": row[11],
                            "created_at": row[12],
                            "updated_at": row[13],
                        }
    ...
    def delete_scheduled_task(self, task_id):
        ...
        with self.lock:
            try:
                cursor = self.conn.cursor()
                self._execute_sql(
                    cursor, "DELETE FROM scheduled_tasks WHERE id = ?", (task_id,)
                )
                self.conn.commit()
                return cursor.rowcount > 0
            except Exception as e:
